...
\begin{exercise}
\label{ex:13.1}
\textbf{Comparing $\tau_{\mathrm{T}},\tau_{\mathrm{C}}$, and $\tau$}

Assume $Z \perp \{Y(1), Y(0)\} \mid X$. Recall $e(X) = \pr(Z = 1 \mid X)$ is the propensity score, $e = \pr(Z = 1)$ is the marginal probability of the treatment, and $\tau(X) = E\{Y(1) - Y(0) \mid X\}$ is the CATE. Show that
\[
\tau_{\mathrm{T}} - \tau = \frac{\cov\{e(X), \tau(X)\}}{e}, \quad \tau_{\mathrm{C}} - \tau = - \frac{\cov\{e(X), \tau(X)\}}{1 - e}.
\]
...
\begin{exercise}
\label{ex:13.6}
\textbf{Estimating individual effect and conditional average causal effect}
...
\begin{enumerate}
\item Under randomization with $Z_{i} \indsim \{Y_{i}(1), Y_{i}(0)\}$ and $e = \pr(Z_{i} = 1)$, show that
\[
...
\item Under ignorability with $Z_{i} \indsim \{Y_{i}(1), Y_{i}(0)\} \mid X_{i}$ and $e(X_{i}) = \pr(Z_{i} = 1 \mid X_{i})$, show that
\[
...
\begin{exercise}
\label{ex:13.10}
\textbf{Doubly robust estimation for general estimand}
...
\begin{exercise}
\label{ex:14.3}
\textbf{Weighted logistic regression with a binary outcome}
...
\begin{exercise}
\label{ex:17.1}
\textbf{A technical lemma for the proof of Theorem 17.1}
...
\begin{exercise}
\label{ex:17.2}
\textbf{Technical assumption for Theorem 17.1}
...
\begin{exercise}
\label{ex:17.6}
\textbf{Cornfield-type inequalities for the risk difference}

Consider binary $Z, Y, U$, and condition on $X$ implicitly. Assume latent ignorability given $U$. Show that under $Z \perp Y \mid U$, we have
\[
...
Denote $\mu_z(X) = \mathbb{E}(Y \mid Z = z, X)$ and $e(X) = \pr(Z = 1 \mid X)$.
Suppose that
\[
...
\begin{exercise}
\label{ex:sens-c}
\textbf{(Optional)} Derive the doubly robust estimation formula.
\end{exercise}
