...
import type { AskOptions, AskResult, GeminiMode } from '../types/index.js';

const wait = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));

export async function geminiAsk(question: string, options: AskOptions = {}): Promise<AskResult> {
...
async function switchMode(page: Page, mode: GeminiMode): Promise<void> {
  try {
    // 点击模式选择器
...
async function waitForResponseImproved(page: Page, timeout: number): Promise<void> {
...
async function extractResponse(page: Page): Promise<string> {
  ...
  try {
    ...
    for (const container of containers) {
      const text = await page.evaluate((el: Element) => el.textContent || '', container);
      // 清理并返回
      const cleaned = text
  ...
  for (const line of lines) {
    ...
    if (inGeminiResponse) {
      if (line.trim() === '' && responseLines.length > 0) break;
      if (line.includes('PRO') || line.includes('Sign in') || line.includes('Gemini is AI')) continue;
      responseLines.push(line.trim());
      if (responseLines.length > 10) break;
    }
  }
