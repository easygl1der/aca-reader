...
**\cref{} 为空是严重错误**：如果 `\ref{xxx}` 显示为 `??`，说明目标不存在。

> **⚠️ 不要删除引用**：引用为空时，**不是删除引用**，而是在教材中找到对应内容，将该公式/图片/例题/章节**添加到笔记中**，创建 `\label{}`，再引用。
...
| 检查项 | 标准 | 如何验证 |
|--------|------|----------|
| **禁止 `\tag{}`** | 笔记中所有公式/图片引用必须用 `\label{}` + `\cref{}/\eqref{}`，禁止 `\tag{}` | 搜索 `\tag{` 无结果 |
| **\cref{} 不为空** | 每个 `\cref{xxx}` / `\eqref{xxx}` 引用的目标必须存在（编译后不显示 `??`） | 编译后检查 `??` |
| **不删除引用** | 如果引用目标不存在，**必须添加内容 + label**，禁止删除引用 | 检查无 `\cref{xxx}` 被删除 |
| **硬编码已替换** | 所有 `(X.Y)`、`Example X.Y`、`Figure X.Y` 等已转为 `\cref{}` 或 `\eqref{}` | 搜索硬编码模式无结果 |

### 格式检查

| 检查项 | 标准 |
|--------|------|
| exercise 环境大小写 | do Carmo 模板用 `exercise`（小写），其他用 `Exercise`（大写） |
| 习题编号 | 与教材原书编号一致 |
| 标签命名 | `exr:{章号}-{题号}`，如 `exr:5-1`、`exr:3-7` |
| 中文专有名词 | 保留英文原文 |
...
# 2. 检查是否有未解析的引用（??）
grep -n "??\|undefined reference" notes/*/chapters/chapter*.tex

# 3. 确认无 \tag{} 使用
grep -rn "\\\\tag{" notes/*/chapters/
```

> **\tag{} 自检命令**：如果上述命令有输出，说明存在违规使用 `\tag{}` 的情况。
