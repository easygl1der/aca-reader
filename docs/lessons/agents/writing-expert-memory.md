...
## L725: BDA Chapter 5 (Hierarchical Models) 写作教训

**日期**: 2026-05-10
**来源**: 为 BDA Chapter 5 分层模型撰写笔记
**累计次数**: 1 次

**写作内容**:
- BDA Chapter 5 分层模型：可交换性、层次模型三层次、伪贝叶斯/经验贝叶斯、正态层次模型、收缩性质、SAT 培训效果研究、meta 分析、弱信息先验
- 共 9 节 + 6 道习题

**做得好的地方**:
1. **正确使用 BDA 原书 SAT 数据**：从 PDF 中读取了 Table 5.2 的实际数据（A:28/14, B:8/10, C:-3/16, D:7/11, E:-1/9, F:1/8, G:18/12, H:12/15）
2. **覆盖核心内容**：伪贝叶斯/经验贝叶斯、收缩性质等 BDA 5.5/5.6/5.10 节内容
3. **Stein 风格动机**：导论中强调"当我们为每个参数独立指定先验时...然而..."的对比模式
4. **历史脉络**：添加了 de Finetti (1930)、Lindley & Smith (1972)、Morris (1983) 的引用

**发现的问题**:
1. **transcript vs PDF 数据不一致**：transcript 标记"See the textbook"表示详细数据在 PDF 中，需要自行提取
2. **reviewer 使用了不同章节编号**：reviewer 引用的 5.1-5.11 节编号可能来自 BDA 第3版，而我用的 transcript 来自第4版

**防止措施**:
- 使用 transcript 时注意"See the textbook"标记，这意味着需要从 PDF 中提取数据
- 章节编号差异可能来自不同版本，核心是覆盖实际内容而非匹配编号
- 发送修订版时明确说明哪些问题已修复，避免 reviewer 用旧标准评估新版本
