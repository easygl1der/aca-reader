...
统计学家 Rubin 将缺失机制分为三类，这一分类至今仍是该领域的标准框架。
...
\begin{Remark}
  MCAR 是最强的假设，意味着缺失是"真正随机"的。MAR 是实际应用中更常见的假设，许多多重插补方法都建立在 MAR 基础上。MNAR 最难处理，因为它要求对缺失机制进行明确的建模。
\end{Remark}
...
多重插补（multiple imputation）是一种实用的缺失数据处理技术，通过创建多个（通常是 $m = 5$ 到 $m = 50$）完整数据集来反映缺失数据的不确定性。
...
其中 $\bar{U} = \frac{1}{m} \sum_{l=1}^m U_l$ 是within-imputation 方差，$B = \frac{1}{m-1} \sum_{l=1}^m (\hat{Q}_l - \bar{Q})^2$ 是 between-imputation 方差。
...
\begin{Remark}
  多重插补的关键洞见是：总方差 $T$ 包括两个部分——来自数据分析的抽样方差 $\bar{U}$ 和来自缺失数据不确定性 的插补间方差 $B$。忽视后者会严重低估总体不确定性。
\end{Remark}
...
\begin{Example}[基于 Pattern-Mixture 的敏感度分析]
