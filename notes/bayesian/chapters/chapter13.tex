...
\textbf{拉普拉斯方法}是逼近形式为 $h(\theta)p(\theta|y)$ 的积分的解析工具，其中 $p(\theta|y)$ 是后验密度（已归一化），$h(\theta)$ 是某个光滑函数。其核心思想是用 $p(\theta|y)$ 的众数处的正态近似来代替 $p$ 本身：
...
\begin{Theorem}[EM 算法的单调性]\labelthm{em-monotonicity}
...
代入 \cref{eq:normal-approx} 中的 Q 函数，再做 M 步最大化，可得
\[
...
\subsection{消息传递框架}\label{sec:message-passing}

\textbf{期望传播}（Expectation Propagation，EP）可以看作是变分贝叶斯的兄弟方法，但采用了一种不同的近似策略。EP 属于"消息传递"（message passing）框架：它将后验分布分解为若干因子的乘积 $p(\theta|y) \propto \prod_f m_f(\theta)$，每个因子对应模型中的一个组件（如先验、似然项）。

EP 的目标是：用一组近似因子 $\tilde{m}_f(\theta)$ 替代真实因子，使得近似后验 $q(\theta) \propto \prod_f \tilde{m}_f(\theta)$ 具有正确的各阶矩。这一过程通过\textbf{迭代 refinement} 实现：每次选一个因子，用真实因子在该因子当前估计处的动差（moments）替换，同时保持其余因子不变。

与变分贝叶斯相比，EP 更关注矩匹配（moment matching），因此在需要精确后验矩的场景下往往更准确。但 EP 对近似因子形式的选择更敏感，且在高维问题中可能不稳定。

\begin{Remark}[EP 与变分贝叶斯的关系]
两者都是通过迭代优化来逼近后验。变分贝叶斯最小化 $\text{KL}(q\|p)$，而 EP 最小化 $\text{KL}(p\|q)$（在每个因子的层面上）。KL 方向的选择影响近似行为：$\text{KL}(q\|p)$ 导致 $q$ 覆盖 $p$ 的支撑（覆盖性过强），而 $\text{KL}(p\|q)$ 导致 $q$ 过于集中（低估方差）。这是理解两种方法差异的关键。
\end{Remark}
...
验证 \cref{ex:normal-mean-variance-em} 中 EM 算法的 M 步公式（13.9）。并编程实现该算法，检查对数边缘后验密度是否随迭代单调递增。
\end{Exercise}
...
证明变分贝叶斯中，坐标上升算法（即交替更新各变分因子）保证 $\text{KL}(q\|p)$ 单调下降。（提示：利用 \cref{eq:variational-update} 中的最优 $q_j^*$ 形式，以及 KL 散度的非负性。）
\end{Exercise}
