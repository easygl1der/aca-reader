...
\begin{Remark}
  经验贝叶斯方法的核心问题：用点估计 $\hat{\phi}$ 代替超参数的后验分布，忽略了对 $\phi$ 的不确定性。在小样本情况下，这种不确定性可能很大。
\end{Remark}
...
\[
p(\theta_j \mid y) = \iint p(\theta_j \mid \alpha, \beta) \, p(\alpha, \beta \mid y) \, d\alpha \, d\beta. \footnote{详细推导见附录 \cref{sec:beta-binomial-marginal}}
\]
...
设置 $\hat{\tau}^2 = 0$ 当 $MSW > MSB$ 似乎过于强烈——这相当于声称所有组均值完全相同，而不是说样本量太小无法区分 $\tau^2$ 与零。
