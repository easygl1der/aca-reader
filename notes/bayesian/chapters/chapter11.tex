...
\begin{Remark}
  重要性抽样的效果高度依赖于 $g(\theta)$ 和 $p(\theta | y)$ 的匹配程度。如果 $g(\theta)$ 在 $p(\theta | y)$ 重要的区域取值很小，重要性权重会变得极不稳定（方差无穷大）。
\end{Remark}
...
拒绝抽样的期望接受率为
\[
\text{accept rate} = \frac{1}{M} \int \frac{p(\theta)}{g(\theta)} \cdot g(\theta) \, d\theta = \frac{1}{M}
\]
...
一个充分条件是\textbf{细致平衡（detailed balance）}：
\[
...
1953 年，Metropolis、Rosenbluth、Rosenbluth、Teller 和 Teller 在一项关于分子物理的工作中提出了著名的 Metropolis 算法。这一算法后来被 Hastings 在 1970 年推广为更一般的形式。
...
可以验证，Metropolis 算法构造的马尔可夫链满足细致平衡，因此其平稳分布正是目标分布 $p(\theta | y)$。
...
\begin{Example}[一维高斯目标分布]
  ...
  接受比 $r = \exp\left(-\frac{1}{2}(\theta^*)^2 + \frac{1}{2}(\theta^{(t)})^2\right)$。

  如果候选值在概率密度更高的区域（更接近 0），则 $r > 1$，必然被接受；如果在更低区域，以一定概率被拒绝。
\end{Example}
...
考虑更新 $\theta_j$ 时，提议分布就是条件分布 $q(\theta_j' | \theta_{-j}) = p(\theta_j' | \theta_{-j}, y)$。

接受比为
\[
r = \frac{p^*(\theta_j', \theta_{-j})}{p^*(\theta_j, \theta_{-j})} \cdot \frac{p(\theta_j | \theta_{-j}, y)}{p(\theta_j' | \theta_{-j}, y)} = 1
\]
因为分子分母中的条件分布恰好互为倒数。因此 Gibbs 抽样总是接受提议值。
...
使用 Gibbs 抽样，我们可以依次从以下条件分布抽样：

\begin{enumerate}
  \item $p(\theta_j | \mu, \tau, y)$：这是正态分布，均值和方差都是解析的
