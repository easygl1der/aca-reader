...
混合正态分布具有以下性质：
\begin{enumerate}
  \item 任何平滑的密度函数都可以用足够多分量的混合正态分布任意逼近（Stone-Weierstrass定理的推论）
...
EM算法（Expectation-Maximization）正是利用了这种思想：通过迭代地估计潜在变量（E步）和更新参数（M步），来最大化观测数据的似然。
...
Gibbs采样器的收敛性可以通过监控多链的 Gelman-Rubin 统计量来诊断。
...
\begin{itemize}
  \item \cite{Mclaut2007}：混合模型的经典教材
  \item \cite{Fruwirth2010}：有限混合模型的统计推断
