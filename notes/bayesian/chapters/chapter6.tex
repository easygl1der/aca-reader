...
在前几章中，我们学习了贝叶斯推断的核心工具——如何从先验和似然出发，计算后验分布，以及如何构建层次模型来合并来自不同来源的信息。然而，在实际应用中，一个关键问题始终萦绕着我们：**我们如何知道所选的模型是否合适？**
...
\begin{Remark}
  后验预测 p 值与经典 p 值有本质区别。经典 p 值假设数据是随机化的，而**后验预测 p 值是条件于观测数据的**，反映的是模型对观测数据的拟合程度。
\end{Remark}
