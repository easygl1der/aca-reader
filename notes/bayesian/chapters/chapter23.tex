...
权重 $w_k$ 的期望为 $\mathbb{E}w_k = 1/\alpha$ 级别的衰减——$\alpha$ 越大，权重衰减越慢，有效成分个数越多。

\subsection{Pólya urn 构造}

DP 还有一种更为概率的构造——Pólya urn scheme。设我们有颜色（atoms）集合，urn 中每次加入一个球：
...
\[
G | \mathbf{x} \sim \DP\!\left(\alpha + n, \frac{\alpha}{\alpha + n} G_0 + \frac{1}{\alpha + n} \sum_{i=1}^n \delta_{x_i}\right).
\]

这一结果表明：后验等价于在先验 $G_0$ 的基础上"添加"了经验分布的 contribution。具体而言：
\begin{itemize}
  \item 有效样本量从 $\alpha$ 增加到 $\alpha + n$；
  \item 后验基分布是 $G_0$ 与经验分布 $\frac{1}{n}\sum_i \delta_{x_i}$ 的加权平均，权重分别为 $\alpha/n$ 和 $n/n$ 的某种调整。
\end{itemize}

预测分布也有简洁形式。设新观测为 $\tilde{x}$，则其后验预测分布为：\footnote{推导见附录 \cref{sec:dp-predictive-derivation}}
\[
p(\tilde{x} | \mathbf{x}) = \frac{\alpha}{\alpha + n} \, G_0(\tilde{x}) + \frac{1}{\alpha + n} \sum_{i=1}^n \delta_{x_i}(\tilde{x}).
\]

也就是说，新数据点有概率 $\alpha/(\alpha+n)$ 来自基分布 $G_0$（产生一个新atom），有概率 $n_k/(\alpha+n)$ 与已有数据点 $x_i$ 相同（与同一 cluster 的数据共享）。这正是 CRP 的 predictive probability。
