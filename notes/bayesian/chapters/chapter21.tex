...
\begin{Remark}
  后验预测方差的解析表达式 \eqref{eq:gp-posterior-variance} 有一个值得注意的性质：即使我们只观测了 $y$ 而非 $f$ 本身，方差公式中的减项也自动反映了数据对函数值的约束。换言之，\textbf{高斯过程的后验方差对函数值的不确定性衰减速度由协方差函数决定}，而不是由观测值本身决定。
\end{Remark}
...
\begin{Example}[\cref{ex:gp-birthday-model} 的验证]\label{ex:gp-birthday-results}
...
\begin{Remark}
  GPC 的一个关键计算瓶颈是：每次超参数更新都需要重新计算协方差矩阵 $K$ 和它的逆。对于大规模数据集，$O(n^3)$ 的计算复杂度成为瓶颈。稀疏近似（sparse approximation）通过引入 $m \ll n$ 个诱导点（inducing points）将计算复杂度降为 $O(n m^2)$。\footnote{稀疏高斯过程近似的详细推导见附录 \cref{sec:sparse-gp-derivation}。}
\end{Remark}
...
当 $n$ 达到数万甚至更多时，精确高斯过程变得不可行。稀疏近似方法通过引入 $m$ 个诱导点（inducing points）$\{u_j\}_{j=1}^m$ 来降低复杂度：

\begin{enumerate}
  \item \textbf{策略近似（Subset of Variables）}：仅使用 $m \ll n$ 个随机选取的训练点
  \item \textbf{诱导点近似（Inducing Points）}：引入独立的诱导输入点 $Z = \{z_j\}$ 和诱导输出 $u_j = f(z_j)$，然后用 $p(f_* | y) \approx \int p(f_* | u) q(u | y) \, du$ 近似
\end{enumerate}
