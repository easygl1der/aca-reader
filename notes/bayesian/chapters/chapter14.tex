...
历史地看，回归分析的发展经历了几个阶段。Legendre 和 Gauss 在 19 世纪初发展了最小二乘法，Fisher 在 20 世纪初建立了估计的统计理论框架。在贝叶斯传统中，回归分析长期以来被视为一个计算上的挑战——直到马尔科夫链蒙特卡洛方法的出现。1980 年前后，随着计算方法的进步，贝叶斯回归分析才真正进入实用阶段。
...
\[
\beta_n = V_n(X^T X\hat{\beta} + V_0^{-1}\beta_0), \quad V_n = (X^TX + V_0^{-1})^{-1}
\]
...
\begin{Remark}
  Jeffreys' 先验背后的直觉是：Fisher 信息矩阵的行列式给出了参数空间中局部曲率的度量。Jeffreys' 先验与这些曲率成正比，确保先验不会人为地偏向参数空间的某些区域。
\end{Remark}
...
g-先验的一个重要性质是后验均值可以在最小二乘估计 $\hat{\beta}$ 和先验均值 $\beta_0$ 之间实现线性插值：
...
另一种思路是从变量选择的角度出发，使用 spike-and-slab 先验或 horseshoe 先验，将大多数回归系数推向零，同时允许少数重要变量保留非零系数。
...
这标志着一个重要的计算转折点：我们必须借助第 10-12 章介绍的模拟方法来处理这类非共轭模型。常用的策略包括：

\begin{enumerate}
  \item \textbf{Metropolis 算法}：使用对称提议分布（如正态提议）在后验分布上生成马尔科夫链。
  \item \textbf{Hamiltonian 蒙特卡洛}：利用梯度信息构造更高效的提议机制，特别适合高维参数空间。
  \item \textbf{拉普拉斯近似}：用正态分布逼近后验模式附近的后验密度，结合重要性抽样或粒子滤波器。
\end{enumerate}
...
\begin{equation}
\epsilon_i \sim (1-\pi)\text{Normal}(0, \sigma^1) + \pi\text{Normal}(0, \sigma^2), \quad \sigma^2 > \sigma^1
...
\begin{Exercise}{\ref{exr:ch14-3} 逻辑回归的后验预测}
\label{exr:ch14-3}
描述如何使用 Metropolis 算法从逻辑回归模型的后验分布 $p(\beta | y)$ 中抽取样本，并说明如何利用这些样本计算后验预测分布 $\Pr(\tilde{y} = 1 | y)$。
\end{Exercise}
