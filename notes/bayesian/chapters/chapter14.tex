...
值得注意的是，$\beta_n$ 的表达式可以写成数据的最大似然估计 $\hat{\beta}$ 与先验均值 $\beta_0$ 的加权平均：
\[
\beta_n = \frac{n}{n + \text{tr}(V_0X^TX)}\hat{\beta} + \frac{\text{tr}(V_0X^TX)}{n + \text{tr}(V_0X^TX)}\beta_0 + O(\text{高阶项})
\]
