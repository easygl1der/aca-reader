...
Figure \ref{fig:bioassay-posterior} 展示了后验分布的散点图以及对应的剂量-反应曲线。\footnote{Bioassay 后验模拟的 R 代码实现见附录 \cref{sec:bioassay-rcode}。}
...
其中 $z^{(t)}$ 是调整后的响应变量，$W^{(t)}$ 是权重矩阵。\footnote{IRLS 算法的详细推导见附录 \cref{sec:irls-derivation}。}
