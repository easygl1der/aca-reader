...
\textbf{决策分析}（decision analysis）正是连接推断与决策的桥梁。这一理论框架由 Wald (1950)、Savage (1954)、Raiffa 和 Schlaifer (1961) 等人在 20 世纪中叶发展起来，其核心思想是：将决策问题形式化为最大化期望效用（expected utility）。贝叶斯版本的决策理论进一步要求，在计算期望效用时，使用后验分布来量化不确定性。

本章的安排如下：\S 9.1 引入效用函数与损失函数的基本概念；\S 9.2 讨论几种常见的损失函数及其对应的贝叶斯最优决策；\S 9.3 探讨后验预测分布与预验分析（preposterior analysis）；\S 9.4 处理层次决策问题与经验贝叶斯的联系。
...
\begin{Example}[医学治疗决策]
  考虑一个简化的医学决策问题：医生需要决定是否对患者进行某种手术。真实参数 $\theta$ 代表手术的实际成功率。设损失函数为
  \[
  \ell(\theta, d) = \begin{cases}
    0 & \text{若 } d = \text{做手术且成功} \\
    1 & \text{若 } d = \text{做手术且失败} \\
    0.5 & \text{若 } d = \text{不做手术}.
  \end{cases}
  \]
  这里"不做手术"被视为一种保守决策，当手术失败后果严重时给予较大的损失权重。
\end{Example}

\subsection{期望后验损失与贝叶斯最优决策}

给定损失函数 $\ell(\theta, d)$ 和后验分布 $p(\theta | y)$，决策 $d$ 的\textbf{期望后验损失}（posterior expected loss）为
\[
\rho(d | y) = \mathbb{E}_{p(\theta|y)}[\ell(\theta, d)] = \int \ell(\theta, d) \, p(\theta | y) \, d\theta.
\]

贝叶斯最优决策 $d^{\text{Bayes}}$ 是使期望后验损失最小化的决策：
\[
d^{\text{Bayes}} = \underset{d \in \mathcal{D}}{\arg\min} \; \rho(d | y).
\]

\textbf{关键洞察}：后验分布综合了先验信息与数据信息，因此"最小化期望后验损失"这一原则自然地将推断与决策统一起来——推断的结论（后验分布）直接决定了最优行动。

值得注意的是，$\rho(d | y)$ 是随机变量 $\theta$ 的函数，但由于我们对 $\theta$ 求了期望，最终得到的是一个确定性的数值（给定 $y$ 和 $d$），因此可以直接比较不同 $d$ 的优劣。
...
代入期望后验损失：
\[
\rho(d | y) = \mathbb{E}_{p(\theta|y)}[(\theta - d)^2]
           = \mathbb{E}(\theta^2 | y) - 2d \, \mathbb{E}(\theta | y) + d^2.
\]

对 $d$ 求导并令其为零，得到最优解
\[
d^{\text{Bayes}} = \mathbb{E}(\theta | y).
\]

\begin{Theorem}[平方损失下的贝叶斯最优估计]\label{th:squared-loss-bayes}
  在平方损失下，贝叶斯最优点估计是后验均值 $\mathbb{E}(\theta | y)$。
\end{Theorem}
...
期望后验损失 $\rho(d | y) = \mathbb{E}_{p(\theta|y)}[|\theta - d|]$ 是 $d$ 的凸函数。其最小化问题
\[
\underset{d}{\min} \; \mathbb{E}[|\theta - d|]
\]
...
期望后验损失为
\[
\rho(k | y) = \mathbb{E}_{p(\theta|y)}[\ell(\theta, k)] = 1 - p(\theta = k | y).
\]

最小化期望后验损失等价于最大化后验概率 $p(\theta = k | y)$，因此
\[
k^{\text{Bayes}} = \underset{k \in \mathcal{K}}{\arg\max} \; p(\theta = k | y),
\]
...
\[
\rho(d) = \mathbb{E}_{p(y', \theta)}[\ell(\theta, d(y'))] = \int \left[ \int \ell(\theta, d(y')) \, p(y' | \theta) \, dy' \right] p(\theta) \, d\theta.
\]

注意这里有两个期望层：内层关于 $y'$（给定 $\theta$），外层关于 $\theta$（先验分布）。通过交换积分次序，可以将预验预期损失写成
\[
\rho(d) = \mathbb{E}_{p(\theta)}[\rho(d | \theta)],
\]
其中 $\rho(d | \theta) = \mathbb{E}_{p(y'|\theta)}[\ell(\theta, d(y'))]$ 是在参数固定为 $\theta$ 时，决策 $d$ 的期望损失。

预验分析的核心问题是：如何选择实验设计 $y'$ 的采样方案（样本量、测量变量等），使得预验预期损失最小？

\begin{Example}[样本量确定]\label{ex:sample-size}
  设我们在一项临床试验中需要确定样本量 $n$。试验结束后，我们将使用后验均值 $\mathbb{E}(\theta | y)$ 作为治疗效果的点估计，采用平方损失。预验分析需要计算不同 $n$ 值下的预验预期损失
  \[
  \rho(n) = \mathbb{E}_{p(\theta)}[\text{var}(\theta | y')],
  \]
...
\[
\theta_i \sim p(\theta | \phi), \quad y_i \mid \theta_i \sim p(y_i \mid \theta_i),
\]
其中 $\phi$ 是超参数。

\subsection{层次决策的两阶段特性}

给定超参数 $\phi$ 的后验分布 $p(\phi | y)$，层次决策问题可以分解为两个层次：

\textbf{第一层（条件最优决策）}：给定 $\phi$ 和数据 $y$，对每个 $i$ 选择最优 $\theta_i$ 的估计 $\hat{\theta}_i(\phi, y)$。

\textbf{第二层（全贝叶斯决策）}：在第一层的基础上，对超参数 $\phi$ 的选择或估计进行决策。

经验贝叶斯（empirical Bayes）方法采取一种务实的策略：用数据估计 $\phi$（例如使用边缘似然函数 $p(y | \phi)$ 的最大似然估计 $\hat{\phi}$），然后在估计得到的 $\phi = \hat{\phi}$ 条件下进行第一层决策。
...
决策理论还为模型选择提供了统一框架。设我们有两个候选模型 $M_1$ 和 $M_2$，对应的后验概率分别为 $p(M_1 | y)$ 和 $p(M_2 | y)$。

在 0-1 损失（选择正确模型损失为 0，选择错误模型损失为 1）下，最优模型选择规则是选择后验概率最大的模型：
\[
\hat{M} = \underset{M \in \{M_1, M_2\}}{\arg\max} \; p(M | y).
\]

这一规则与贝叶斯因子（Bayes factor）密切相关，因为
\[
\frac{p(M_1 | y)}{p(M_2 | y)} = \frac{p(y | M_1)}{p(y | M_2)} \cdot \frac{p(M_1)}{p(M_2)} = B_{12} \cdot \text{先验比}.
\]
...
\begin{enumerate}
  ...
  \item \textbf{贝叶斯最优决策原则}：在给定后验分布 $p(\theta | y)$ 的条件下，最优决策是使期望后验损失最小化的决策。
...
\begin{Exercise}{\ref{exr:ch9-1} 平方损失的性质}
证明在平方损失下，后验均值 $\mathbb{E}(\theta | y)$ 是唯一最优估计量。
\end{Exercise}

\begin{Exercise}{\ref{exr:ch9-2} 绝对损失与后验中位数}
设后验分布 $p(\theta | y)$ 均匀分布于区间 $[a, b]$。证明在绝对损失下，任何 $d \in [a, b]$ 都是最优决策，并解释这一结论的直觉。
\end{Exercise}

\begin{Exercise}{\ref{exr:ch9-3} 0-1 损失与后验概率}
...
\begin{Exercise}{\ref{exr:ch9-4} 预验分析}
...
\begin{Exercise}{\ref{exr:ch9-5} 经验贝叶斯与全贝叶斯}
...
\textbf{背景（Background）}：在 \S 9.2 中，我们声称平方损失 $(\theta - d)^2$ 下的贝叶斯最优估计是后验均值 $\mathbb{E}(\theta | y)$。本节给出这一结论的完整推导。

\textbf{参数定义（Parameter Definitions）}：
\begin{itemize}
  \item $\theta$：未知标量参数；
  \item $d$：点估计量（决策变量）；
  \item $p(\theta | y)$：给定数据 $y$ 后的后验分布密度；
  \item $\rho(d | y) = \mathbb{E}_{p(\theta|y)}[(\theta - d)^2]$：决策 $d$ 的期望后验损失。
\end{itemize}

\textbf{目标（Goal）}：证明 $\underset{d}{\arg\min} \; \rho(d | y) = \mathbb{E}(\theta | y)$。

\textbf{推导步骤（Derivation Steps）}：

对 $\rho(d | y)$ 关于 $d$ 求导：
\[
\frac{\partial}{\partial d} \rho(d | y)
= \frac{\partial}{\partial d} \int (\theta - d)^2 \, p(\theta | y) \, d\theta
= \int \frac{\partial}{\partial d} (\theta - d)^2 \, p(\theta | y) \, d\theta
= \int -2(\theta - d) \, p(\theta | y) \, d\theta.
\]
...
令导数为零：
\[
\int -2(\theta - d) \, p(\theta | y) \, d\theta = 0
\;\Longleftrightarrow\;
\int \theta \, p(\theta | y) \, d\theta = d \int p(\theta | y) \, d\theta.
\]

由于 $p(\theta | y)$ 是密度函数，$\int p(\theta | y) \, d\theta = 1$，因此
\[
d = \int \theta \, p(\theta | y) \, d\theta = \mathbb{E}(\theta | y).
\]

二阶条件：$\frac{\partial^2}{\partial d^2} \rho(d | y) = 2 > 0$，确认是最小值点。

\textbf{结论}：平方损失下的贝叶斯最优估计量是后验均值 $\mathbb{E}(\theta | y)$。\qed

\subsection{样本量确定的预验分析推导}\label{sec:derivation-sample-size}

\textbf{背景（Background）}：在 \cref{ex:sample-size} 中，我们声称在平方损失下，预验预期损失 $\rho(n) = \mathbb{E}_{p(\theta)}[\text{var}(\theta | y')]$。本节证明这一结论，并推导具体形式。

\textbf{参数定义（Parameter Definitions）}：
\begin{itemize}
  \item $\theta$：手术成功率（未知参数）；
  \item $y'$：临床试验的 $n$ 个独立观测，$y'_i \sim \text{Binomial}(1, \theta)$；
  \item 决策 $d(y') = \mathbb{E}(\theta | y')$（平方损失下的贝叶斯最优决策）；
  \item 损失函数：$\ell(\theta, d) = (\theta - d)^2$。
\end{itemize}

\textbf{目标（Goal）}：计算 $\rho(n) = \mathbb{E}_{p(y', \theta)}[(\theta - \mathbb{E}(\theta | y'))^2]$。

\textbf{推导步骤（Derivation Steps）}：

第一步：对给定 $\theta$ 的条件期望损失 $\rho(n | \theta) = \mathbb{E}_{p(y'|\theta)}[(\theta - \mathbb{E}(\theta | y'))^2]$。

由贝叶斯公式，后验均值 $\mathbb{E}(\theta | y') = \frac{a + \sum y'_i}{a + b + n}$，其中 $a, b$ 是先验 Beta 参数。
...
因此：
\[
\mathbb{E}(\theta | y') = \frac{a + \sum y'_i}{a + b + n}
