...
在深入具体场景之前，我们需要一个统一的框架来描述各种数据收集问题。
...
完全数据似然为：
\begin{equation}
p(y, I|\theta, \phi) = p(y|\theta) \cdot p(I|y, \phi). \tag{8.1}
\end{equation}

实际可用的信息是 $(y_{\mathrm{obs}}, I)$，因此贝叶斯推断的适当似然（\textbf{观测数据似然}）为：
\begin{equation}
p(y_{\mathrm{obs}}, I|\theta, \phi) = \int p(y, I|\theta, \phi) \, dy_{\mathrm{mis}}. \tag{8.2}
\end{equation}

给定 $(x, y_{\mathrm{obs}}, I)$ 的联合后验分布为：
\begin{equation}
p(\theta, \phi|x, y_{\mathrm{obs}}, I) \propto p(\theta, \phi|x) \int p(y|x, \theta) p(I|x, y, \phi) \, dy_{\mathrm{mis}}. \tag{8.3}
\end{equation}
...
\begin{equation}
p(\theta|x, y_{\mathrm{obs}}) \propto p(\theta|x) \int p(y|x, \theta) \, dy_{\mathrm{mis}}. \tag{8.4}
\end{equation}
...
\textbf{有限总体均值的贝叶斯推断}：将 $\bar{y}$ 分解为已观测和未观测部分：
\begin{equation}
...
\begin{equation}
\bar{y} | y_{\mathrm{obs}} \approx \mathrm{N}\left(\bar{y}_{\mathrm{obs}}, \left(\frac{1}{n} - \frac{1}{N}\right)s_{\mathrm{obs}}^2\right). \tag{8.6}
\end{equation}
...
\begin{equation}
(\bar{y}^A - \bar{y}^B)|y_{\mathrm{obs}} \approx \mathrm{N}\left(\bar{y}_{\mathrm{obs}}^A - \bar{y}_{\mathrm{obs}}^B, \frac{2}{n}(s_{\mathrm{obs}}^{2A} + s_{\mathrm{obs}}^{2B})\right). \tag{8.7}
\end{equation}
...
\begin{Example}[有依从性问题的随机实验]
  ...
  依从者平均因果效应（CACE）的估计：
  \begin{equation}
    \widehat{\mathrm{CACE}} = (\bar{y}_1 - \bar{y}_0) / \hat{p}_c, \tag{8.8}
  \end{equation}
  其中 $p_c$ 为依从者比例。这被称为\textbf{工具变量估计}。
...
\begin{equation}
p(\theta|y_{\mathrm{obs}}, I) \propto \mathrm{N}(\theta|\bar{y}_{\mathrm{obs}}, 1/91) \cdot [\Phi(\theta-200)]^9. \tag{8.9}
\end{equation}
