...
这里 $\widetilde{\sigma}(v)^T$ 表示 equivariant quantum Schubert class\indexfootnote{问：$\langle \sigma(u)^T, \widetilde{\sigma}(v)^T \rangle_T$ 中的 $\widetilde{\sigma}(v)^T$ 是什么？它与普通的 $\sigma(v)^T$ 有何区别？见附录 \cref{sec:qa-tilde-sigma-definition}。}
