...
在前一节中，我们看到了从经典到等变量子上同调的正性问题自然延伸。但等变量量子上同调（equivariant quantum cohomology）究竟是什么？本节给出其正式定义。\footnote{问：等变量子上同调与量子上同调有什么区别？见附录 \cref{sec:EquivariantvsQuantum}。}

\subsection{思想来源：Kim 的贡献}

在深入定义之前，有必要提及 Kim 的重要贡献 \cite{Kim2000}。正是在 Kim 的工作基础上，Mihalcea 才得以将 Graham 的 positivity 结果推广到量子情形。Kim 认识到，等变量子上同调（equivariant cohomology）与量子上同调（quantum cohomology）之间存在一个``公共的母体''——等变量量子上同调（equivariant quantum cohomology），它同时包含了这两者的结构 \cite[\S 1]{Kim2000}。
...
\textbf{等变量量子上同调}（equivariant quantum cohomology）$QH_T^*(X)$ 是一个分次 $\Lambda[q]$-代数\index footnote{问：什么是分次 $\Lambda[q]$-代数？$q$ 变量代表什么？见附录 \cref{sec:graded-Lambda-q-algebra}。}，其 $\Lambda[q]$-基底为 $\{\sigma(w) \mid w \in W^P\}$。乘法 $\circ$ 定义为：
\begin{equation}
...
\item \textbf{等变量量子上同调} $QH_T^*(X)$ 是等变量子上同调（equivariant cohomology）和量子上同调（quantum cohomology）的共同推广，同时包含了这两者的结构；
