...
在深入定义之前，有必要提及 Kim 的重要贡献 \cite{Kim2000}。正是在 Kim 的工作基础上，Mihalcea 才得以将 Graham 的 positivity 结果推广到量子情形。Kim 认识到，等变上同调（equivariant cohomology）与量子上同调（quantum cohomology）之间存在一个``公共的母体''——等变量子上同调中的量子 Schubert 演算（equivariant quantum cohomology），它同时包含了这两者的结构 \cite[\S 1]{Kim2000}。

这一认识的关键在于：我们可以同时考虑 torus 作用（产生 $x_i$ 多项式）和量子修正（产生 $q$ 变量）。

\subsection{设置}

\begin{Definition}[连通复半单李群与抛物子群]\label{def:G-P-definition}
$G$ 为\textbf{连通复半单李群}（connected complex semisimple Lie group），$P \subset G$ 为\textbf{抛物子群}（parabolic subgroup）。
\end{Definition}

\begin{Definition}[Borel 子群与极大环面]\label{def:B-T-definition}
$B \subset P$ 为\textbf{Borel 子群}（Borel subgroup），$T \subset B$ 为\textbf{极大环面}（maximal torus）。
\end{Definition}

\begin{Definition}[齐次空间]\label{def:homogeneous-space}
$X = G/P$ 为\textbf{齐次空间}（homogeneous space）。
\end{Definition}

\begin{Definition}[Weyl 群与抛物子群确定的子群]\label{def:Weyl-group}
$W$ 为 $G$ 的\textbf{Weyl 群}（Weyl group），$W_P \subset W$ 为由 $P$ 确定的子群。
\end{Definition}

\begin{Definition}[正单根系]\label{def:root-system}
$\Delta = \{\alpha_1, \ldots, \alpha_r\}$ 为\textbf{正单根系}（positive simple root system），$\Delta_P \subset \Delta$ 为由 $P$ 确定的子集。
\end{Definition}

\begin{Definition}[最短长度代表元]\label{def:shortest-length-representatives}
$W^P \subset W$ 为 $W/W_P$ 的\textbf{最短长度代表元集合}（shortest length representatives）。
\end{Definition}
...
\item \textbf{等变量子上同调中的量子 Schubert 演算} $QH_T^*(X)$ 是等变上同调（equivariant cohomology）和量子上同调（quantum cohomology）的共同推广，同时包含了这两者的结构；
