...
设 $\overline{\mathcal{M}} = \overline{\mathcal{M}}_{0,3}(X,d)$ 为稳定映射的模空间（stable map moduli space），$ev_i : \overline{\mathcal{M}} \to X$ 为评估映射（evaluation map）。\footnote{问：请详细解释 $\overline{\mathcal{M}}_{0,3}(X, d)$ 的定义。见附录 \cref{sec:qa-moduli-space-definition}。}\footnote{问：请详细解释稳定映射中``幽灵分支''（ghost component）的概念，以及稳定性条件的动机。见附录 \cref{sec:qa-ghost-component-stability}。}
...
$$c_{u,v}^{w,d} = \sum_i a_i (\pi_{X \times X})_*^T\left([X(u) \times X(v)]_T \cdot [F(V_i)]_T\right).\footnote{问：笔记中 Mihalcea 证明的符号（半直积 $B'$、投影公式中的 $F(V_i)$ 等）与原文是否一致？见附录 \cref{sec:qa-mihalcea-notation}。}$$
...
\subsection{证明的整体结构}
\label{sec:proof-structure-ch4}

\textbf{注}：本节的归约策略的完整证明详见第 9 节附录（\cref{sec:appendix-eqc-proof}），其中包含每一步的详细推导。\footnote{问：Section 5 中的归约策略每一步具体是如何推导的？见附录 \cref{sec:appendix-eqc-proof}。}
