...
\textbf{Lemma 6.4}（\cref{prop:ev3inverse}，\cite[Lemma 6.4]{Mihalcea}） & $ev_3^{-1}(Y(w))$ 的不可约分支
\textbf{Corollary 6.2}（\cref{cor:GrahamOnXX}，\cite[Corollary 6.2]{Mihalcea}） & $X \times X$ 上的 Graham Positivity
\textbf{Lemma 6.3}（\cref{prop:KleimanTransversality}，\cite[Lemma 6.3]{Mihalcea}） & Kleiman transversality：$F: Z \to X \times X$ 是 $G$-等变映射，则 $F^{-1}(X(u) \times Y(v))$ 是 reduced、locally irreducible，余维数为 $c(u) + l(v)$
\end{tabular}
\end{center}

\textbf{证明的关键步骤}：先用 \cref{prop:ev3inverse} 将 $ev_3^{-1}(Y(w))$ 分解为不可约分支 $V_i$，再用 \cref{prop:KleimanTransversality}（Kleiman transversality）处理每个 $V_i$ 通过 $F = (ev_1, ev_3)$ 在 $X \times X$ 上的像，最后由 \cref{cor:GrahamOnXX}（Graham Positivity）保证每个 $[F(V_i)]_T$ 都是非负组合。投影公式则将这些结果``推送''回 $\overline{\mathcal{M}}_{0,3}(X,d)$ 上的 EQLR 系数。
