...
\end{equation}
其中 $c_{u,v}^{w,d}$ 称为\textbf{等变量子 Littlewood-Richardson 系数}（equivariant quantum LR coefficient，简称 EQLR 系数）。\footnote{问：在 \cref{def:EquivariantQuantumCohomology} 中提到：$QH_T^*(X)$ 是一个分次 $\Lambda[q]$-代数。什么是分次 $\Lambda[q]$-代数？其中的 $q$ 变量代表什么？见附录 \cref{sec:graded-Lambda-q-algebra}。}
\end{Definition}
...
在前一节的定义之后，Kim 证明了等变量子上同调具有代数结构：
...
\textbf{与 EQLR 系数的关系}：由这些同构可以推出，当 $d = 0$ 时，$c_{u,v}^{w,d}$ 退化为等变 LR 系数 $c_{u,v}^w$；当 $c_{u,v}^{w,d}$ 的多项式次数为 0 时（即 $c(u) + c(v) = c(w) + \sum d_i \cdot \deg q_i$），它等于普通量子 LR 系数。
...
\textbf{论文的核心——§6 证明结构}（\cite[§6, 第9-12页]{Mihalcea}）：Mihalcea 的论文 \S 6（第9-12页）包含了 positivity 定理的完整证明。\footnote{问：Mihalcea 论文 §6 的页码是否正确？它在第几页到第几页？实际阅读后确认。详见附录 \cref{sec:Mihalcea-page-check}。}整个证明的灵魂是\textbf{归约思想}：将 EQLR 系数计算归约到已知的 Graham positivity。
...
\textbf{Lemma 6.4} & $ev_3^{-1}(Y(w))$ 的不可约分支：$ev_3^{-1}(Y(v))$ 是 $T$-stable、reduced、不可约分支的不交并，每个分支的余维数为 $l(v)$ \\ \hline
\textbf{Corollary 6.2} & $X \times X$ 上的 Graham Positivity：$T$-stable 子簇的等变上同调类可以唯一写成 $\sum f_j [Y(u_j) \times Y(v_j)]_T$，其中 $f_j$ 是非负多项式 \\ \hline
\textbf{Lemma 6.3} & Kleiman transversality：$F: Z \to X \times X$ 是 $G$-等变映射，则 $F^{-1}(X(u) \times Y(v))$ 是 reduced、locally irreducible，余维数为 $c(u) + l(v)$
\end{tabular}
\end{center}

\textbf{证明的关键步骤}：先用 Lemma 6.4 将 $ev_3^{-1}(Y(w))$ 分解为不可约分支 $V_i$，再用 Lemma 6.3（Kleiman transversality）处理每个 $V_i$ 通过 $F = (ev_1, ev_3)$ 在 $X \times X$ 上的像，最后由 Corollary 6.2（Graham Positivity）保证每个 $[F(V_i)]_T$ 都是非负组合。投影公式则将这些结果``推送''回 $\overline{\mathcal{M}}_{0,3}(X,d)$ 上的 EQLR 系数。

\textbf{Lemma 6.3 的正式陈述}（\cite[Lemma 7.2]{FW}）：设 $Z$ 是不可约 $G$-簇，$F: Z \to X \times X$ 是 $G$-等变映射（$G$ 在 $X \times X$ 上对角作用）。则对任意 $u, v \in W^P$，$F^{-1}(X(u) \times Y(v))$ 是 reduced、locally irreducible 的，其余维数为 $c(u) + l(v)$。
...
在证明之前，让我们理解这个策略的直觉。EQLR 系数涉及稳定映射模空间 $\overline{\mathcal{M}}_{0,3}(X,d)$——它参数化了满足特定条件的稳定映射 $f: \mathbb{P}^1 \to X$\footnote{问：在量子上同调的定义中，出现稳定映射 $f: \mathbb{P}^1 \to X$，这里的 $X$ 是什么？$\mathbb{P}^1$ 又是什么？见附录 \cref{sec:P1-to-X}。}。
