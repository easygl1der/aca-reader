...
\end{equation}\footnote{量子上同调乘积 $\star$ 的几何定义为稳定映射模空间 $\overline{\mathcal{M}}_{0,3}(X,d)$ 上的 Gromov--Witten 相交，而非 Schubert 簇在 $X$ 中的普通相交。详见附录 \cref{sec:qa-QuantumStarProductGeometry}。}
...
\begin{Definition}[\textbf{连通复半单李群}]
\label{def:ConnectedComplexSemisimpleLieGroup}
...
\begin{Definition}[\textbf{抛物子群}]
\label{def:ParabolicSubgroup}
...
\begin{Definition}[\textbf{Borel 子群与极大环面}]
\label{def:BorelAndMaximalTorus}
...
\begin{Definition}[\textbf{齐次空间}]
\label{def:HomogeneousSpace}
...
\begin{Definition}[\textbf{Weyl 群与 $W_P$ 子群}]
\label{def:WeylGroupAndWP}
...
\begin{Definition}[\textbf{正单根系与 $\Delta_P$}]
\label{def:PositiveRootSystemAndDeltaP}
...
\begin{Definition}[\textbf{最短长度代表元 $W^P$}]
\label{def:ShortestLengthRepresentatives}
...
\begin{Definition}[\textbf{等变量子上同调} {\cite[Definition 2.1]{Mihalcea}}]
...
\end{equation}
其中两个求和分别遍历多重次数 $d$ 和 Schubert 类指标 $w \in W^P$。\footnote{问：$\sigma(u) \circ \sigma(v) = \sum_{d} \sum_{w \in W^P} q^d c_{u,v}^{w,d} \sigma(w)$ 这个公式中，这两个求和分别是对什么求和的？见附录 \cref{sec:qa-eqc-summations}。}
...
\textbf{元素的代数形式}：由定义，$QH_T^*(X)$ 作为 $\Lambda[q]$-模由 $\{\sigma(w) \mid w \in W^P\}$ 生成。因此，任意元素的代数形式为
\[
\alpha = \sum_{w \in W^P} f_w(\mathbf{x}, \mathbf{q}) \cdot \sigma(w), \qquad f_w(\mathbf{x}, \mathbf{q}) \in \Lambda[q] = \mathbb{Z}[x_1,\ldots,x_r][q_1,\ldots,q_m].
\]
其中 $f_w$ 是关于 torus 参数 $x_i$ 和量子参数 $q_j$ 的多项式，$m = \dim_{\mathbb{C}} X - \dim_{\mathbb{C}} G/P$。特别地，当所有 $q_j = 0$ 时，元素退化为等变上同调 $H_T^*(X)$ 中的类（只有 $\mathbf{x}$ 的多项式）；当所有 $x_i = 0$ 时，退化为普通量子上同调 $QH^*(X)$ 中的类（只有 $q$ 的多项式）。\footnote{问：$QH_T^*(X)$ 中元素的这种形式在几何上对应什么？见附录 \cref{sec:qa-eqc-element-geometry}。}

\textbf{关键性质}：$c_{u,v}^{w,d} \in \Lambda = \mathbb{Z}[x_1, \ldots, x_r]$ 是齐次多项式，次数为 $c(u) + c(v) - c(w) - \sum d_i \cdot \deg(q_i)$\footnote{问：次数公式 $c(u) + c(v) - c(w) - \sum d_i \cdot \deg(q_i)$ 中的求和指标 $i$ 的范围是什么？见附录 \cref{sec:qa-degree-summation-range}。}。\footnote{问：$c_{u,v}^{w,d} \in \mathbb{Z}[x_1, \ldots, x_r]$ 是非负系数多项式，但 ch4 的证明似乎没有提到根 $x_i$ 从何而来——这些根是怎么进来的？见附录 \cref{sec:cuwwd-ring-origin}。}
...
在前一节的定义之后，Kim 证明了等变量子上同调具有代数结构。\footnote{问：Kim 的等变量子上同调结构定理（Proposition 3.1）到底说了什么？证明在哪里？直观上如何理解两个同构？见附录 \cref{sec:qa-KimStructureTheorem}。}
...
\label{eq:P1-quantum-product}
\end{equation}
其中 $w = s_1$ 不可能出现，而只能是 $w = 1$。\footnote{问：为什么在量子乘积公式 $\sigma(s_1) \circ \sigma(s_1) = q \sigma(1)$ 中，$w = s_1$ 不可能出现，而只能是 $w = 1$？见附录 \cref{sec:qa-w-s1-degree-mismatch}。}
...
\textbf{正性验证}：这里 $x_2 - x_3$ 是非负系数多项式（系数为 1），体现了 Mihalcea positivity 的核心结论。这正是我们在 \cref{th:EQCPositivityCh4} 中要证明的性质——Equivariant Quantum Littlewood-Richardson 系数是关于负根 $x_i$ 的非负整数系数多项式。
...
本节展示 Mihalcea 如何将 Equivariant Quantum Littlewood-Richardson 系数的正性证明归约到已经解决的 Graham 情形。

下面我们将逐步完成这一归约链条。

\subsection{核心猜想（待证）}
\label{sec:CoreConjectureCh4}

Mihalcea 的核心结果是：等变量子 Littlewood-Richardson 系数 $c_{u,v}^{w,d}$ 是非负多项式。

\begin{Theorem}[等变量子上同调 Positivity {\cite[ Theorem 1]{Mihalcea}}]
\label{th:EQCPositivityCh4-Statement}
设 $u, v, w \in W^P$ 且 $d$ 为多重次数。则等变量子 Littlewood-Richardson 系数 $c_{u,v}^{w,d}$ 是关于负根 $x_1, \ldots, x_r$ 的非负整数系数多项式。
\end{Theorem}

这个定理在几何上对应：稳定映射模空间 $\overline{\mathcal{M}}_{0,3}(X,d)$ 中曲线计数的正性。

\subsection{等变量子上同调的定义}
\label{sec:EQCDefinitionCh4}

证明需要用到以下定义。

\begin{Definition}[\textbf{等变量子上同调} {\cite[Definition 2.1]{Mihalcea}}]
\label{def:EquivariantQuantumCohomologyCh4}
$QH_T^*(X)$ 是一个分次 $\Lambda[q]$-代数，$\Lambda = H_T^*(pt) = \mathbb{Z}[x_1,\ldots,x_r]$，$\Lambda[q]$-基底为 $\{\sigma(w) \mid w \in W^P\}$。乘法 $\star$ 定义为
\begin{equation}
\sigma(u) \star \sigma(v) = \sum_{d} \sum_{w \in W^P} q^d c_{u,v}^{w,d} \sigma(w),
\label{eq:EQCDefinitionCh4}
\end{equation}
其中 $c_{u,v}^{w,d} \in \Lambda$ 称为\textbf{等变量子 Littlewood-Richardson 系数}。
\end{Definition}

\textbf{几何意义}：等变量子上同调中的系数 $c_{u,v}^{w,d}$ 计数了 $\overline{\mathcal{M}}_{0,3}(X,d)$ 中满足特定 Schubert 约束的稳定映射的个数。$q^d$ 记录曲线的多重次数。

现在，请读者回忆 Mihalcea 的论文 \cite[Section 6]{Mihalcea} 中的三个关键引理——它们是整个证明链条的前置条件。

\subsection{第一步：几何归约（论文 \S 6 的三个关键引理）}
\label{sec:KeyLemmasCh4Proof}

\textbf{目标}：将系数 $c_{u,v}^{w,d}$ 从 $\overline{\mathcal{M}}_{0,3}(X,d)$ 上的曲线计数问题，归约到 $X \times X$ 上的 Graham positivity。

\textbf{Lemma 6.3（Kleiman Transversality）}{\cite[Lemma 7.2]{FW}}：
\label{prop:KleimanTransversalityCh4}
设 $Z$ 是不可约 $G$-簇，$F: Z \to X \times X$ 是 $G$-等变映射。则对任意 $u, v \in W^P$，$F^{-1}(X(u) \times Y(v))$ 是 reduced、locally irreducible 的，其余维数为 $c(u) + \ell(v)$。

\textbf{Lemma 6.4（$ev_3^{-1}(Y(w))$ 的分支）}{\cite[Lemma 6.4]{Mihalcea}}：
\label{prop:ev3inverseCh4}
$ev_3^{-1}(Y(w))$ 的每个不可约分支 $V_i$ 都是 $B'$-稳定子簇，其中 $B' = T \cdot (U^- \times U^-)$。

\textbf{Corollary 6.2（$X \times X$ 上的 Graham Positivity）}{\cite[Corollary 6.2]{Mihalcea}}：
\label{cor:GrahamOnXXCh4}
...
\subsection{读者练习：完成归约链条}
\label{sec:ProofChallengeCh4}

有了上述三个引理，请读者完成以下归约链条，将 \cref{th:EQCPositivityCh4-Statement} 的证明补全。

\textbf{给定}：等变量子 Littlewood-Richardson 系数定义为
$$c_{u,v}^{w,d} = \pi_*^T\left((ev_1^T)^* \sigma(u)^T \cdot (ev_2^T)^* \sigma(v)^T \cdot (ev_3^T)^* \widetilde{\sigma}(w)^T\right).$$

\textbf{第一步}：对映射 $F = (ev_1, ev_2): \overline{\mathcal{M}}_{0,3}(X,d) \to X \times X$，使用投影公式，将上式改写为
$$c_{u,v}^{w,d} = \sum_i a_i (\pi_{X \times X})_*^T\left([X(u) \times X(v)]_T \cdot [F(V_i)]_T\right),$$
其中 $V_i$ 是 $ev_3^{-1}(Y(w))$ 的不可约分支，$a_i \in \mathbb{N}$ 是映射次数。

\textbf{第二步（请补全）}：对每个 $F(V_i)$，由 \cref{cor:GrahamOnXXCh4} 写出 $[F(V_i)]_T$ 的表达式。

\textbf{第三步（请补全）}：代入第二步的结果，再次使用投影公式（或直接计算），将整个表达式化为只含非负整数和 Kronecker $\delta$ 的形式。结论：$c_{u,v}^{w,d} \in \mathbb{N}[x_1,\ldots,x_r]$。

\textbf{关键洞察}：量子上同调中的曲线计数问题可以通过投影公式归约为 $X \times X$ 上的经典 Graham positivity 问题。这与 \cref{chap:TriplePositivity} 中 Gao--Xiong \cite{GX2025} 证明三重 Schubert positivity 的思路有本质联系——两者都使用了投影公式作为核心工具。

\textbf{完整证明}见附录 \cref{sec:appendix-eqc-proof}。

\section{与三重 Schubert Positivity 的联系}

\label{sec:ConnectionToTripleCh4}

在 \cref{chap:TriplePositivity} 中，我们遇到了 Refined Graham Positivity——Gao--Xiong \cite{GX2025} 证明了三重变量 $(\mathbf{x}; \mathbf{y}; \mathbf{t})$ 下的展开系数 $c^w_{u,v}(\mathbf{y}, \mathbf{t})$ 是关于 $t_i - y_j$ 的非负多项式（\cref{def:RefinedGraham}）。Mihalcea 的结果与此有着深刻的联系。

\textbf{三重 Schubert Positivity}（triple Schubert positivity）涉及的是点的相交（point intersection），而等变量子上同调涉及的是曲线的计数（curve counting）\footnote{问：三重 Schubert positivity 涉及点的相交，而等变量子上同调涉及曲线的计数。这里"曲线的计数"是一个什么概念？见附录 \cref{sec:qa-curve-counting-vs-point-intersection}。}。
...
\textbf{注意}：$T$ 是 $PGL(m)$ 的极大环面，而 $T'$ 是更大的 torus。两者的关系是 $T = T'/\mathbb{C}^\times$\footnote{问：为什么 $PGL(m)$ 的极大环面是 $(\mathbb{C}^{\times})^{m-1}$？这与 $T = T'/\mathbb{C}^\times$ 有什么关系？见附录 \cref{sec:qa-PGL-maximal-torus}。}。
...
我们已经看到了 Theorem \cref{th:EQCPositivityCh4} 的代数陈述。但从几何角度理解，这个定理告诉我们什么？
...
\item $f(p_1) \in g_1 X(u)$，$f(p_2) \in g_2 X(v)$，$f(p_3) \in g_3 Y(w)$（其中 $g_i \in G$ 是一般位置，general position）\footnote{问：在稳定映射的 Gromov-Witten 不变量定义中，"一般位置"（general position）是什么意思？为什么需要这个条件？见附录 \cref{sec:qa-general-position}。}。
\end{itemize}
其中 $p_1, p_2, p_3$ 是 $\mathbb{P}^1$ 上的三个标记点（marked points）。

从纯粹的几何角度，这些计数自然是非负的——计数``有多少条曲线通过指定位置''这个问题，答案显然不能是负数。

而 Theorem \cref{th:EQCPositivityCh4} 进一步指出：\textbf{即使加入了等变修正（这会产生关于 $x_i$ 的多项式因子），整体系数仍然保持非负}。
...
\item \textbf{主定理}（Theorem \cref{th:EQCPositivityCh4}）：Equivariant Quantum Littlewood-Richardson 系数 $c_{u,v}^{w,d}$ 是关于负根 $x_i$ 的非负整数系数多项式；
...
这一结果与 Chapter 1 的 Gao--Xiong \cite{GX2025} 三重 positivity 工作以及 Chapter 3 的量子 Schubert 多项式理论共同构成了现代 Schubert calculus 的核心框架。
...
本节给出 Theorem \cref{th:EQCPositivityCh4} 的详细证明思路。该定理由 Mihalcea 在 2007 年证明，是量子 Schubert calculus 的里程碑结果。

证明的核心思想是将 Equivariant Quantum Littlewood-Richardson 系数的计算归约到 Graham 的经典 positivity 定理（Theorem \cref{th:GrahamPositivityRefined}）。
...
\textbf{关于 $[V]_T$ 记号的说明}：本文中所有出现的 $[V]_T$ 均指 Borel 构造下的等变基本类（equivariant fundamental class），属于 $H_T^*(X)$ 或 $H_T^*(X \times X)$。它与量子上同调中的 Schubert 基底 $\sigma(w)$ 有本质区别：
\begin{itemize}
\item $[V]_T$：任意 $T$-稳定子簇 $V$ 的等变类（几何类）
\item $\sigma(w)^T$：Schubert 簇 $X(w)$ 的等变类，即 $\sigma(w)^T = [X(w)]_T$（几何类的特殊情形）
\item $\sigma(w)$：量子上同调环 $QH_T^*(X)$ 的代数基底，不再是子簇的基本类
\end{itemize}
在证明中出现的 $[F(V_i)]_T$ 是中间计算工具——它表示 $F(V_i) \subset X \times X$ 的几何类，展开为 $[Y(u_j) \times Y(v_j)]_T$ 的线性组合，最后通过 Poincaré 配对与 $\sigma(w)$ 建立联系。Ch1 中的 $[-]_T$ 与本节定义完全一致（均为等变基本类），区别仅在于目标空间（Ch1 在 $H_T^*(G/B)$，本节在 $H_T^*(X \times X)$）。
...
其中 $\langle \cdot, \cdot \rangle_T$ 是 $H_T^*(X)$ 上的等变 Poincaré pairing\footnote{问：等变 Poincaré pairing $\langle \cdot, \cdot \rangle_T$ 的具体定义是什么？它与普通 Poincaré pairing 有什么关系？为什么 Schubert 类的配对是 Kronecker delta？见附录 \cref{sec:qa-equivariant-poincare-pairing}。}
...
这里 $\widetilde{\sigma}(v)^T$ 表示 equivariant quantum Schubert class（等变 Poincaré 对偶意义下的对偶类）\footnote{问：$\langle \sigma(u)^T, \widetilde{\sigma}(v)^T \rangle_T$ 中的 $\widetilde{\sigma}(v)^T$ 是什么？它与普通的 $\sigma(v)^T$ 有何区别？见附录 \cref{sec:qa-tilde-sigma-definition}。}
...
其中 $a_i$ 是映射 $F|_{V_i} : V_i \to F(V_i)$ 的次数（degree）\footnote{问：映射的次数（degree）是如何定义的？见附录 \cref{sec:qa-morphism-degree}。}。
...
对每个 $[F(V_i)]_T$，由 Graham 定理（\cref{cor:GrahamOnXX}）
...
\end{align*}
 \footnote{问：为什么 $[X(u) \times X(v)]_T = (pr_1^T)^* \sigma(u)^T \cdot (pr_2^T)^* \sigma(v)^T$？这个等式是怎么来的？见附录 \cref{sec:qa-product-class-pullback}。}
...
这完成了 Theorem \cref{th:EQCPositivityCh4} 的证明。
