...
\begin{enumerate}
\item \textbf{经典上同调}（Schubert Positivity）：结构常数 $c_{u,v}^w \in \mathbb{N}$ 是非负整数。
\item \textbf{等变量子上同调}（Graham Positivity {\cite[ Theorem 3.2]{Gr}}）：结构常数 $c_{u,v}^w \in \mathbb{Z}[x_1,\ldots,x_r]$ 是非负整数系数的多项式。
\item \textbf{等变量子上同调中的量子 Schubert 演算}（Mihalcea Positivity {\cite[ Theorem 1]{Mihalcea}}）：结构常数 $c_{u,v}^{w,d} \in \mathbb{Z}[x_1,\ldots,x_r]$ 是非负整数系数的多项式。
\end{enumerate}
