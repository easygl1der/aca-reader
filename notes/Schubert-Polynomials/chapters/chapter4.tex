...
\textbf{论文的核心——§6 证明结构}（\cite[§6, 第9-12页]{Mihalcea}）：Mihalcea 的论文 \S 6（第9-12页）包含了 positivity 定理的完整证明。\footnote{问：Mihalcea 论文 §6 的页码是否正确？它在第几页到第几页？实际阅读后确认。}整个证明的灵魂是\textbf{归约思想}：将 EQLR 系数计算归约到已知的 Graham positivity。
...
\textbf{Lemma 6.4}\cite[Lemma 6.4]{Mihalcea} & $ev_3^{-1}(Y(w))$ 的不可约分支：$ev_3^{-1}(Y(v))$ 是 $T$-stable、reduced、不可约分支的不交并，每个分支的余维数为 $l(v)$ \\ \hline
\textbf{Corollary 6.2}\cite[Corollary 6.2]{Mihalcea} & $X \times X$ 上的 Graham Positivity：$T$-stable 子簇的等变上同调类可以唯一写成 $\sum f_j [Y(u_j) \times Y(v_j)]_T$，其中 $f_j$ 是非负多项式 \\ \hline
\textbf{Lemma 6.3}\cite[Lemma 6.3]{Mihalcea} & Kleiman transversality：$F: Z \to X \times X$ 是 $G$-等变映射，则 $F^{-1}(X(u) \times Y(v))$ 是 reduced、locally irreducible，余维数为 $c(u) + l(v)$
\end{tabular}
\end{center}

\textbf{证明的关键步骤}：先用 \cite[Lemma 6.4]{Mihalcea} 将 $ev_3^{-1}(Y(w))$ 分解为不可约分支 $V_i$，再用 \cite[Lemma 6.3]{Mihalcea}（Kleiman transversality）处理每个 $V_i$ 通过 $F = (ev_1, ev_3)$ 在 $X \times X$ 上的像，最后由 \cite[Corollary 6.2]{Mihalcea}（Graham Positivity）保证每个 $[F(V_i)]_T$ 都是非负组合。投影公式则将这些结果``推送''回 $\overline{\mathcal{M}}_{0,3}(X,d)$ 上的 EQLR 系数。
