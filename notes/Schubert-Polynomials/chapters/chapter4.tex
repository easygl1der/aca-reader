...
&= (\pi_{X \times X})_*^T\big((pr_1^T)^* \sigma(u)^T \cdot (pr_2^T)^* \sigma(v)^T \footnote{问：为什么 $[X(u) \times X(v)]_T = (pr_1^T)^* \sigma(u)^T \cdot (pr_2^T)^* \sigma(v)^T$？这个等式是怎么来的？见附录 \cref{sec:qa-product-class-pullback}。} \\
