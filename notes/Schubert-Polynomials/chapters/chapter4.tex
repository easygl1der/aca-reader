...
设 $\overline{\mathcal{M}} = \overline{\mathcal{M}}_{0,3}(X,d)$ 为稳定映射的模空间（stable map moduli space），$ev_i : \overline{\mathcal{M}} \to X$ 为评估映射（evaluation map）。\footnote{问：请详细解释 $\overline{\mathcal{M}}_{0,3}(X, d)$ 的定义。见附录 \cref{sec:qa-moduli-space-definition}。}
