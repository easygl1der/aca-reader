...
在前三章中，我们逐步建立了 Schubert calculus 的现代框架：\cref{chap:TriplePositivity} 在三重变量集下建立了 Graham positivity（\cref{def:GrahamPositivity}），\cref{chap:MolevSaganFormula} 给出了 Molev-Sagan 类型公式，\cref{chap:QuantumDouble} 引入了量子双重 Schubert 多项式。每一步推广都自然地引出了下一个问题。
...
在 Chapter 1 中，我们遇到了三重 Schubert Positivity——Gao-Xiong 证明了三重变量 $(\mathbf{x}; \mathbf{y}; \mathbf{t})$ 下的展开系数 $c^w_{u,v}(\mathbf{y}, \mathbf{t})$ 是关于 $t_i - y_j$ 的非负多项式。Mihalcea 的结果与此有着深刻的联系。
...
取特殊情形 $d = 0$（无量子修正），$c_{u,v}^{w,0}$ 正好是经典等变量子上同调的系数——即 Graham 的结果。此时，EQLR 系数的正性退化为 Graham positivity。
...
在 Schubert calculus 的研究中，有多种不同层次的正性。它们之间存在包含关系：
