...
双重 Graham（二重 S$_w$） & 等变 $H_T^*(G/P)$（双变量） & $c_{u,v}^w(\mathbf{y},\mathbf{t}) \in \mathbb{Z}[t_i - y_j]$ & $\mathbf{y}, \mathbf{t}$：两组 torus 权 \\ \hline
\end{tabular}
\end{center}
...
\item 双重 Graham（\cref{def:GrahamPositivity}）是两变量情形 $c_{u,v}^w(\mathbf{y},\mathbf{t})$：$\mathbf{y}$ 记录左作用位移，$\mathbf{t}$ 记录右作用位移，$\mathfrak{S}_u(\mathbf{x};\mathbf{y}) \cdot \mathfrak{S}_v(\mathbf{x};\mathbf{t}) = \sum_w c_{u,v}^w(\mathbf{y},\mathbf{t}) \mathfrak{S}_w(\mathbf{x};\mathbf{t})$
\end{itemize}
