...
这里 $d$ 是曲线类的多重次数（multi-degree），$q^d$ 是形式变量\index footnote{问：$q^d$ 是形式变量是什么意思？见附录 \cref{sec:q-formal-variable}。}。关键问题出现了：系数 $c_{u,v}^{w,d}$ 作为 $x_i$ 的多项式，是否仍然满足正性？
...
在证明之前，让我们理解这个策略的直觉。EQLR 系数涉及稳定映射模空间 $\overline{\mathcal{M}}_{0,3}(X,d)$——它参数化了满足特定条件的稳定映射 $f: \mathbb{P}^1 \to X$\footnote{问：$f: \mathbb{P}^1 \to X$ 中的 $X$ 是什么？见附录 \cref{sec:P1-to-X}。}。
