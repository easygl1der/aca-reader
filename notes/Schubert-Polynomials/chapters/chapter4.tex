...
为了理解这个定理的意义，让我们陈述三种层次的正性定理：

\begin{enumerate}
\item \textbf{Schubert Positivity}（经典情形）：设 $u, v, w \in W^P$，则结构常数 $c_{u,v}^w \in \mathbb{N}$ 是非负整数。
\item \textbf{Graham Positivity}（等变量子上同调 {\cite[ Theorem 3.2]{Gr}}）：设 $u, v, w \in W^P$，则结构常数 $c_{u,v}^w \in \mathbb{Z}[x_1,\ldots,x_r]$ 是非负整数系数的多项式。
\item \textbf{Mihalcea Positivity}（等变量子上同调中的量子 Schubert 演算 {\cite[ Theorem 1]{Mihalcea}}）：设 $u, v, w \in W^P$ 且 $d$ 为多重次数，则结构常数 $c_{u,v}^{w,d} \in \mathbb{Z}[x_1,\ldots,x_r]$ 是非负整数系数的多项式。
\end{enumerate}
