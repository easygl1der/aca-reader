...
这个问题并非无的放矢。我们已经知道，在经典等变量子上同调（equivariant cohomology）中，Graham 证明了 Peterson 的猜想（\cref{def:GrahamPositivity}）——展开系数是关于负根 $x_i$ 的非负多项式。但量子上同调的情况有所不同：乘法不再是单纯的相交计数，而涉及 Gromov-Witten 不变量（Gromov-Witten invariant）\footnote{问：什么是 Gromov-Witten 不变量？见附录 \cref{sec:GW-invariant}。}——计数的是满足特定条件的曲线个数。在更复杂的代数几何世界中，同类正性结果是否仍然成立？

\textbf{核心发现}（Mihalcea, 2007）：答案是肯定的。等变量子上同调中的量子 Schubert 演算（equivariant quantum cohomology）的结构常数 $c_{u,v}^{w,d}$ 仍然是关于负根 $x_1, \ldots, x_r$ 的非负整数系数多项式。
...
但当引入等变修正时，问题变得微妙了。等变量子上同调（equivariant cohomology）$H_T^*(G/P)$ 中的结构常数 $c_{u,v}^w$ 不再是简单的整数，而是关于 torus 作用参数的多项式 \cite{Borel1957,AndersonFulton}。\textbf{这些多项式的系数还能保持非负吗？}
...
设 $X = G/P$ 为齐次空间，$u, v, w \in W^P$。则在等变量子上同调（equivariant cohomology）中，\footnote{问：$\sigma(u)^T$ 是什么意思？见附录 \cref{sec:sigma-u-T}。}
$$\sigma(u)^T \cdot \sigma(v)^T = \sum_w c_{u,v}^w \sigma(w)^T$$
其中 $\sigma(u)^T$ 表示等变量子上同调（equivariant cohomology）中的 Schubert 类，$T$ 代表 torus 等变（equivariant）。
\end{Theorem}

\textbf{为什么叫``正性''？}\footnote{问：Graham Positivity 中展开系数的"非负性"具体是什么含义？见附录 \cref{sec:GrahamPositivityCoefficientMeaning}。} 因为每个 $c_{u,v}^w$ 都可以表示为形如 $x_1^{a_1} x_2^{a_2} \cdots$ 的单项式的非负线性组合，其中所有系数都是非负整数。

\subsection{量子化带来的挑战}

Graham 定理的证明利用了等变量子上同调的几何性质\footnote{问：为什么 Graham 的证明要依赖几何性质？具体用了什么几何性质？见附录 \cref{sec:GrahamProofGeometry}。}。但当我们将目光投向量子上同调时，同样的正性问题变得更加复杂。在量子上同调（quantum cohomology）中，乘法不再是单纯的相交计数，而涉及 Gromov-Witten 不变量（Gromov-Witten invariant）\cite{Kontsevich1994,Gromov1995}——计数的是满足特定条件的曲线个数：
...
\section{等变量子上同调中的量子 Schubert 演算的定义}
\label{sec:EQCDefinition}

在前一节中，我们看到了从经典到等变量子上同调的正性问题自然延伸。但等变量子上同调中的量子 Schubert 演算（equivariant quantum cohomology）究竟是什么？本节给出其正式定义。\footnote{问：等变量子上同调与量子上同调有什么区别？见附录 \cref{sec:EquivariantvsQuantum}。}

\subsection{思想来源：Kim 的贡献}

在深入定义之前，有必要提及 Kim 的重要贡献 \cite{Kim2000}。正是在 Kim 的工作基础上，Mihalcea 才得以将 Graham 的 positivity 结果推广到量子情形。Kim 认识到，等变量子上同调（equivariant cohomology）与量子上同调（quantum cohomology）之间存在一个``公共的母体''——等变量子上同调中的量子 Schubert 演算（equivariant quantum cohomology），它同时包含了这两者的结构 \cite[\S 1]{Kim2000}。
...
记 $\Lambda = H_T^*(pt) = \mathbb{Z}[x_1, \ldots, x_r]$\footnote{问：$H_T^*(pt)$ 是什么意思？等变上同调类如何定义？见附录 \cref{sec:EquivariantCohomologyClass}。}，其中 $x_i$ 对应负单根 $-\alpha_i$（negative simple roots）。

\subsection{等变量子上同调中的量子 Schubert 演算}

基于 Kim 的思想，Mihalcea 将等变量子上同调中的量子 Schubert 演算定义为分次 $\Lambda[q]$-代数。

\begin{Definition}[\textbf{等变量子上同调中的量子 Schubert 演算}]
\label{def:EquivariantQuantumCohomology}
\textbf{等变量子上同调中的量子 Schubert 演算}（equivariant quantum cohomology）$QH_T^*(X)$ 是一个分次 $\Lambda[q]$-代数，其 $\Lambda[q]$-基底为 $\{\sigma(w) \mid w \in W^P\}$。乘法 $\circ$ 定义为：
\begin{equation}
\sigma(u) \circ \sigma(v) = \sum_{d} \sum_{w \in W^P} q^d c_{u,v}^{w,d} \sigma(w),
\label{eq:EQCDefinition}
\end{equation}
其中 $c_{u,v}^{w,d}$ 称为\textbf{等变量子量子 Littlewood-Richardson 系数}（equivariant quantum LR coefficient，简称 EQLR 系数）。\footnote{问：什么是分次 $\Lambda[q]$-代数？$q$ 变量代表什么？见附录 \cref{sec:graded-Lambda-q-algebra}。}
\end{Definition}

\textbf{关键性质}：$c_{u,v}^{w,d} \in \Lambda = \mathbb{Z}[x_1, \ldots, x_r]$ 是齐次多项式，次数为 $c(u) + c(v) - c(w) - \sum d_i \cdot \deg(q_i)$。

\textbf{与经典情形的关系}：等变量子上同调中的量子 Schubert 演算是以下两者的共同推广：
\begin{itemize}
\item 当 $d = 0$ 时，退化为等变量子上同调（equivariant cohomology）$H_T^*(X)$（对应 Graham positivity）；
\item 当所有 $x_i = 0$ 时，退化为普通量子上同调（quantum cohomology）$QH^*(X)$。
\end{itemize}

这种``共同推广''的性质解释了为什么等变量子上同调中的量子 Schubert 演算是一个自然的研究对象——它同时推广了两个重要的上同调理论。

\subsection{例子}

\textbf{例 1：Grassmannian $Gr(1,2) = \mathbb{P}^1$ 的情形。}
...
等变量子上同调中的量子 Schubert 演算 $QH_T^*(\mathbb{P}^1)$ 的乘法为：
$$\sigma(s_1) \circ \sigma(s_1) = q \sigma(1).$$

\textbf{几何意义}：$\mathbb{P}^1$ 上两点决定的直线与自身的交点数为 1，但需要量子修正因子 $q$（计数亏格 0 曲线）。

\textbf{例 2：$Gr(2,4)$ 情形。}

设 $X = Gr(2,4)$（2 维平面构成的 Grassmannian）。此时 $W = S_4$ 的最短代表元有 6 个，对应 6 个 Schubert 类。

考虑 $u = s_1$，$v = s_3$。在等变量子上同调中的量子 Schubert 演算中：
...
这验证了 Definition \ref{def:EquivariantQuantumCohomology} 中的退化关系。
...
\begin{Theorem}[等变量子上同调中的量子 Schubert 演算 Positivity {\cite[Theorem 1]{Mihalcea}}]
...
\subsubsection{三种正性定理的正式陈述}

\begin{Theorem}[Schubert Positivity]
设 $X = G/P$ 为齐次空间，$u, v, w \in W^P$。则经典 Schubert calculus 中的结构常数
$$c_{u,v}^w \in \mathbb{N}$$
是非负整数。
\end{Theorem}

\begin{Theorem}[Graham Positivity {\cite[ Theorem 3.2]{Gr}}]
设 $X = G/P$ 为齐次空间，$u, v, w \in W^P$。则在等变量子上同调中，结构常数
$$c_{u,v}^w \in \mathbb{Z}[x_1,\ldots,x_r]$$
是非负整数系数的多项式。
\end{Theorem}

\begin{Theorem}[Mihalcea Positivity {\cite[ Theorem 1]{Mihalcea}}]
\label{th:MihalceaPositivity}
设 $X = G/P$ 为齐次空间，$u, v, w \in W^P$ 且 $d$ 为多重次数。则在等变量子上同调中的量子 Schubert 演算中，结构常数
$$c_{u,v}^{w,d} \in \mathbb{Z}[x_1,\ldots,x_r]$$
是非负整数系数的多项式。
\end{Theorem}
$$c_{u,v}^w \in \mathbb{N} \Longrightarrow c_{u,v}^w \in \mathbb{Z}[x_i] \Longrightarrow c_{u,v}^{w,d} \in \mathbb{Z}[x_i].$$
...
\textbf{论文的核心——§6 证明结构}：Mihalcea 的论文 \cite{Mihalcea} 的 \S 6（第 12-20 页）是整个工作的核心，包含了 positivity 定理的完整证明。这个证明建立了以下关键引理的逻辑链：

\begin{center}
\begin{tabular}{ll}
\textbf{Lemma 6.4} & $ev_3^{-1}(Y(w))$ 的不可约分支 \\
\textbf{Corollary 6.2} & $X \times X$ 上的 Graham Positivity \\
\textbf{Lemma 6.1-6.6} & 投影公式与推送公式的应用
\end{tabular}
\end{center}

整个证明的灵魂是 \textbf{归约思想}：通过投影公式，将 $\overline{\mathcal{M}}_{0,3}(X,d)$ 上的曲线计数问题归约到 $X \times X$ 上的已知正性。\footnote{等变量子上同调中的量子 Schubert 演算 positivity 定理的完整证明见附录 \cref{sec:appendix-eqc-proof}。}

\subsection{为什么可以归约？}

在证明之前，让我们理解这个策略的直觉。EQLR 系数涉及稳定映射模空间（stable map moduli space）$\overline{\mathcal{M}}_{0,3}(X,d)$，这是一个复杂的几何对象——它参数化了所有满足特定条件的稳定映射 $f: \mathbb{P}^1 \to X$\footnote{问：$f: \mathbb{P}^1 \to X$ 中的 $X$ 是什么？见附录 \cref{sec:P1-to-X}。}。

但 Mihalcea 的关键洞察是：\textbf{通过适当的映射和投影公式（projection formula），复杂的上同调表达式可以``推送''到积空间 $X \times X$ 上，而在 $X \times X$ 上 Graham positivity 已经告诉我们答案。}
...
设 $f: X_1 \to X_2$ 为 $T$-等变映射。对于 $T$-等变量子上同调类 $a \in H_T^*(X_2)$ 和 $b \in H_T^*(X_1)$，有：
\begin{equation}
f_*^T\left((f^T)^*(a) \cdot b\right) = a \cdot f_*^T(b).
\label{eq:ProjectionFormulaCh4}
\end{equation}

\textbf{直观理解}：这个公式告诉我们，当我们对 $a$ 和 $b$ 的乘积做推送时，如果 $a$ 本身来自目标空间 $X_2$，它可以``提取出来''到外面，只留下 $b$ 的推送。这正是我们能够将问题从 $\overline{\mathcal{M}}$ 转移到 $X \times X$ 的关键。
...
\end{itemize}
因此 $c_{u,v}^{w,d} \in \mathbb{Z}[x_1, \ldots, x_r]$ 是非负系数多项式，这正是 Theorem \ref{th:EQCPositivity} 所要证明的。
...
\node (EQLR) at (0,0) {$c_{u,v}^{w,d}$};
\node (M) at (0,-1.5) {$\overline{\mathcal{M}}_{0,3}(X,d)$};
\node (F) at (0,-3) {$F = (ev_1, ev_2)$};
\node (XX) at (0,-4.5) {$X \times X$};
\node (Graham) at (0,-6) {$[F(V_i)]_T = \sum_j f_j^i [Y(u_j) \times Y(v_j)]_T$};

\draw[->] (EQLR) -- (M) node[midway, right] {$\pi_*^T$};
\draw[->] (M) -- (F) node[midway, right] {$F_*^T$};
\draw[->] (F) -- (XX);
\draw[->] (XX) -- (Graham) node[midway, right] {Graham positivity};
\end{tikzpicture}
\end{center}
...
\textbf{三重 Schubert Positivity}（triple Schubert positivity）涉及的是点的相交（point intersection），而等变量子上同调中的量子 Schubert 演算涉及的是曲线的计数（curve counting）。

\subsection{为什么这两个正性定理是相关的？}

粗略一看，三重 Schubert calculus 和等变量子上同调中的量子 Schubert 演算似乎是不同的领域：前者涉及多组变量，后者涉及量子修正。但它们都关注 Schubert 结构系数的正性，而且都涉及旗流形的几何。

当我们仔细审视时会发现：\textbf{两者都反映了旗流形中 Schubert 簇相交理论的基本非负性}。在三重情形，这种非负性来自点的相交；在量子情形，它来自曲线的计数。数学工具不同，但几何本质相同。

\subsection{旗流形情形的联系}

当 $X = Fl_n = GL_n/B$ 时，等变量子上同调中的量子 Schubert 演算的 EQLR 系数 $c_{u,v}^{w,d}$ 与三重 Schubert calculus 中的正性有如下关系：

取特殊情形 $d = 0$（无量子修正），$c_{u,v}^{w,0}$ 正好是经典等变量子上同调（equivariant cohomology）的系数——即 Graham 的结果（\cref{def:GrahamPositivity}）。此时，EQLR 系数的正性退化为 Graham positivity。

当考虑量子修正 $d \neq 0$ 时，新的正性结果补充了 \cref{chap:TriplePositivity} 的框架。几何上，这是因为量子上同调涉及曲线的计数，而三重情形只涉及点。\textbf{关键区别}在于：三重 Schubert positivity 涉及的是点的相交（point intersection），而等变量子上同调中的量子 Schubert 演算涉及的是曲线的计数（curve counting）。
...
\begin{Corollary}[$T'$-等变量子上同调中的量子 Schubert 演算 Positivity {\cite[Corollary 7.1]{Mihalcea}}]
\label{cor:TdashPositivity}
在 $T'$-等变量子上同调中的量子 Schubert 演算中，EQLR 系数是关于变量 $T_1 - T_2, \ldots, T_{m-1} - T_m$ 的非负系数多项式。
\end{Corollary}
...
EQLR 系数 $c_{u,v}^{w,d}$ 是（3 点，带基点，亏格 0）Gromov-Witten 不变量（Gromov-Witten invariant）\footnote{定义见 \cite[Definition 1.2]{Mihalcea}。问：什么是 Gromov-Witten 不变量？见附录 \cref{sec:GW-invariant}。}：
...
\textbf{为什么这不显然？} 在等变量子上同调（equivariant cohomology）中，类不再是子簇的直接上同调类，而是提升到 $X \times_T ET$（即 $X$ 关于 torus 作用的同伦商空间/homotopy quotient）上的类。在这种更抽象的设置中，上同调类的相交理论变得复杂——我们不再能简单地说"两条子簇相交于几个点"，因为相交的结果可能是一个上同调类，而不是一个数字。在这种更抽象的设置中，非负性不再能从几何相交直接读出。Mihalcea 的证明揭示了隐藏的统一结构。
...
本章我们跟随 Mihalcea 的工作，探索了等变量子上同调中的量子 Schubert 演算中的正性问题。让我们回顾一下这段旅程的脉络：

\textbf{问题的起源}：从经典 Schubert calculus 到等变量子上同调（equivariant cohomology），Graham 证明了 Peterson 猜想——等变系数仍然满足正性。一个自然的问题是：这个性质能否推广到量子上同调？

\textbf{核心突破}（Mihalcea, 2007）：答案是肯定的。等变量子上同调中的量子 Schubert 演算的结构常数 $c_{u,v}^{w,d}$ 仍然是关于负根 $x_i$ 的非负整数系数多项式。

关键要点如下：
\begin{enumerate}
\item \textbf{Peterson 猜想的历史}：由 Peterson 提出（1997），由 Graham 证明（2001），Mihalcea 将其推广到量子情形（2007）——这是一段跨越十年的数学探索；
\item \textbf{等变量子上同调中的量子 Schubert 演算} $QH_T^*(X)$ 是等变量子上同调（equivariant cohomology）和量子上同调（quantum cohomology）的共同推广，同时包含了这两者的结构；
