...
\begin{equation}
\mathfrak{S}_{321}(\mathbf{x}) = x_1^{2} x_2. \tag{7}
...
对于 $\ell(w) = 1$ 的置换，Schubert 多项式由其 Lehmer 码给出：\footnote{Lehmer 码 $c(w) = (c_1, \ldots, c_n)$ 定义为 $c_i = \#\{j > i : w(j) < w(i)\}$，即每个位置后面比它小的元素个数。例如 $w = 213$：$c_1 = 1$（$2,1$ 中 $1 < 2$），$c_2 = 0$（$3$ 中无比 $1$ 小），$c_3 = 0$。}
\begin{align}
...
&= (x_1x_2 - x_1y_2 - x_2y_2 + y_2^2)(x_1 - t_1) \label{eq:u132-v213-expand2} \\
&= x_1^2x_2 - x_1x_2t_1 - x_1^2y_2 + x_1y_2t_1 \label{eq:u132-v213-expand3} \\
&\quad - x_1x_2y_2 + x_2y_2t_1 + x_1y_2^2 - y_2^2t_1 \label{eq:u132-v213-full}
\end{align}
...
&= (x_1x_2 - x_1y_3 - x_2y_3 + y_2y_3)(x_1 - t_3) \label{eq:u231-v312-expand1} \\
&= x_1^2x_2 - x_1x_2t_3 - x_1^2y_3 + x_1y_3t_3 \label{eq:u231-v312-expand2} \\
&\quad - x_1x_2y_3 + x_2y_3t_3 + x_1y_2y_3 - y_2y_3t_3 \label{eq:u231-v312-full}
\end{align}
...
其中 $F_\alpha$ 是权为 $-\alpha$ 的根向量（root vector）。\footnote{问：$F_\alpha$ 是权为 $-\alpha$ 的根向量是什么意思？见附录 \cref{sec:RootVectorWeight}。}
...
现在取 $[\overline{B^-uB/B}]_T$ 与两边做 Poincaré 对偶。左边是 $[\overline{B^-uB/B}]_T|_w$，右边是 (3) 中 $[\overline{BuB/B}]_T$ 的系数，由 \cite[Proposition 7.3]{anderson2023}，这就给出 (2)。
...
$$\mathfrak{S}_\pi(\mathbf{x}; \mathbf{0}) \cdot \mathfrak{S}_\sigma(\mathbf{x}; \mathbf{0}) = \sum_w c^w_{\pi,\sigma}(0, 0) \cdot \mathfrak{S}_w(\mathbf{x}; \mathbf{0}) \tag*{(1)}$$
...
另一方面，skew 除差算子 $\partial_{w/\sigma}$ 作用于 $\mathfrak{S}_\pi$ 时，其系数与式 (1) 中的系数有如下关系（见 Macdonald \cite[Section 5]{macdonald1991}）：
$$\partial_{w/\sigma}(\mathfrak{S}_\pi(\mathbf{x})) = \sum_\tau c^\tau_{\pi,\sigma}(0, 0) \cdot \mathfrak{S}_\tau(\mathbf{x}) \tag*{(2)}$$

式 (2) 表明：$\partial_{w/\sigma}(\mathfrak{S}_\pi)$ 在 Schubert 基下的展开系数，等于 triple Schubert 系数 $c^\tau_{\pi,\sigma}$ 在 $\mathbf{y} = \mathbf{t} = 0$ 时的特殊值。
