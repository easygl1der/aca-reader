...
由 Graham Positivity 定理，它们的乘积展开系数 $c^w_{u,v}(\mathbf{y}, \mathbf{t})$ 必为 $t_i - y_j$ 的非负整数系数多项式。
...
验证：系数 $c^{21}_{12,21} = 1 \in \mathbb{N}$，$c^{12}_{12,21} = t_1 - y_1 \in \mathbb{N}[t_1 - y_1]$，符合正性要求。
\end{Example}

\begin{Example}[Samuel 猜想：一般排列的正性]
\label{ex:SamuelConjectureExample}
Samuel 猜想声称：\textbf{即使 $u, v$ 不是 Grassmannian 排列}，展开系数仍然保持正性。
...
整理后可得展开系数均为 $t_i - y_j$ 的非负多项式，验证了猜想在小规模情形下的正确性。
\end{Example}

\begin{Example}[Kirillov 猜想：Skew 除差算子的正性]
\label{ex:KirillovConjectureExample}
令 $\mathbf{y} = 0$，Samuel 猜想退化为 Kirillov 猜想：
...
利用除差算子的递推性质逐步计算，可验证结果的所有系数均为非负整数——这正是 Kirillov 猜想所预测的。
\end{Example}
