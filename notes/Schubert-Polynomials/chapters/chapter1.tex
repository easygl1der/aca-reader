...
\textbf{注}：此处计算较为复杂，且与除差算子定义的结果存在差异。正确的双重 Schubert 多项式应通过几何意义或标准公式验证。\footnote{问：双重 Schubert 多项式的标准公式是什么？见附录 \cref{sec:DoubleSchubertDividedDifference}。}
...
有了这些准备，我们现在可以进入正式的证明。证明的核心思想是将几何相交（交集在等变量子上同调中的类）翻译为代数展开系数，然后利用上述引理证明这些系数必为非负。\footnote{问：几何相交如何翻译为代数展开系数？除差算子的几何意义是什么？见附录 \cref{sec:GeoToAlgebraTranslation}。}
