...
其中 $\mathfrak{S}_{312} = x_1^2$ 意味着管道图只有 $x_1^2$ 这一项，而 $\mathfrak{S}_{231} = x_1 x_2$ 意味着首项是 $x_1 x_2$ 但由于对称性，实际多项式可能更复杂。
...
\item $\mathfrak{S}_{132} = x_2$
\item $\mathfrak{S}_{231} = x_1 x_2$
\item $\mathfrak{S}_{312} = x_1$
...
\label{def:ET}
设 $T \cong (\mathbb{C}^\times)^n$ 为一个 $n$ 维 torus，其中 $\mathbb{C}^\times = \mathbb{C} \setminus \{0\}$ 表示非零复数的乘法群，$\times$ 表示笛卡尔积（Cartesian product）。因此 $T$ 同构于 $n$ 维复环面（complex torus），也可以写作 $T \cong (S^1)^n$，这是 $n$ 维实环面。$T$ 的分类空间 (classifying space) $ET$ 是一个拓扑空间, 满足以下两个核心属性:
\begin{enumerate}
...
当 $u$ 和 $v$ 是 Grassmannian 排列时，$\mathfrak{S}_u(\mathbf{x}; \mathbf{y})$ 和 $\mathfrak{S}_v(\mathbf{x}; \mathbf{t})$ 是因子化 Schur 多项式（factorial Schur polynomials）。Molev-Sagan \cite{molevsagan1999} 首次给出了这些系数的组合公式，Knutson-Tao \cite{knutson2003} 给出了展开的几何意义。Graham \cite{Gr} 证明了以下正性结果：
...
本文证明依赖于精化版本的 Graham 正性定理（\cref{def:RefinedGraham}），建立了系数 $c^w_{u,v}(\mathbf{y}, \mathbf{t})$ 的新几何解释，\footnote{这与 Knutson-Tao \cite{knutson2003} 给出的解释不同。}作为副产品还给出了 Billey 公式 \cite{billey1999} 中正性的几何解释。
...
由\cref{def:GrahamPositivity}，它们的乘积展开系数 $c^w_{u,v}(\mathbf{y}, \mathbf{t})$ 必为 $t_i - y_j$ 的非负整数系数多项式。

具体计算（取 $n=3$ 的简化情形）：
$$\mathfrak{S}_{12}(\mathbf{x}; \mathbf{y}) \cdot \mathfrak{S}_{21}(\mathbf{x}; \mathbf{t}) = (x_1 - t_1) = \mathfrak{S}_{21}(\mathbf{x}; \mathbf{t})$$

验证：此处恰好有 $c^{21}_{12,21} = 1 \in \mathbb{N}$，符合正性。对于涉及不同 $\mathbf{y}, \mathbf{t}$ 变量的情形，系数会是 $t_i - y_j$ 的非负多项式。
\end{Example}

\begin{Example}[\cref{conj:Samuel} 的验证]
\label{ex:SamuelConjectureExample}
\cref{conj:Samuel} 声称：\textbf{即使 $u, v$ 不是 Grassmannian 排列}，展开系数仍然保持正性。

在 $S_3$ 中验证\cref{conj:Samuel}：取 $u = 21$，$v = 12$（两个简单的 Grassmannian 排列）。计算
$$\mathfrak{S}_{21}(\mathbf{x};\mathbf{y}) \cdot \mathfrak{S}_{12}(\mathbf{x};\mathbf{t}) = (x_1 - y_1) \cdot 1 = x_1 - y_1 = \mathfrak{S}_{21}(\mathbf{x};\mathbf{t})$$

展开系数为 $1 \in \mathbb{N}$，符合正性。
\end{Example}

\begin{Example}[\cref{cor:Kirillov} 的验证]
\label{ex:KirillovConjectureExample}
令 $\mathbf{y} = 0$，\cref{conj:Samuel} 退化为\cref{cor:Kirillov}。对 $S_3$ 中的 Grassmannian 排列 $u = 231$，有 $\mathfrak{S}_{231} = x_1x_2$。验证 skew 除差算子
$$\partial_{231/12}(\mathfrak{S}_{231}) = x_1$$
结果系数非负，验证了\cref{cor:Kirillov}。
\end{Example}
...
$$\mathfrak{S}_{21}(\mathbf{x};\mathbf{y}) = x_1, \quad \mathfrak{S}_{213}(\mathbf{x};\mathbf{t}) = x_1$$
两者相乘得
$$\mathfrak{S}_{21}(\mathbf{x};\mathbf{y}) \cdot \mathfrak{S}_{213}(\mathbf{x};\mathbf{t}) = x_1^2$$
展开系数为 $1 \in \mathbb{N}$，确实是非负整数。

更复杂的非平凡例子：取 $u = 231$（有 2 个 descents，\textbf{非} Grassmannian）和 $v = 312$。由 $S_3$ 表：
$$\mathfrak{S}_{231}(\mathbf{x}; \mathbf{y}) = x_1 x_2 - x_1 y_3 - x_2 y_3 + y_2 y_3$$
$$\mathfrak{S}_{312}(\mathbf{x}; \mathbf{t}) = x_1 - t_3$$
\begin{align*}
\mathfrak{S}_{231} \cdot \mathfrak{S}_{312} &= x_1^2 x_2 - x_1^2 y_3 - x_1 x_2 y_3 - x_1 x_2 t_3 + x_1 y_3 t_3 + x_2 y_3 t_3 + x_1 y_2 y_3 - y_2 y_3 t_3
\end{align*}
利用 $t_3 = (t_3 - y_3) + y_3$，每个系数可化为 $t_i - y_j$ 的多项式，验证了 \cref{conj:Samuel} 的正性。
\end{Example}
