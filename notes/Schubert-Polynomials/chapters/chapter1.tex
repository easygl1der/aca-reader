...
$S_\infty$ 表示所有有限置换的并集，即 $S_\infty = \bigcup_{n=1}^\infty S_n$，可以理解为可数无限集 $\{1, 2, \ldots\}$ 上的对称群。例如，$w = (3,1,4,2,5,\ldots) \in S_\infty$ 表示一个有限支撑的置换（只有前4位发生交换）。
\end{Definition}
...
对于两个置换 $w, v \in S_n$，\textbf{\underline{Skew 除差算子}} $\partial_{w/v}$ 定义为：
\begin{equation}
\partial_{w/v} = \partial_{w} \circ \partial_{v}^{-1}
\end{equation}
其中 $\partial_{v}^{-1}$ 是 $\partial_v$ 的逆算子，$\circ$ 表示算子复合。
...
我们在\cref{def:DoubleSchubertPolynomial}中已经看到双重 Schubert 多项式对应于旗流形上的等变量子上同调类. 本节我们将详细介绍这一核心概念.
...
具体计算（取 $n=3$ 的简化情形）：
$$\mathfrak{S}_{12}(\mathbf{x}; \mathbf{y}) \cdot \mathfrak{S}_{21}(\mathbf{x}; \mathbf{t}) = \mathfrak{S}_{21}(\mathbf{x}; \mathbf{t}) + (t_1 - y_1)\mathfrak{S}_{12}(\mathbf{x}; \mathbf{t})$$

验证：系数 $c^{21}_{12,21} = 1 \in \mathbb{N}$，$c^{12}_{12,21} = t_1 - y_1 \in \mathbb{N}[t_1 - y_1]$，符合\cref{def:GrahamPositivity} 的正性要求。
\end{Example}
...
在 $S_3$ 中验证\cref{conj:Samuel}：
$$w = 213,\quad u = 132,\quad v = 231$$

计算（利用 $S_3$ 的双重 Schubert 多项式表）：
\begin{align*}
S_{132}(x; y) \cdot S_{231}(x; t) &= (x_2 - y_2) \cdot (x_1x_2 - x_1t_3 - x_2t_3 + t_2t_3) \\
&= \cdots
\end{align*}

整理后可得展开系数均为 $t_i - y_j$ 的非负多项式，验证了\cref{conj:Samuel} 在小规模情形下的正确性。
\end{Example}

\begin{Example}[\cref{cor:Kirillov} 的验证]
\label{ex:KirillovConjectureExample}
令 $\mathbf{y} = 0$，\cref{conj:Samuel} 退化为\cref{cor:Kirillov}：

对 $S_3$ 中的置换，验证 $\partial_{w/v} \mathfrak{S}_u(\mathbf{x}) \in \mathbb{N}[\mathbf{x}]$。

取 $u = 231$，$v = 12$（恒等置换），$w = 231$：
$$\partial_{231/12}(\mathfrak{S}_{231}) = \partial_{231}(x_1x_2) = ?$$

利用除差算子的递推性质逐步计算，可验证结果的所有系数均为非负整数——这正是\cref{cor:Kirillov} 所预测的。
\end{Example}
...
\textbf{Lemma 2.2}（正规子群引理）建立了归纳法的李代数基础：若 $ws_i > w$，则 $N^-(ws_i)$ 是 $N^-(w)$ 的正规子群。这允许我们从短长度排列逐步构造长的分解。

\textbf{Lemma 2.5}（横截相交引理）是整个证明的核心技术工具：交集 $\tau \overline{B^-uB/B} \cap \overline{B^-vB/B}$ 是 proper 且横截的。横截性保证了几何相交是"好"的——没有奇点、没有退化的情形。

\textbf{Corollary 2.4}将这些观察组织成一个简洁的公式：任意 $B^-$-不变的有效闭链必是 Schubert 类的非负线性组合。这建立了从几何闭链到代数展开系数的桥梁。

\textbf{具体例子}：取 $u = 21$（一个 Grassmannian 排列）和 $v = 213$（另一个 Grassmannian 排列），有
...
展开系数为 $1 \in \mathbb{N}$，确实是非负整数。对于涉及 $\mathbf{y}, \mathbf{t}$ 变量的情形（如 Theorem 1.1），系数会是关于 $t_i - y_j$ 的多项式，但非负性依然成立。
