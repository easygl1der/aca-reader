...
\begin{itemize}
\item $x_1^2 = x_1^2$（留在 $\mathfrak{S}_{321}$ 的系数中，$w_0 = 321$ 是最长置换）
...
&\quad + (t_1 - y_1) \cdot \mathfrak{S}_{21}(\mathbf{x}; \mathbf{t}) \tag{$c^{21} = t_1 - y_1 \in \mathbb{N}[t_j - y_i]$} \\
&\quad + \text{其他 } (t_j - y_i) \text{ 的非负整数系数项}
\end{align}

注意 $c^{21} = t_1 - y_1$ 是一个关于 $t_j - y_i$ 的多项式（这里只有一项），其系数是非负整数 $1$。这验证了 \cref{def:GrahamPositivity} 的断言。

\textbf{澄清一个常见误解：} 前面计算中出现的 $x_1^2$ 在 $\mathbf{y} = \mathbf{t} = 0$ 时确实等于 $\mathfrak{S}_{321}(\mathbf{x}; \mathbf{0})$，但在一般情况下不能简单地说 "$x_1^2$ 是 $\mathfrak{S}_{231}$"。实际上（见\cref{ex:DoubleSchubertS3}）：
$$\mathfrak{S}_{231}(\mathbf{x}; \mathbf{y}) = x_1 x_2 - x_1 y_3 - x_2 y_3 + y_2 y_3$$
这只在 $y = t = 0$ 时才简化为 $x_1x_2$。
\end{Example}
