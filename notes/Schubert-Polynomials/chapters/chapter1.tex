...
在 $S_3$ 中，$123$ 是最小元，$321$ 是最大元。S$_3$ 的 Bruhat order 是偏序（部分元素不可比较），其 Hasse 图为：
\[
\begin{tikzpicture}[node distance=1.5cm]
\node (321) at (0,4) {$321$};
\node (312) at (-2,2.5) {$312$};
\node (231) at (0,2.5) {$231$};
\node (132) at (2,2.5) {$132$};
...
第三步：配凑——利用恒等式 $t_j = (t_j - y_i) + y_i$ 将每项改写为"\textbf{$(t_j - y_i)$ 因子} $+$ \textbf{不含 $t$ 的余项}"的形式：

\begin{itemize}
\item $x_1x_2$：已是 $\mathfrak{S}_{231}(\mathbf{x}; \mathbf{t})$ 的首项，系数为 $1$（非负整数）。

\item $-x_1t_2 = -x_1\bigl[(t_2 - y_2) + y_2\bigr] = -x_1(t_2 - y_2) - x_1y_2$

\item $-y_2x_2 = -y_2(x_2 - t_2) - y_2t_2
       = -y_2(x_2 - t_2) - y_2\bigl[(t_2 - y_2) + y_2\bigr]
       = -y_2(x_2 - t_2) - y_2(t_2 - y_2) - y_2^2$

\item $y_2t_2 = y_2\bigl[(t_2 - y_2) + y_2\bigr] = y_2(t_2 - y_2) + y_2^2$
\end{itemize}

第四步：合并同类项——将所有含 $(t_2 - y_2)$ 的项提出，余下的 $y$ 多项式归入系数 $c^w_{u,v}(\mathbf{y}, \mathbf{t})$：
...
\textbf{最终形式}：
\begin{align}\label{eq:GrahamPositivity-result}
(x_1 - y_2)(x_2 - t_2)
&= 1 \cdot \mathfrak{S}_{231}(\mathbf{x}; \mathbf{t}) \tag{非负整数系数} \\
&\quad + 1 \cdot (t_2 - y_2) \cdot \mathfrak{S}_{132}(\mathbf{x}; \mathbf{t}) \tag{非负整数系数} \\
&\quad + \text{其他 } (t_j - y_i) \text{ 的非负整数系数项}
\end{align}

式 \eqref{eq:GrahamPositivity-result} 表明：每个 Schubert 多项式的系数确实是 $t_j - y_i$ 的非负整数系数多项式——这正是 \cref{def:GrahamPositivity} 的核心断言。
\end{Example}
