...
\begin{Definition}[\textbf{\underline{Proper（正常/固有）交集}}]
...
\begin{Definition}[\textbf{\underline{Transverse（横截）交集}}]
...
\textbf{为什么 Lemma 2.5 关注这两个性质？} Proper 保证交集是紧的（有限多个点），transverse 保证每个交点都是良定的（横截点）。两者结合使得在等变上同调中可以定义明确的相交类，从而得到非负的展开系数。
...
\end{equation}\footnote{问：为什么初始条件是这个形式？见附录 \cref{sec:InitialCondition}。}
...
$231$ & $(2,3,1)$ & $(1,1,0)$ & $2$ & $x_1x_2$ \\
$312$ & $(3,1,2)$ & $(2,0,0)$ & $2$ & $x_1+x_2$ \\
$321$ & $(3,2,1)$ & $(2,1,0)$ & $3$ & $x_1^2x_2$
\end{tabular}
\end{center}
...
$$\mathfrak{S}_\pi(\mathbf{x}; \mathbf{0}) \cdot \mathfrak{S}_\sigma(\mathbf{x}; \mathbf{0}) = \sum_w c^w_{\pi,\sigma}(0, 0) \cdot \mathfrak{S}_w(\mathbf{x}; \mathbf{0}) \tag{★}$$
...
另一方面，skew 除差算子 $\partial_{w/\sigma}$ 作用于 $\mathfrak{S}_\pi$ 时，其系数与 (★) 中的系数有如下关系（见 Macdonald \cite[Section 5]{macdonald1991}）：
$$\partial_{w/\sigma}(\mathfrak{S}_\pi(\mathbf{x})) = \sum_\tau c^\tau_{\pi,\sigma}(0, 0) \cdot \mathfrak{S}_\tau(\mathbf{x}) \tag{★★}$$

式 (★★) 表明：$\partial_{w/\sigma}(\mathfrak{S}_\pi)$ 在 Schubert 基下的展开系数，等于 triple Schubert 系数 $c^\tau_{\pi,\sigma}$ 在 $\mathbf{y} = \mathbf{t} = 0$ 时的特殊值。
