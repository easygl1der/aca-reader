...
如图 \ref{fig:ConjectureHierarchy} 所示，三个猜想的条件依次减弱——从要求 $u, v$ 都是 Grassmannian，到任意排列，再到最简单的 $\mathbf{y} = \mathbf{t} = 0$ 情形。

\begin{figure}[h]
\centering
\begin{tikzpicture}[node distance=3cm, on grid, >=Stealth]
\node (Graham) [rectangle, draw] {$u, v$ 都是 Grassmannian\\ Graham Positivity};
\node (Samuel) [rectangle, draw, right=of Graham] {$u, v$ 任意排列\\ Samuel 猜想};
\node (Kirillov) [rectangle, draw, below=of Samuel] {$\mathbf{y} = \mathbf{t} = 0$\\ Kirillov 猜想};

\draw[->] (Graham) -- (Samuel) node[midway, above] {去掉 Grassmannian 限制};
\draw[->] (Samuel) -- (Kirillov) node[midway, right] {取特殊值 $\mathbf{y} = \mathbf{t} = 0$};
\draw[->, dashed] (Graham) to [bend left=30] node[midway, left] {取特殊值 $\mathbf{y} = \mathbf{t} = 0$} (Kirillov);
\end{tikzpicture}
\caption{三个正性猜想的层次关系：条件依次减弱，正性结论逐步推广。}
\label{fig:ConjectureHierarchy}
\end{figure}
...
关键观察：利用 $t_3 = (t_3 - y_3) + y_3$ 展开含 $t_3$ 的项：
\begin{align*}
...
所以 \cref{cor:Kirillov} 断言 $\partial_{w/v}(\mathfrak{S}_u) \in \mathbb{N}[\mathbf{x}]$——即 skew 除差算子作用在 Schubert 多项式上仍保持系数非负。

\textbf{一个具体例子：} 取 $u = 21$，$v = 21$。由前面的计算（令 $\mathbf{y} = \mathbf{t} = 0$）：
$$(x_1)(x_1) = x_1^2$$
在 S$_3$ 中，$\mathfrak{S}_{321}(\mathbf{x}; \mathbf{0}) = x_1^2$，因此 $c^{321}_{21,21}(0,0) = 1 \in \mathbb{N}$。
\end{Example}
