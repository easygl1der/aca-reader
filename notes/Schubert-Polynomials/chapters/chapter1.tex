...
\item \textbf{Lemma \ref{def:Lemma25}}（横截相交引理（Transverse intersection lemma））：这是 Refined 版本的核心技术工具。它断言交集 $\tau \overline{B^-uB/B} \cap \overline{B^-vB/B}$ 是 proper 且横截的。横截性意味着相交是``好的``——没有奇点、退化或不确定的点个数。横截相交的非负性来自于拓扑学的基本事实：横截相交的交集点个数是良定义的整数。
