...
其中 Grassmannian 置换（如 $132, 231$）的 Schubert 多项式是单项式 $x_1^{c_1} x_2^{c_2}$；非 Grassmannian 置换（如 $213, 312$）的 Schubert 多项式是\text{对称}多项式.
...
更等价且便于计算的形式是 Lascoux-Schützenberger 的组合定义{}\footnote{问：什么是 Lascoux-Schützenberger 型代表元？见附录 \cref{sec:LascouxSchutzenberger}。}：考虑旗流形中的 Schubert 胞腔.
...
其中 $F_\alpha$ 是权为 $-\alpha$ 的根向量（root vector）。
...
具体来说，设 $u = w$ 且 $v = w_0 / w$（其中 $w_0$ 是最长置换）\footnote{问：$w_0 / w$ 这个记号是什么意思？见附录 \cref{sec:permutation-division}。}，则：
