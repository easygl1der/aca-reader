...
\textbf{注}：此处计算较为复杂，且与除差算子定义的结果存在差异。正确的双重 Schubert 多项式应通过几何意义或标准公式验证。\footnote{问：双重 Schubert 多项式的标准公式是什么？见附录 \cref{sec:DoubleSchubertStandard}。}
...
\subsection{等变上同调（Equivariant cohomology）}

我们在\cref{def:DoubleSchubertPolynomial}中已经看到双重 Schubert 多项式对应于旗流形上的等变上同调类.\footnote{问：等变上同调类是什么？见附录 \cref{sec:EquivariantCohomologyClass}。} 本节我们将详细介绍这一核心概念.
...
\begin{Definition}[\textbf{\underline{等变上同调（Equivariant cohomology）}}]
\label{def:EquivariantCohomology}
设 $T \cong (\mathbb{C}^\times)^n$ 为一个 torus, $X$ 是一个带有 $T$ 作用的拓扑空间. 空间 $X$ 上的 $T$-等变上同调定义为:
\begin{equation}
H_T^*(X) = H^*(X \times_T ET)
\end{equation}
...
等变上同调将传统的上同调与 torus 的作用相结合, 产生了丰富的代数结构. 它使得我们可以把复杂的几何交点计算问题转化为多项式环上的线性代数问题.
