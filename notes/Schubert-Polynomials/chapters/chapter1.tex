...
\begin{Example}[\cref{def:GrahamPositivity} 的验证（条件最强）]
\label{ex:GrahamPositivityExample}
\cref{def:GrahamPositivity} 要求 $u, v$ 都是 Grassmannian 排列。

考虑 $S_4$ 中的两个 Grassmannian 排列：
\begin{itemize}
\item $u = 1324$：前两位 $1<3$ 递增，后两位 $3>2$ 递减，故是 2-Grassmannian
\item $v = 2413$：前两位 $2<4$ 递增，后两位 $4>1$ 递减，故是 2-Grassmannian
\end{itemize}

具体计算（取 $n=3$ 的简化情形）：
$$\mathfrak{S}_{12}(\mathbf{x}; \mathbf{y}) \cdot \mathfrak{S}_{21}(\mathbf{x}; \mathbf{t}) = (x_1 - t_1) = \mathfrak{S}_{21}(\mathbf{x}; \mathbf{t})$$

验证：展开系数 $c^{21}_{12,21} = 1 \in \mathbb{N}$，符合正性。

更复杂的例子：取 $u = 213$，$v = 132$。由 $S_3$ 表：
$$\mathfrak{S}_{213}(\mathbf{x}; \mathbf{y}) = x_1 - y_2,\quad \mathfrak{S}_{132}(\mathbf{x}; \mathbf{t}) = x_2 - t_2$$
\begin{align*}
\mathfrak{S}_{213} \cdot \mathfrak{S}_{132} &= (x_1 - y_2)(x_2 - t_2) \\
&= x_1 x_2 - x_1 t_2 - y_2 x_2 + y_2 t_2
\end{align*}
利用 $y_2 t_2 = y_2(t_2 - y_2) + y_2^2$，每项系数都是 $t_j - y_i$ 的多项式，验证了 \cref{def:GrahamPositivity}。
\end{Example}

\begin{Example}[\cref{conj:Samuel} 的验证（条件减弱）]
\label{ex:SamuelConjectureExample}
\cref{conj:Samuel} 去掉了 Grassmannian 的限制，声称对\textbf{任意} $u, v$ 都有正性。

取 $u = 231$（有 2 个 descents，非 Grassmannian）和 $v = 312$：
$$\mathfrak{S}_{231}(\mathbf{x}; \mathbf{y}) = x_1 x_2 - x_1 y_3 - x_2 y_3 + y_2 y_3$$
$$\mathfrak{S}_{312}(\mathbf{x}; \mathbf{t}) = x_1 - t_3$$
\begin{align*}
\mathfrak{S}_{231} \cdot \mathfrak{S}_{312} &= x_1^2 x_2 - x_1^2 y_3 - x_1 x_2 y_3 - x_1 x_2 t_3 \\
&\quad + x_1 y_3 t_3 + x_2 y_3 t_3 + x_1 y_2 y_3 - y_2 y_3 t_3
\end{align*}
利用 $t_3 = (t_3 - y_3) + y_3$，每项系数都可化为 $t_j - y_i$ 的多项式，验证了 \cref{conj:Samuel} 在非 Grassmannian 情形下的正性。
\end{Example}

\begin{Example}[\cref{cor:Kirillov} 的验证（条件最弱）]
...
令 $\mathbf{y} = \mathbf{t} = 0$，得 $x_1 \in \mathbb{N}[\mathbf{x}]$。更直接的验证：
$$\partial_1(x_1) = \frac{x_1 - x_1}{x_1 - x_1} = 1 \in \mathbb{N}[\mathbf{x}]$$
这验证了 skew 除差算子 $\partial_{w/v}$ 作用在 Schubert 多项式上得到非负系数，即 \cref{cor:Kirillov}。
\end{Example}
