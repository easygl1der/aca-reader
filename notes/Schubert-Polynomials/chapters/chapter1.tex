...
$$\mathfrak{S}_{123} = 1, \quad \mathfrak{S}_{231} = x_1x_2, \quad \mathfrak{S}_{321} = x_1^2x_2$$
...
\begin{Proof}由于旗流形只有有限多个 $B^-$-轨道，任意 $B^-$-不变的有效闭链在 $G/B$ 上必是 Schubert 类 $[\overline{B^-wB/B}]_T$ 的非负线性组合。\end{Proof}
\end{Corollary}
...
\begin{Proof}由 \cref{def:Lemma25}，我们可重写所感兴趣的系数：
...
\begin{Proof} 我们限制到 $G = \mathrm{GL}_{2n}(\mathbb{C})$ 的情形。设 $u, v \in S_n$ 满足 separated descents 条件。考虑特殊排列 $\tau \in S_{2n}$ 满足 $\tau(i) = n + i$，$\tau(n + i) = i$。
...
\begin{Proof} 该定理是 \cref{def:Theorem12} 的直接推论.
