...
\textbf{例子：$u = 21$，$v = 21$}（都是 Grassmannian）。

第一步：查表得 S$_3$ 中双重 Schubert 多项式（见\cref{ex:DoubleSchubertS3}）：
$$\mathfrak{S}_{21}(\mathbf{x}; \mathbf{y}) = x_1 - y_1$$
注意：这里的 $y$ 变量是占位符，当我们将两个 $\mathfrak{S}_{21}$ 相乘时，第二个的 $y$ 要替换为 $t$。

第二步：计算乘积
\begin{align*}
\mathfrak{S}_{21}(\mathbf{x}; \mathbf{y}) \cdot \mathfrak{S}_{21}(\mathbf{x}; \mathbf{t})
&= (x_1 - y_1)(x_1 - t_1) \\
&= x_1^2 - x_1 t_1 - x_1 y_1 + t_1 y_1 \tag{展开}
\end{align*}

第三步：写成 $c^w_{u,v}(\mathbf{y}, \mathbf{t}) \cdot \mathfrak{S}_w(\mathbf{x}; \mathbf{t})$ 的形式

这里的 $c^w_{u,v}(\mathbf{y}, \mathbf{t})$ 必须是 $t_j - y_i$ 的非负整数系数多项式。

利用恒等式 $t_1 = (t_1 - y_1) + y_1$，将每项改写：

\begin{itemize}
\item $x_1^2 = x_1^2$（留在 $\mathfrak{S}_{w_0}$ 的系数中，$w_0 = 321$ 是最长置换）

\item $-x_1 t_1 = -x_1[(t_1 - y_1) + y_1] = -x_1(t_1 - y_1) - x_1 y_1$

\item $-x_1 y_1 = -x_1 y_1$

\item $t_1 y_1 = (t_1 - y_1 + y_1) y_1 = y_1(t_1 - y_1) + y_1^2$
\end{itemize}
...
\textbf{为什么系数是"非负"的？} 注意 $c^{21} = t_1 - y_1 = 1 \cdot t_1 + (-1) \cdot y_1$，这是一个关于 $t_j - y_i$ 的多项式（这里只有一项），其系数是非负整数 $1$。符合 \cref{def:GrahamPositivity} 的断言。

\textbf{正确的 $\mathfrak{S}_{231}$ 等形式：} 前面例子中我错误地说 "$x_1x_2$ 是 $\mathfrak{S}_{231}$"。实际上，在 S$_3$ 中（见\cref{ex:DoubleSchubertS3}）：
$$\mathfrak{S}_{231}(\mathbf{x}; \mathbf{y}) = x_1 x_2 - x_1 y_3 - x_2 y_3 + y_2 y_3$$
这只在 $y = t = 0$ 时才简化为 $x_1x_2$。原例子的错误在于混淆了双重 Schubert 多项式和其在特殊点上的取值。
\end{Example}
