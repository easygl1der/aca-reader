...
\textbf{具体例子}：取 $u = 21$（一个 Grassmannian 排列，对应 Grassmannian $\mathbb{G}(1,2)$）和 $v = 132$（另一个 Grassmannian 排列），有
$$\mathfrak{S}_{21}(\mathbf{x};\mathbf{y}) = x_1, \quad \mathfrak{S}_{132}(\mathbf{x};\mathbf{t}) = x_2$$
两者相乘得
$$\mathfrak{S}_{21}(\mathbf{x};\mathbf{y}) \cdot \mathfrak{S}_{132}(\mathbf{x};\mathbf{t}) = x_1 x_2$$
展开系数为 $1 \in \mathbb{N}$，确实是非负整数。对于更一般的情形（如 Theorem 1.1），系数会是关于 $t_i - y_j$ 的多项式，但非负性依然成立。
