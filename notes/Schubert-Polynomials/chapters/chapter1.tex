...
双重 Schubert 多项式 $\mathfrak{S}_w(\mathbf{x}; \mathbf{y})$ 是 Schubert 多项式的双变量推广，其中 $x = (x_1, \ldots, x_n)$ 和 $y = (y_1, \ldots, y_n)$ 是两组变量。
...
设 $X$ 是光滑复流形，$\beta \in H_2(X, \mathbb{Z})$ 表示曲线类。量子上同调环 $QH^*(X)$ 的乘法 $\star$ 定义为：
\begin{equation}
...
\subsection{Lie 代数基础}
\label{sec:LieAlgebraBasics}

\begin{Definition}[代数群与旗流形]
\label{def:AlgebraicGroup}
设 $G$ 为连通复约化代数群，$B \subset G$ 为 Borel 子群，$B^- \subset G$ 为其相反 Borel 子群，$T = B \cap B^-$ 为极大环面，$N$（分别 $N^-$）为 $B$（分别 $B^-$）的幂幺根基。
\end{Definition}

\begin{Definition}[李代数]
\label{def:LieAlgebra}
每个代数群 $G$ 对应一个李代数 $\mathfrak{g} = \mathrm{Lie}(G)$，它是群在单位元处的切空间。

对于 $G = \mathrm{GL}_n(\mathbb{C})$，其李代数 $\mathfrak{gl}_n(\mathbb{C})$ 由所有 $n \times n$ 矩阵组成，李括号为 $[X, Y] = XY - YX$。
\end{Definition}

\begin{Definition}[Cartan 子代数与根系]
\label{def:CartanRootSystem}
Cartan 子代数 $\mathfrak{h}$ 是 $\mathfrak{g}$ 中的一个极大阿贝尔子代数。对于 $G = \mathrm{GL}_n(\mathbb{C})$，可以取所有对角矩阵构成的子代数作为 Cartan 子代数。

根系（root system）$\Phi$ 是 $\mathfrak{h}^*$ 中的有限子集。每个根 $\alpha \in \Phi$ 对应一个一维根空间 $\mathfrak{g}_\alpha$，且有分解
$$\mathfrak{g} = \mathfrak{h} \oplus \bigoplus_{\alpha \in \Phi} \mathfrak{g}_\alpha$$
\end{Definition}

\begin{Definition}[反射与 Weyl 群]
\label{def:WeylGroup}
反射（reflection）$s_\alpha$ 是 $\mathfrak{h}^*$ 上关于超平面 $\ker(\alpha)$ 的线性变换。Weyl 群是由根系 $\Phi$ 中的所有反射生成的有限群。设 $\Delta = \{\alpha_1, \ldots, \alpha_r\}$ 为单根集合，每个单根 $\alpha_i$ 对应一个基本反射 $s_i = s_{\alpha_i}$。

Weyl 群也可以表示为 $W = N_G(T)/T$，其中 $T$ 是极大环面，$N_G(T)$ 是其在 $G$ 中的正规化子。对于 $\alpha \in \Phi$，记 $s_\alpha$ 为对应的反射，简记 $s_i = s_{\alpha_i}$。
\end{Definition}

\begin{Definition}[逆序集与非逆序集]
...
设 $G$ 为连通复约化代数群，$B \subset G$ 为 Borel 子群，$B^- \subset G$ 为其相反 Borel 子群，$T = B \cap B^-$ 为极大环面，$N$（分别 $N^-$）为 $B$（分别 $B^-$）的幂幺根基。Lie 代数、反射、Weyl 群 $W$、逆序集 $I(w)$、$J(w)$ 以及子群 $N^-(w)$、$B^-(w)$ 的定义见\cref{def:LieAlgebraBasics}、\cref{def:WeylGroup}、\cref{def:InversionSet}、\cref{def:NMinusW}。

根据 Humphreys \cite[Section 28]{humphreys1975}，$N^-(w)$ 同构于 Schubert 胞腔 $B^-wB/B \cong \mathbb{C}^{\ell(w_0) - \ell(w)}$.
