...
$w = 312$（非 Grassmannian，$D(w) = \{1\}$）：
使用 Stanley 对称多项式展开：$\mathfrak{S}_{312}(\mathbf{x}) = x_1 + x_2$。

验证：$\mathfrak{S}_{312}$ 是对称多项式。
...
$w$ & 一行数组 & Lehmer 码 & $\ell(w)$ & $\mathfrak{S}_w(\mathbf{x})$ \\ \hline
$123$ & $(1,2,3)$ & $(0,0,0)$ & $0$ & $1$ \\
...
其中，Grassmannian 置换（$132, 231$）的 Schubert 多项式是单项式 $x_1^{c_1} x_2^{c_2}$；非 Grassmannian 置换（$213, 312$）的 Schubert 多项式是对称多项式。
