...
\textbf{注记（除差算子下标的说明）}：读者可能注意到这里使用了 $\partial_{ww_0}$，而经典版本使用的是 $\partial_{w^{-1}w_0}$。这两种记号实际上反映了 Weyl 群作用方向的不同约定。在 KM 的量子版本中，除差算子的下标使用 $ww_0$ 是为了与量子配对（quantum pairing）的正交性条件相匹配——具体而言，使用 $ww_0$ 使得正交性关系
$$\langle \widetilde{\mathfrak{S}}_u, \widetilde{\mathfrak{S}}_v \rangle_Q = \delta_{v, w_0 u}$$
具有更简洁的形式。这是 Kirillov-Maeno 量子化方案的一个特征，并非笔误。
...
\end{align*}
\end{Example}
\footnote{这些符号约定基于 \cite[ Section 4]{KM}。}
...
\textbf{三重版本的关键洞察}：当 Gao-Xiong 研究三重变量集 $(x, y, t)$ 时，他们实际上是在研究等变量量子上同调（equivariant quantum cohomology）的一个精化版本。\cref{prop:QuantumTripleConnection} 明确了两者的关系：Gao-Xiong 的正性结果对应于量子配对下 $\beta = 0$ 的特殊情形，而 Kirillov-Maeno 的量子双重 Schubert 多项式则给出了完整的代数框架。
