...
\textbf{但当我们研究旗流形（flag manifold）上曲线的个数时，普通的相交理论遇到了根本性的困难。}
...
这引导我们进入\textbf{量子上同调}（quantum cohomology）的世界——一个源于物理中拓扑场论（topological field theory）的思想，由 Kontsevich 和 Manin 系统化。

\textbf{核心观察}：在量子上同调（quantum cohomology）中，Schubert 类的乘法不再是普通的乘积，而是一种带有形式变量 $q^\beta$ 的新运算
\[
[\sigma_u] \star [\sigma_v] = \sum_{w,\beta} q^{\beta} c^w_{u,v}(\beta) [\sigma_w],
\tag{3.1}
\]
其中 $\beta$ 记录了连接子簇的曲线类（curve class），$q^{\beta}$ 是形式变量。这一公式最初来自物理中的 \textbf{A-twisted topological field theory}，它揭示了曲线计数与多项式结构常数之间的深层联系。
