...
\]
其中 $\beta$ 记录了连接子簇的曲线类（curve class），$q^{\beta}$ 是形式变量。这一公式最初来自物理中的 \textbf{A-twisted topological field theory}，它揭示了曲线计数与多项式结构常数之间的深层联系。\footnote{公式 \eqref{eq:QuantumSchubertMultiplication} 的详细解释和几何意义见附录 \cref{sec:quantum-multiplicity-meaning}。}
...
\textbf{澄清}：虽然 $\mathbb{P}^1$ 的 Gromov-Witten 不变量并非完全平凡，但相比高亏格或更复杂的旗流形，其量子结构相对简单。对于亏格 0 曲线，量子上同调环的维数与经典上同调相同，只是乘法结构被量子修正了。
\end{Example}
...
\textbf{关于 $\widetilde{e}_i$ 的显式定义}：虽然 $\widetilde{e}_i$ 在 \cite{KM} 中由行列式生成函数隐式定义，但附录 \cref{sec:quantum-elementary-explicit} 给出了其显式展开形式。简而言之，$\widetilde{e}_i(x|q)$ 是 $x$ 变量的初等对称多项式 $e_i$ 的量子修正，其中 $q$ 变量记录了旗流形中曲线类的信息。当所有 $q_j = 0$ 时，$\widetilde{e}_i(x|0) = e_i(x_1, \ldots, x_i)$。
...
\textbf{注记}：此处的除差算子下标 $\partial_{ww_0}$ 与经典情形（见 \eqref{eq:SchubertPolynomial}）的 $\partial_{w^{-1}w_0}$ 不同。两种约定在旗流形几何中有不同的方便之处，但当 $q=0$ 时结果一致。
...
\end{align*}%
其中 $q_1$ 项的符号约定基于 \cite[ Section 4]{KM}。
\end{Example}
...
\textbf{关于完整性的说明}：由于 $s_2s_1$ 在 $S_3$ 中的长度为 $2$，其 Schubert 多项式 $\mathfrak{S}_{s_2s_1}(x,y)$ 关于 $x$ 变量的总次数为 $2$（等于置换长度）。因此上述展开 $x_1^2 + x_1y_1 + x_1y_2 + y_1y_2$ 是完整的——没有遗漏更高次项，因为多项式的次数已由 Bruhat 序固定。
\end{Example}
...
在 Chapter 1 中，我们研究了 Gao--Xiong \cite{GX2025} 关于三重 Schubert Positivity（triple Schubert positivity）的结果。现在我们揭示量子双重 Schubert 多项式与这一结果的联系。
...
\begin{Proposition}[量子与三重的联系]
\label{prop:QuantumTripleConnection}
设 $u, v, w \in S_n$。则在等变量量子上同调 $QH^*_{U_n}(Fl_n)$ 中，Schubert 类的量子乘积展开系数 $c_{u,v}^{w}(\mathbf{y}, \beta)$ 满足：
\begin{itemize}
\item 当 $\beta = 0$（曲线类为零）时，系数退化为经典等变量子上同调的结构常数；
\item 当 $\beta \neq 0$ 时，系数记录了连接 Schubert 子簇的 $\beta$ 类曲线个数。
\end{itemize}
特别地，Gao--Xiong \cite{GX2025} 的 triple Schubert positivity 结果可以理解为等变量量子上同调在 $\beta = 0$ 时的情形——此时量子双重 Schubert 多项式给出了结构常数的代数代表元。
\end{Proposition}

\textbf{三重版本的关键洞察}：当 Gao--Xiong \cite{GX2025} 研究三重变量集 $(x, y, t)$ 时，他们实际上是在研究等变量量子上同调（equivariant quantum cohomology）的一个精化版本。\cref{prop:QuantumTripleConnection} 明确了两者的关系：Gao--Xiong \cite{GX2025} 的正性结果对应于量子配对下 $\beta = 0$ 的特殊情形，而 Kirillov-Maeno 的量子双重 Schubert 多项式则给出了完整的代数框架。
...
\item 这些结果与 Chapter 1 的 Gao--Xiong \cite{GX2025} 三重 Schubert positivity 工作有深刻的联系
\end{enumerate}
