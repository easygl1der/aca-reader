...
\subsection{Lascoux-Schützenberger 型代表元}\label{sec:LascouxSchutzenberger}
...
\subsection{ET 为什么是奇数维球面的极限}\label{sec:ETIntuition}
...
\subsection{Torus 作用与分类空间 ET}\label{sec:TorusActionET}
...
\subsection{等变量子上同调与量子同调的区别}\label{sec:EquivariantvsQuantum}
...
\subsection{Hilbert 第十五问题与 Schubert 演算}\label{sec:hilbert15}
...
\subsection{$S_\infty$ 的例子}\label{sec:SInfinityExample}
...
\subsection{量子上同调的定义}\label{sec:QuantumCohomology}
...
\subsection{Theorem 1.1 的例子：Triple Schubert Positivity}\label{sec:Theorem11Examples}
...
\subsection{Theorem 1.1 如何推出 Corollary 1.2}\label{sec:Thm1ToCor1}
...
\subsection{Knutson-Tao (2003) 展开系数的几何意义}\label{sec:KnutsonTao2003}
...
\subsection{Gao-Xiong 与 Knutson-Tao 解释的不同}\label{sec:GaoXiongVsKnutsonTao}
...
\subsection{Graham Positivity 中 Grassmannian 限制的作用}\label{sec:GrassmannianRole}
...
\subsection{Littlewood-Richardson 系数为何非负}\label{sec:LittlewoodRichardsonPositivity}
...
\subsection{什么是 Schur 函数}\label{sec:WhatIsSchurFunction}
...
\subsection{Graham Positivity 中展开系数的"非负性"含义}\label{sec:GrahamPositivityCoefficient}
...
\subsection{第一章第三节的定理脉络：三层定理与三层引理}\label{sec:ThmFlowCh1S3}
...
\subsection{几何相交如何翻译为代数展开系数}\label{sec:GeoToAlgebra}
...
\subsection{Graham 定理为何依赖几何性质}\label{sec:GrahamProofGeometry}
...
\subsection{$B^-(w)$ 与 $B^-$ 的关系}\label{sec:BMinusWVsBMinus}
...
\subsection{$\mathbb{N}[-\alpha]_{\alpha \in I(w)}$ 的含义}\label{sec:NMinusAlphaInIw}
...
\subsection{EQLR 系数的定义与意义}\label{sec:EQLRCoefficient}
