...
\subsection{为何限制到 $\mathrm{GL}_m(\mathbb{C})$}\label{sec:qa-why-glm}
...
\subsection{为何使用 $S_\infty$ 而非 $S_n$}\label{sec:qa-sinfty-vs-sn}
...
\subsection{$S_\infty$ 的定义与直观理解}\label{sec:qa-sinfty-definition}
...
\subsection{Skew 除差算子与除差算子的关系}\label{sec:qa-skew-vs-divided}
...
\subsection{逆除差算子的定义}\label{sec:qa-inverse-divided}
...
\subsection{双重与三重 Schubert 多项式的区别}\label{sec:qa-double-vs-triple}
...
\subsection{等变量子上同调类的定义}\label{sec:qa-equivariant-cohomology-class}
