...
\begin{Remark}\label{rem:ipm-manifold-adapt}
    当 $Y$ 支撑于低维流形 $\mathcal{M}_Y \subset \mathbb{R}^{D_Y}$ 上时，
    IPM $d_\gamma$ 的定义表面上仍使用全空间 $\mathbb{R}^{D_Y}$ 上的测试函数类
    $\mathcal{H}_1^\gamma(\mathbb{R}^{D_Y})$，
    但光滑性条件 $\gamma \geq 1$ 保证测试函数在远离 $\mathcal{M}_Y$ 的区域
    自然衰减或变化平缓——
    从而 IPM 的值主要由 $\mathcal{M}_Y$ 的 $d_Y$ 维邻域内的概率质量差异决定，
    环境维度 $D_Y$ 不出现在收敛速率中。
    这正是 $d_\gamma$ 能自然适应流形结构的核心机制。
\end{Remark}
...
\begin{Theorem}[流形回归的极小极大速率]\label{def:thm-manifold-regression}
...
此处采用更强的联合光滑性条件 $\mathcal{H}^{\alpha_Y,\alpha_X}$
（\cref{def:marginal-smooth}），以保证 $\alpha_Y$ 的定义在局部坐标卡变换下不变。

\begin{Definition}[Regime 3b：协变量依赖流形响应空间]\label{def:regime3b}
    给定维度 $D_Y, d_Y, D_X, d_X \in \mathbb{N}_+$、
    光滑度 $\beta_Y, \beta_X, \alpha_Y, \alpha_X > 0$、
    函数 $g: \mathbb{R}_+ \to \mathbb{R}_+$ 和常数 $\tau, \tau_1, L > 0$，
    定义分布族
    \[
        \mathcal{P}_3^* = \mathcal{P}_3^*(D_Y, D_X, d_Y, d_X,
            \beta_Y, \beta_X, \alpha_Y, \alpha_X, \tau, \tau_1, g, L)
    \]
    为满足以下条件的 $\mu^* = \mu^*_X\mu^*_{Y|X}$ 构成的集合：
    \begin{enumerate}
        \item $\mathcal{M}_X \in \mathcal{M}_X(D_X, d_X, L)$；
        \item 对任意 $x \in \mathcal{M}_X$，$\mu^*_{Y|x}$ 支撑于
              $d_Y$ 维 $\beta_Y$-光滑子流形 $\mathcal{M}_{Y|x}$，
              且族 $\{\mathcal{M}_{Y|x} : x \in \mathcal{M}_X\}$ 为
              $(\beta_Y, \beta_X)$-光滑的
              （\cref{def:covdep-manifold}）；
        \item 对任意 $x \in \mathcal{M}_X$，$\mu^*_{Y|x}$ 有密度函数
              $u^*(\cdot|x)$ 相对于 $\mathcal{M}_{Y|x}$ 的体积测度，
              且 $u^* \in \mathcal{H}_L^{\alpha_Y, \alpha_X}
                 (\mathcal{M}_{Y|x}, \mathcal{M}_X)$
              （联合光滑性）；
        \item 下界条件与 Regime 2 相同。
    \end{enumerate}
\end{Definition}
...
\begin{Remark}\label{rem:regime-comparison}
    ...
    \begin{enumerate}
        ...
        \item \textit{流形支撑效应}（Regime 依赖）：
              ...
              Regime 3b $\to n^{-\gamma/(d_Y/\beta_Y+d_X/\beta_X)}$\allowbreak。
    \end{enumerate}
...
\begin{equation}\label{eq:J-choice}
    J \;\asymp\;
    \frac{1}{2\alpha_Y + D_Y + \frac{\alpha_Y}{\alpha_X}\,d_X}
    \log_2\!\left(\frac{n}{\log n}\right).
\end{equation}

\begin{Theorem}[Regime 1 估计量的收敛速率]\label{def:thm-estimator-regime1}
    在 $\mathcal{P}_1^*$ 假设下，
    对任意 $\gamma \geq 0$，
...
\begin{Lemma}[联合 vs 独立估计的最优性]\label{def:lem-joint-vs-separate}
    ...
    则对任意固定的 $x \in \mathcal{M}_X$，
    \[
        \mathbb{E}_Y\!\left[
            \big|\widehat{u}_j^\dagger(x) - u^*(x)\big|^2
        \right]
        \;\leq\;
        \mathbb{E}_Y\!\left[
            \big|\widehat{u}_j(x) - u^*(x)\big|^2
        \right]
        \quad \text{（至多差常数因子）},
    \]
    其中 $u^*(x) \equiv u^*(\cdot|x)$ 表示在 $x$ 处取值的条件密度函数
    （为书写简便省略参数 $y$），
    期望 $\mathbb{E}_Y$ 对 $Y \sim \mu^*_{Y|x}$ 取条件期望。
    且在某些构造下严格更优。
...
\begin{Lemma}[广义 Fano 不等式]\label{def:lem-fano}
...
\begin{Lemma}[Varshamov-Gilbert 引理]\label{def:lem-vg}
...
\begin{Lemma}[热核平滑的逼近误差]\label{def:lem-heat-kernel}
...
本章系统介绍了~\citep{tang2025regression} 关于分布回归极小极大速率的理论。
