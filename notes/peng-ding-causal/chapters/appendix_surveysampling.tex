...
Let $\bm{Z} = (Z_1, \ldots, Z_n)$ be the inclusion indicators of the $n$ units with $Z_i=1$ if unit $i$ is sampled and $Z_i=0$ otherwise.
The vector $\bm{Z}$ can take $\binom{n}{n_1}$ possible permutations of a vector of $n_1$ 1's and $n_0$ 0's, and each has equal probability.
\end{definition}
...
$$
where $ \bm{1}_n$ is a $n$-dimensional vector of $1$'s, and $\bm{P}_n = \bm{I}_n  - n^{-1} \bm{1}_n \bm{1}_n\tran $ is the $n\times n$ projection matrix orthogonal to $ \bm{1}_n.$
\end{lemma}
