...
\begin{prop}[Poisson Process Distribution]
...
\begin{ythm}[Poisson Process Stationarity]
...
\begin{ythm}[Poisson Process Independent Increments]
...
\begin{prop}[Increments are Normal]
...
\begin{prop}[Independent Increments]
...
\begin{ythm}[Wiener Process Characterization]
...
\begin{ythm}[Lévy's Martingale Characterization]
...
\begin{ythm}[Doob's Maximal $L^2$ Inequality]
\label{thm:doob-maximal-l2}
For any $t > 0$
\begin{equation}
E\left(\max_{s \leq t} |W(s)|^2\right) \leq 4 E|W(t)|^2.
\tag{6.6}
\label{eq:doob-maximal}
\end{equation}
\endythm}
...
\begin{equation}
M_k^n = \left|W\left(\frac{kt}{2^n}\right)\right|, \quad 0 \leq k \leq 2^n.
\tag{6.7}
\label{eq:m-kn-definition}
\end{equation}
...
\begin{equation}
\lim_{n \to \infty} \frac{N(n)}{n} = \lambda \quad \text{a.s.}
\tag{6.8}
