...
\textbf{背景（Background）}：
Cauchy-Schwarz 不等式是概率论中最重要的不等式之一。

\textbf{目标（Goal）}：
证明 $|\mathbb{E}[XY]| \leq \sqrt{\mathbb{E}[X^2] \cdot \mathbb{E}[Y^2]}$。

\textbf{推导步骤（Derivation Steps）}：
占位符内容（待补充）。$\blacksquare$

\subsection{Jensen 不等式的证明}\label{sec:derivation-jensen}

\textbf{背景（Background）}：
Jensen 不等式是凸函数理论在概率论中的应用。

\textbf{目标（Goal）}：
若 $g$ 是凸函数，则 $\mathbb{E}[g(X)] \geq g(\mathbb{E}X)$。
...
\textbf{背景（Background）}：
Markov 不等式提供了概率的上界估计。

\textbf{目标（Goal）}：
对非负随机变量 $X$ 和 $a > 0$，证明 $\mathbb{P}(X \geq a) \leq \mathbb{E}[X]/a$。
...
\textbf{背景（Background）}：
Poisson 分布是二项分布的极限分布。

\textbf{目标（Goal）}：
证明当 $n \to \infty$, $p \to 0$, $np = \lambda$ 时，$\binom{n}{p}^k (1-p)^{n-k} \to e^{-\lambda} \lambda^k/k!$。
...
\textbf{目标（Goal）}：
证明 $\mathbb{P}(X > s + t \mid X > s) = \mathbb{P}(X > t)$。
...
\textbf{目标（Goal）}：
证明 $\mathbb{E}[g(X)] = \int g(x) f_X(x) dx$。
...
\textbf{目标（Goal）}：
证明 $\text{var}(X) = \mathbb{E}[X^2] - (\mathbb{E}[X])^2$。
