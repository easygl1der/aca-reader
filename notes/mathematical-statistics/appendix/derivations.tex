...
\subsection{两样本置信区间的详细推导}\label{sec:derivation-two-sample-ci}

\textbf{背景（Background）}：
在正文中我们给出了两样本 $t$ 检验置信区间的公式。这里给出完整推导，包括方差相等和方差不等两种情形。

\textbf{参数定义（Parameter Definitions）}：
\begin{itemize}
\item $X_1, \ldots, X_{n_1} \sim \text{i.i.d. } N(\mu_1, \sigma^2)$：第一组样本
\item $Y_1, \ldots, Y_{n_2} \sim \text{i.i.d. } N(\mu_2, \sigma^2)$：第二组样本
\item $\bar{X}$、$\bar{Y}$：两组样本均值
\item $S_1^2$、$S_2^2$：两组样本方差
\item $S_p^2$：合并方差（方差相等时使用）
\end{itemize}
...
\textbf{2. 方差不等时的置信区间（Welch 近似）}：

当 $\sigma_1^2 \neq \sigma_2^2$ 时，使用 Welch-Satterthwaite 近似：

\[
T^* = \frac{\bar{X} - \bar{Y} - (\mu_1 - \mu_2)}{\sqrt{\frac{S_1^2}{n_1} + \frac{S_2^2}{n_2}}} \xrightarrow{D} t(\nu),
\]
其中自由度为：
\[
...
\textbf{结论}：
在实际应用中，应先用 Levene 检验或 Bartlett 检验判断方差是否相等，再选择合适的置信区间公式。$\blacksquare$

\subsection{接受-拒绝算法的详细推导}\label{sec:derivation-accept-reject}

\textbf{背景（Background）}：
接受-拒绝算法（Accept-Reject Algorithm）是生成非均匀随机数的经典方法。其核心思想是：如果难以直接从目标分布 $f(x)$ 采样，但可以计算 $f(x)$（直到归一化常数），且存在一个易采样的提议分布 $g(x)$，使得 $f(x) \leqslant M g(x)$ 对所有 $x$ 成立，则可以通过接受-拒绝采样得到来自 $f(x)$ 的样本。

\textbf{参数定义（Parameter Definitions）}：
\begin{itemize}
\item $f(x)$：目标分布的密度函数（可能未归一化）
\item $g(x)$：提议分布的密度函数（已归一化且易采样）
\item $M$：接受常数，满足 $f(x) \leqslant M g(x)$，$M \geqslant 1$
\item $r = f(x) / (M g(x))$：接受概率
\end{itemize}

\textbf{算法步骤（Algorithm Steps）}：
\begin{enumerate}
\item 从 $g(x)$ 生成候选样本 $X \sim g(x)$。
\item 从 $\text{Uniform}(0,1)$ 生成 $U \sim \text{U}(0,1)$。
\item 若 $U \leqslant r = f(X) / (M g(X))$，则接受 $X$；否则拒绝。
\item 重复直到获得所需数量的接受样本。
\end{enumerate}

\textbf{理论推导（Theoretical Derivation）}：

接受-拒绝算法的核心结论是：接受样本的分布正好是 $f(x)$。

\textbf{证明}：设 $A$ 表示"接受"事件。接受样本的分布为：
\[
\mathbb{P}(X \in B \mid \text{接受}) = \frac{\mathbb{P}(X \in B, \text{接受})}{\mathbb{P}(\text{接受})}.
\]

由算法设计，$\mathbb{P}(\text{接受} \mid X = x) = r = f(x) / (M g(x))$。因此：
\[
\mathbb{P}(X \in B, \text{接受}) = \int_B g(x) \cdot \frac{f(x)}{M g(x)} dx = \frac{1}{M} \int_B f(x) dx.
\]

而 $\mathbb{P}(\text{接受}) = \int \frac{f(x)}{M} dx = \frac{1}{M}$（因为 $f$ 是归一化的）。于是：
\[
\mathbb{P}(X \in B \mid \text{接受}) = \frac{f(x)/M}{1/M} = f(x).
\]
得证。$\blacksquare$

\textbf{Cauchy 提议分布的选取理由}：

在正态分布的接受-拒绝采样中，选择 Cauchy 作为提议分布的原因是：
\begin{itemize}
\item Cauchy 分布与正态分布形态相似（都是钟形、对称）
\item Cauchy 分布的尾部更重：$f(x)/g(x) \to \text{常数}$ 当 $|x| \to \infty$
\item 这使得 $M = \sup_x f(x)/g(x)$ 有界且较小
\item 接受率约 $1/M \approx 65\%$，效率较高
\end{itemize}

\textbf{$M$ 的优化}：
对于标准正态目标 $f(x) = \phi(x)$ 和 Cauchy 提议 $g(x) = \text{Cauchy}(0, 1)$，有：
\[
\frac{f(x)}{g(x)} = \frac{\phi(x)}{\text{Cauchy}(x)} = \frac{e^{-x^2/2}}{\pi(1+x^2)} \cdot \pi \approx \frac{e^{-x^2/2}}{1+x^2}.
\]

通过数值优化，可得 $M \approx 1.52$，对应的接受率约为 $65.8\%$。$\blacksquare$

\subsection{Bootstrap 置信区间的详细推导}\label{sec:derivation-bootstrap-ci}

\textbf{背景（Background）}：
Bootstrap 方法通过重采样来估计统计量的抽样分布，而不需要解析推导。Bootstrap 置信区间有多种构造方法，本节介绍基本 Bootstrap 区间、百分位数区间和 BCa 区间。

\textbf{参数定义（Parameter Definitions）}：
\begin{itemize}
\item $\mathbf{x} = (x_1, \ldots, x_n)$：原始样本
\item $\widehat{\theta} = s(\mathbf{x})$：目标统计量（样本函数）
\item $\widehat{\theta}^*_1, \ldots, \widehat{\theta}^*_B$：$B$ 次 Bootstrap 重复计算的统计量
\item $\widehat{\theta}^*_{(1)}, \ldots, \widehat{\theta}^*_{(B)}$：Bootstrap 统计量的有序排列
\end{itemize}

\textbf{Bootstrap 分布的性质}：

Bootstrap 的理论基础是：样本的经验分布 $\widehat{F}_n$ 是真实分布 $F$ 的良好估计。因此，
\[
\widehat{\theta}^* - \widehat{\theta} \approx \theta^* - \theta,
\]
其中 $\theta^*$ 是从真实分布 $F$ 抽取的样本计算的统计量。

\textbf{三种置信区间构造方法}：

\textbf{1. 基本 Bootstrap 区间（Basic Bootstrap CI）}：

百分位数法：
\[
\left( \widehat{\theta}^*_{(\alpha B)}, \widehat{\theta}^*_{((1-\alpha)B)} \right).
\]

基本区间（偏差修正）：
\[
\left( 2\widehat{\theta} - \widehat{\theta}^*_{((1-\alpha)B)}, 2\widehat{\theta} - \widehat{\theta}^*_{(\alpha B)} \right).
\]

\textbf{2. 百分位数区间（Percentile CI）}：

直接使用 Bootstrap 分布的百分位数：
\[
\left( \widehat{\theta}^*_{(\alpha B/2)}, \widehat{\theta}^*_{((1-\alpha/2)B)} \right).
\]

\textbf{3. BCa 区间（Bias-Corrected and Accelerated）}：

BCa 区间通过两项修正：
\[
\alpha_1 = \Phi\left( \widehat{z}_0 + \frac{\widehat{z}_0 + z^{(\alpha)}}{1 - \widehat{a}(\widehat{z}_0 + z^{(\alpha)})} \right),
\]
其中：
\begin{itemize}
\item $\widehat{z}_0$：偏差修正因子（bias correction）
\item $\widehat{a}$：加速度因子（acceleration），反映统计量方差对参数值的依赖
\end{itemize}

\textbf{比较}：
\begin{itemize}
\item 百分位数区间：适用于对称分布，当 Bootstrap 分布偏离正态时可能有偏差
\item BCa 区间：通过偏差修正，在大多数情况下更准确
\item 基本 Bootstrap 区间：适用于需要对称性保证的情形
\end{itemize}

\textbf{结论}：
在实际中，BCa 区间通常是最准确的选择，特别是当 Bootstrap 分布偏离正态时。$\blacksquare$

\subsection{Bootstrap 检验的详细推导}\label{sec:derivation-bootstrap-test}

\textbf{背景（Background）}：
Bootstrap 不仅可以构造置信区间，还可以用于假设检验。当检验统计量的原假设分布难以解析计算时，Bootstrap 提供了有效的近似方法。

\textbf{参数定义（Parameter Definitions）}：
\begin{itemize}
\item $H_0$：原假设
\item $T(\mathbf{x})$：检验统计量
\item $t_{\text{obs}}$：观测数据计算的统计量值
\item $\widehat{F}_n$：样本的经验分布函数
\item $T^*_1, \ldots, T^*_B$：$B$ 次 Bootstrap 重复的检验统计量
\end{itemize}

\textbf{Bootstrap 检验的原理}：

Bootstrap p 值的计算：
\[
 p = \mathbb{P}_{H_0}(T \geqslant t_{\text{obs}}) \approx \frac{1}{B} \sum_{b=1}^B \mathbb{I}(T^*_b \geqslant t_{\text{obs}}).
\]
...
\textbf{1. 参数化 Bootstrap（参数已知）}：

如果原假设完全指定了分布（如 $H_0: \theta = \theta_0$），则：
\begin{enumerate}
\item 从 $F(\cdot; \theta_0)$ 生成 Bootstrap 样本
\item 计算 $T^*_b$
\item 比较 $t_{\text{obs}}$ 与 $T^*_b$ 的分布
\end{enumerate}

\textbf{2. 非参数化 Bootstrap（分布自由）}：

如果原假设只涉及部分参数（如 $H_0: \mu_1 = \mu_2$），需要：
\begin{enumerate}
\item 对数据进行中心化/变换，使其符合原假设
\item 从变换后的经验分布采样
\item 计算 Bootstrap 统计量
\end{enumerate}

\textbf{功效分析（Power Analysis）}：

Bootstrap 也可用于功效分析：
\[
\text{power} \approx \mathbb{P}\left( \text{拒绝 } H_0 \mid H_1 \text{ 为真} \right).
\]

通过模拟不同参数值下的 Bootstrap 检验，可以估计检验的族错误率（FWER）或发现率（FDR）。

\textbf{p 值修正}：

当进行多重检验时，Bootstrap p 值需要修正：
\[
p_i^{\text{adj}} = \min\left( 1, \frac{p_i \cdot m}{\text{rank}(p_i)} \right),
\]
其中 $m$ 是检验总数。

\textbf{结论}：
Bootstrap 检验的核心在于正确模拟原假设下的分布。在实践中，应根据具体问题选择合适的重采样方案。$\blacksquare$
