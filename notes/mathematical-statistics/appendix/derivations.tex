...
\subsection{弱大数定律的完整推导}\label{sec:wlln-proof}

\textbf{背景（Background）}：
弱大数定律（Weak Law of Large Numbers, WLLN）表明，当样本量趋于无穷时，样本均值依概率收敛到总体均值。

\textbf{目标（Goal）}：
证明对于独立同分布的随机变量 $X_1, X_2, \ldots$，若 $\mathbb{E}[X_i] = \mu$ 且 $\text{var}(X_i) = \sigma^2 < \infty$，则 $\overline{X}_n \xrightarrow{P} \mu$。

\textbf{推导步骤（Derivation Steps）}：
占位符内容（待补充）。

\subsection{乘积收敛性的完整证明}\label{sec:convergence-product-proof}

\textbf{背景（Background）}：
若 $X_n \xrightarrow{P} X$ 且 $Y_n \xrightarrow{P} Y$，在什么条件下有 $X_n Y_n \xrightarrow{P} XY$？
...
\subsection{样本方差一致性的完整推导}\label{sec:sample-variance-consistency}

\textbf{背景（Background）}：
样本方差 $S_n^2$ 是总体方差 $\sigma^2$ 的一致估计。
...
\subsection{均匀分布最大值的详细分析}\label{sec:uniform-max-derivation}

\textbf{背景（Background）}：
对于 $U(0, \theta)$ 分布，样本最大值 $Y_n$ 的分布分析。

\textbf{目标（Goal）}：
详细推导 $Y_n$ 的期望、方差及其一致性。

\textbf{推导步骤（Derivation Steps）}：
占位符内容（待补充）。

\subsection{退化分布的详细分析}\label{sec:degenerate-distribution}

\textbf{背景（Background）}：
退化分布是所有概率质量集中于一点的分布。

\textbf{目标（Goal）}：
分析在什么情况下 $\overline{X}_n$ 收敛到退化分布。
...
\textbf{背景（Background）}：
Stirling 公式用于近似阶乘：$n! \approx \sqrt{2\pi n} (n/e)^n$。
...
\subsection{均匀分布到指数分布的完整推导}\label{sec:uniform-to-exponential}

\textbf{背景（Background）}：
标准化后的均匀分布最大值收敛到指数分布。

\textbf{目标（Goal）}：
证明 $n(\max\{X_1, \ldots, X_n\} - 1) \xrightarrow{D} \text{Exp}(1)$。

\textbf{推导步骤（Derivation Steps）}：
占位符内容（待补充）。

\subsection{依概率收敛到依分布收敛的完整证明}\label{sec:prob-to-dist-proof}

\textbf{背景（Background）}：
依概率收敛是依分布收敛的充分条件，但非必要条件。

\textbf{目标（Goal）}：
证明当 $X_n \xrightarrow{P} c$（常数）时，有 $X_n \xrightarrow{D} c$。
...
\textbf{背景（Background）}：
Slutsky 定理允许在依分布收敛的极限中替换依概率收敛的随机变量。
...
\subsection{乘积收敛到依概率收敛的完整证明}\label{sec:product-convergence-proof}

\textbf{背景（Background）}：
若 $X_n \xrightarrow{P} X$ 且 $Y_n \xrightarrow{P} 0$，则 $X_n Y_n \xrightarrow{P} 0$。
...
\textbf{目标（Goal）}：
证明若 $\sqrt{n}(X_n - \mu) \xrightarrow{D} N(0, \sigma^2)$，则 $\sqrt{n}(g(X_n) - g(\mu)) \xrightarrow{D} N(0, \sigma^2 [g'(\mu)]^2)$。
...
\textbf{背景（Background）}：
将 $\Delta$ 方法应用于卡方变量的标准差。

\textbf{目标（Goal）}：
求 $\sqrt{n}(\sqrt{V_n} - \sqrt{v}) \的渐近分布$。

\textbf{推导步骤（Derivation Steps）}：
占位符内容（待补充）。

\subsection{二项分布到泊松分布近似的误差分析}\label{sec:binomial-poisson-approximation}

\textbf{背景（Background）}：
当 $n$ 增大、$p$ 减小时，二项分布可用泊松分布近似。
...
\subsection{中心极限定理的完整证明}\label{sec:clt-proof}

\textbf{背景（Background）}：
中心极限定理（CLT）是统计学中最重要的定理之一。

\textbf{目标（Goal）}：
用矩母函数方法证明独立同分布随机变量的 CLT。

\textbf{推导步骤（Derivation Steps）}：
占位符内容（待补充）。

\subsection{均匀分布样本均值概率的详细计算}\label{sec:uniform-mean-probability}

\textbf{背景（Background）}：
$U(0,1)$ 样本均值 $\overline{X}$ 的分布不是正态，但可用 CLT 近似。

\textbf{目标（Goal）}：
计算 $\mathbb{P}(\overline{X} \leq 0.5)$ 的精确值和近似值。
...
\subsection{方差稳定化变换的详细讨论}\label{sec:variance-stabilizing}

\textbf{背景（Background）}：
方差稳定化变换使得变换后的变量方差近似常数。
...
\subsection{多元中心极限定理的完整证明}\label{sec:multivariate-clt-proof}
...
\subsection{Beta 分布的均值与方差推导}\label{sec:derivation-beta-moments}

\textbf{背景（Background）}：
在正文中我们给出 Beta 分布的均值为 $\mathbb{E}X = \frac{\alpha}{\alpha+\beta}$，方差为 $\text{var}(X) = \frac{\alpha\beta}{(\alpha+\beta)^2(\alpha+\beta+1)}$。这里给出完整推导。

\textbf{目标（Goal）}：
基于 Beta 分布的 pdf 推导其均值和方差。

\textbf{推导步骤（Derivation Steps）}：
占位符内容（待补充）。

\subsection{正态分布的矩母函数推导}\label{sec:derivation-normal-mgf}

\textbf{背景（Background）}：
在正文中我们给出正态分布 $N(\mu, \sigma^2)$ 的矩母函数为 $M(t) = \exp\{\mu t + \frac{1}{2}\sigma^2 t^2\}$。这里给出完整推导。
...
\subsection{t 分布的推导}\label{sec:derivation-t-distribution}

\textbf{背景（Background）}：
在正文中我们给出 $t$ 分布的 pdf。这里基于正态变量和卡方变量的比值给出完整推导。
...
\subsection{F 分布的推导}\label{sec:derivation-f-distribution}

\textbf{背景（Background）}：
在正文中我们给出 $F$ 分布的 pdf。这里基于两个独立卡方变量的比值给出完整推导。
...
\subsection{F 分布的均值与方差推导}\label{sec:derivation-f-moments}

\textbf{背景（Background）}：
在正文中我们给出 $F$ 分布的均值为 $\frac{n}{n-2}$（当 $n>2$），方差为 $\frac{2n^2(m+n-2)}{m(n-2)^2(n-4)}$（当 $n>4$）。这里给出完整推导。
...
\subsection{Poisson 分布的均值与方差推导}\label{sec:derivation-poisson-moments}

\textbf{背景（Background）}：
在正文中我们给出 Poisson 分布的均值为 $\lambda$，方差也为 $\lambda$。这里给出完整推导。

\textbf{目标（Goal）}：
基于 Poisson 分布的 pmf 推导其均值和方差。

\textbf{推导步骤（Derivation Steps）}：
占位符内容（待补充）。

\subsection{正态变量线性组合的推导}\label{sec:derivation-normal-linear-combination}

\textbf{背景（Background）}：
在正文中我们给出若 $X_1 \sim N(\mu_1, \sigma_1^2)$ 和 $X_2 \sim N(\mu_2, \sigma_2^2)$ 独立，则 $aX_1 + bX_2 \sim N(a\mu_1 + b\mu_2, a^2\sigma_1^2 + b^2\sigma_2^2)$。这里给出完整证明。
...
\textbf{背景（Background）}：
在连续情形，条件概率 $\mathbb{P}(X_2 \in B \mid X_1 = x_1)$ 需要用条件密度来定义。

\textbf{目标（Goal）}：
解释条件密度的数学基础——Radon-Nikodym 定理。

\textbf{推导步骤（Derivation Steps）}：
详细推导见 chapter2.tex 第 \ref{sec:derivation-ch2-conditional-density} 节。

\subsection{Rao-Blackwell 定理}\label{sec:derivation-ch2-rao-blackwell}

\textbf{背景（Background）}：
Rao-Blackwell 定理是统计推断中的核心结果，它告诉我们如何利用条件期望改进估计。

\textbf{目标（Goal）}：
证明若 $T$ 是充分统计量的函数，则 $\mathbb{E}(T \mid S)$ 比 $T$ 更精确。

\textbf{推导步骤（Derivation Steps）}：
详细推导见 chapter2.tex 第 \ref{sec:derivation-ch2-rao-blackwell} 节。
...
\textbf{推导步骤（Derivation Steps）}：
详细推导见 chapter2.tex 第 \ref{sec:derivation-ch2-bayes-theorem} 节。
...
\textbf{推导步骤（Derivation Steps）}：
详细讨论见 chapter2.tex 第 \ref{sec:derivation-ch2-bayes-history} 节。
...
\textbf{推导步骤（Derivation Steps）}：
详细反例见 chapter2.tex 第 \ref{sec:derivation-ch2-independent-implies-uncorrelated} 节。

\subsection{回归理论的简要推导}\label{sec:derivation-ch2-regression}

\textbf{背景（Background）}：
回归理论建立了条件均值与线性预测之间的联系。

\textbf{目标（Goal）}：
推导最小二乘估计与相关系数的关系。

\textbf{推导步骤（Derivation Steps）}：
详细推导见 chapter2.tex 第 \ref{sec:derivation-ch2-regression} 节。

\subsection{两两独立不推出相互独立反例}\label{sec:derivation-ch2-pairwise-not-mutual}

\textbf{背景（Background）}：
两两独立是 mutual independence 的必要条件，但不充分。

\textbf{目标（Goal）}：
构造两两独立但不相互独立的反例（Bernstein's example）。

\textbf{推导步骤（Derivation Steps）}：
详细反例见 chapter2.tex 第 \ref{sec:derivation-ch2-pairwise-not-mutual} 节。

\subsection{中心极限定理概述}\label{sec:derivation-ch2-clt}

\textbf{背景（Background）}：
中心极限定理（CLT）是概率论和统计学中最重要的结果之一。

\textbf{目标（Goal）}：
概述 CLT 的内容和意义。

\textbf{推导步骤（Derivation Steps）}：
详细讨论见 chapter2.tex 第 \ref{sec:derivation-ch2-clt} 节。
