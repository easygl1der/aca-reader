...
\textbf{为什么即使 LRT 不是最优的，我们仍然使用它？} 历史上，Neyman 和 Pearson 引入 LRT 的动机正是为了突破最佳检验只存在于简单假设的限制。LRT 继承了 Neyman-Pearson 定理的似然比思想（用似然比衡量数据对假设的支持程度），但将比较扩展到了复合假设。虽然在一般情况下 LRT 不能保证最优性，但它是渐近最优的——在样本量趋向无穷时，LRT 具有许多优良性质。
\subsection{LRT 的定义}
