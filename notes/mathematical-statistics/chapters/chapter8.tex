...
\[
3 \cdot \mathbb{P}_{\theta=75}(\bar{X} \geq c) = P_{\theta=78}(\bar{X} < c).
...
3. \textbf{无偏性（保证 level 不超过 $\alpha$）}：设 $\gamma_Y(\theta) = \mathbb{P}_\theta(Y \geq c_Y)$ 为功效函数。对 $\theta_1 < \theta_2$，由第 1 步，$C$ 是 $\theta_1$ 对 $\theta_2$ 的最佳检验，故 $\gamma_Y(\theta_2) \geq \gamma_Y(\theta_1)$。因此 $\gamma_Y(\theta)$ 是 $\theta$ 的非降函数，$\max_{\theta \leq \theta'} \gamma_Y(\theta) = \gamma_Y(\theta') = \alpha$。故该检验是 level $\alpha$ 的。
