...
\textbf{充分性}：值得注意的是，定理的证明表明，如果存在一个大小为 $\alpha$ 的最佳检验，那么它的形式必然是上述似然比检验。而且，这个最佳检验（如果存在）是唯一的（在连续型情况下，除了一个零概率集；在离散型中可能存在多个）。
...
\textbf{关键洞察}：最佳检验包含了"最支持 $H_1$ 而最不支持 $H_0$"的数据点——这正是似然比方法告诉我们的。\footnote{二项分布最佳检验的详细分析见附录 \cref{sec:binomial-ump-analysis}。}
\end{example}
...
这符合无偏性要求（功效大于显著性水平）。\footnote{Poisson vs Geometric 检验的详细分析见附录 \cref{sec:poisson-geometric-ump}。}
\end{example}
...
推论 8.1.1 表明 Neyman-Pearson 最佳检验是无偏的——在 $H_0$ 为假时拒绝它的概率不低于在 $H_0$ 为真时错误拒绝它的概率。\footnote{推论 8.1.1 的证明及无偏性的讨论见附录 \cref{sec:corollary-np-unbiased}。}
...
即当 $\sum x_i^2 \geq 74.988$ 时拒绝 $H_0: \theta = 3$。\footnote{正态方差 UMP 检验的详细推导见附录 \cref{sec:normal-variance-ump}。}
\end{example}
...
这正是我们直观上期望的：观察到更多的成功次数，就越有理由拒绝"成功率不高"的原假设。\footnote{Bernoulli UMP 检验的 MLR 证明见附录 \cref{sec:bernoulli-ump-mlr}。}
\end{example}
...
\]
且 $p(\theta)$ 是 $\theta$ 的递增函数，则对假设 $H_0: \theta \leq \theta'$ 对 $H_1: \theta > \theta'$，UMP 检验存在，基于统计量 $Y = \sum_{i=1}^n K(X_i)$。\footnote{正规指数族的 UMP 检验存在性讨论见附录 \cref{sec:exponential-family-ump}。}
...
即约 $29.5\%$ 的概率检测出均值为 5 的差异。\footnote{非中心 $t$ 分布的功效计算见附录 \cref{sec:nct-power-derivation}。}

\textbf{稳健性注记}：$t$ 检验在正态性假设不满足时表现如何？在大样本下，由 Slutsky 定理，即使底层分布非正态，只要方差有限，$t$ 统计量仍然渐近正态——因此 $t$ 检验是\textbf{渐近正确的}。然而，在有限样本且严重非正态（特别是高度偏态）时，$t$ 检验可能失效——实际显著性水平会偏离名义水平。\footnote{非正态情况下 $t$ 检验的稳健性讨论见附录 \cref{sec:t-robustness}。}
...
\textbf{序贯抽样的图示}：想象一条随机游走——每观察一次 $X_i = 1$，我们就向右跳 $1$ 个单位；每观察一次 $X_i = 0$，我们就向左跳 $1$ 个单位。只要这条路径处于上下边界之间，我们就继续。一旦路径碰到上界（强烈支持 $H_1$）或下界（强烈支持 $H_0$），我们就停止并做出相应决策。\footnote{Bernoulli SPRT 的详细分析见附录 \cref{sec:bernoulli-sprt-analysis}。}
\end{example}
...
决策规则：在第 $n$ 步，当 $\sum x_i \geq c_1(n)$ 时拒绝 $H_0$（认为 $\theta = 78$）；当 $\sum x_i \leq c_0(n)$ 时接受 $H_0$（认为 $\theta = 75$）；否则继续采样。\footnote{正态 SPRT 的完整推导见附录 \cref{sec:normal-sprt-derivation}。}
\end{example}
...
考虑简单假设 $H_0: \theta = \theta'$ 对 $H_1: \theta = \theta''$，损失函数满足 $\mathcal{L}(\theta', \theta') = \mathcal{L}(\theta'', \theta'') = 0$（正确决策无损失），而 $\mathcal{L}(\theta', \theta'') > 0$ 和 $\mathcal{L}(\theta'', \theta') > 0$（错误决策有损失）。\footnote{极小化极大检验的决策论框架见附录 \cref{sec:minimax-decision-framework}。}
...
\]
时（即两类错误造成的风险相等），临界区域 $C$ 是极小化极大的。这个条件与 NP 似然比条件是一致的——本质上，最佳检验同时也是极小化极大检验（当两类错误的相对代价被适当选择时）。\footnote{极小化极大检验的完整证明见附录 \cref{sec:minimax-proof-derivation}。}
...
求解得 $c \approx 76.8$。此时显著性水平 $\alpha = 1 - \Phi(1.8) \approx 0.036$，功效为 $1 - \Phi(-1.2) \approx 0.885$。\footnote{极小化极大检验的数值求解见附录 \cref{sec:minimax-numerical}。}
\end{example}
...
\textbf{Fisher 线性判别函数}：在正态分布的特殊情况下，似然比不等式化为一个线性函数 $ax + by \leq c$——这就是著名的 Fisher 线性判别函数。当参数未知时，用训练样本的均值和方差代替，就得到了 Fisher 判别函数。\footnote{Fisher 线性判别函数的完整推导见附录 \cref{sec:fisher-discriminant-derivation}。}
...
\]
其中 $a$ 和 $b$ 是由两个总体的均值向量和协方差矩阵决定的常数。这给出了一个\textbf{线性决策边界}——在二维空间中是一条直线，将平面分为两部分，分别对应两类。\footnote{二元正态线性判别的详细推导见附录 \cref{sec:bivariate-normal-discriminant}。}
\end{example}
...
\subsection{二项分布最佳检验的详细分析}\label{sec:binomial-ump-analysis}
\textbf{背景}：补充正文省略的细节推导。

\subsection{Poisson vs Geometric 检验的详细分析}\label{sec:poisson-geometric-ump}
\textbf{背景}：补充正文省略的细节推导。

\subsection{推论 8.1.1 的证明及无偏性讨论}\label{sec:corollary-np-unbiased}
\textbf{背景}：证明正文省略的推论 8.1.1 及无偏性。

\subsection{正态方差 UMP 检验的详细推导}\label{sec:normal-variance-ump}
\textbf{背景}：补充正文省略的正态方差UMP检验推导。

\subsection{Bernoulli UMP 检验的 MLR 证明}\label{sec:bernoulli-ump-mlr}
\textbf{背景}：补充正文省略的 Bernoulli UMP 检验推导。

\subsection{正规指数族的 UMP 检验存在性讨论}\label{sec:exponential-family-ump}
\textbf{背景}：补充正文省略的指数族 UMP 存在性讨论。

\subsection{LRT 与 Neyman-Pearson 引理的关系讨论}\label{sec:lrt-np-relationship}
\textbf{背景}：说明 LRT 与 NP 定理的关系。

\subsection{两样本 $t$ 检验的完整推导}\label{sec:two-sample-t-derivation}
\textbf{背景}：补充正文省略的两样本 $t$ 检验推导。

\subsection{非中心 $t$ 分布的功效计算}\label{sec:nct-power-derivation}
\textbf{背景}：补充正文省略的非中心 $t$ 功效计算。

\subsection{$t$ 检验稳健性讨论}\label{sec:t-robustness}
\textbf{背景}：讨论 $t$ 检验在非正态情况下的表现。

\subsection{$F$ 检验稳健性问题讨论}\label{sec:f-test-robustness}
\textbf{背景}：讨论 $F$ 检验对正态性假设的敏感性。

\subsection{Bernoulli SPRT 的详细分析}\label{sec:bernoulli-sprt-analysis}
\textbf{背景}：补充正文省略的 Bernoulli SPRT 推导。

\subsection{正态 SPRT 的完整推导}\label{sec:normal-sprt-derivation}
\textbf{背景}：补充正文省略的正态 SPRT 推导。

\subsection{极小化极大检验的决策论框架}\label{sec:minimax-decision-framework}
\textbf{背景}：从决策论角度阐述极小化极大原理。

\subsection{极小化极大检验的完整证明}\label{sec:minimax-proof-derivation}
\textbf{背景}：补充正文省略的极小化极大检验证明。

\subsection{极小化极大检验的数值求解}\label{sec:minimax-numerical}
\textbf{背景}：Newton 法求解极小化极大检验临界值。

\subsection{Fisher 线性判别函数的完整推导}\label{sec:fisher-discriminant-derivation}
\textbf{背景}：从似然比角度推导 Fisher 判别函数。

\subsection{二元正态线性判别的详细推导}\label{sec:bivariate-normal-discriminant}
\textbf{背景}：补充正文省略的二元正态线性判别推导。

\section*{Exercises}\label{sec:exercises-ch8}

\subsection*{Section 8.1}

\begin{exercise}{\ref{exr:8.1.1} Best Test for Normal Mean (θ'' = −1) – Hogg 8.1.1}\label{exr:8.1.1}
...
\[
P_{H_0}[(X_1, X_2, \ldots, X_n) \in C] = P_{H_0}(\bar{X} \geq c) = 0.05
\]
and
\[
P_{H_1}[(X_1, X_2, \ldots, X_n) \in C] = P_{H_1}(\bar{X} \geq c) = 0.90,
\]
approximately.
\end{exercise}

\begin{exercise}{\ref{exr:8.1.8} Best Test for Beta(θ, θ) Distribution – Hogg 8.1.8}\label{exr:8.1.8}
...
Let $X_1, X_2, \ldots, X_n$ be iid with pmf $f(x; p) = p^x(1-p)^{1-x}$, $x = 0, 1$, zero elsewhere. Show that $C = \{(x_1, \ldots, x_n): \sum_{1}^n x_i \leq c\}$ is a best critical region for testing $H_0: p = 1/2$ against $H_1: p = 1/3$. Use the Central Limit Theorem to find $n$ and $c$ so that approximately $P_{H_0}(\sum_{1}^n X_i \leq c) = 0.10$ and $P_{H_1}(\sum_{1}^n X_i \leq c) = 0.80$.
\end{exercise}
...
\item[(b)] As discussed in Example 8.3.3, compute the two-sample, two-sided $t$-test that there is no difference in the true mean temperatures between genders. Obtain the $p$-value of the test and conclude in terms of the problem at the nominal $\alpha$-level of 0.05.
...
\item[(b)] With $k = 1$, compute $P(aX + bY \leq c; \mu_1' = \mu_2' = 0)$ and $P(aX + bY > c; \mu_1'' = \mu_2'' = 1)$.
\end{enumerate}
\end{exercise}
