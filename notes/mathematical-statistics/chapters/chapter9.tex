...
假设 $X_1, \ldots, X_n$ 是独立随机变量，有公共均值 $\mu$ 但有不等的方差 $\sigma_i^2 = \operatorname{Var}(X_i)$。
...
对 $M''(t)$ 在 $t=0$ 求值，可得 $\text{var}(Y) = 2(r+2\theta)$。\footnote{该推导的详细计算见附录 \cref{sec:noncentral-chisq-variance}。}
