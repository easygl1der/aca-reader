...
这个定理表明：随着样本量增大，似然函数在真值处达到全局最大值的概率趋于1。\footnote{该定理的完整证明见附录 \cref{sec:proof-likelihood-asymptotic}。}
...
\textbf{注}：在例 6.1.1 中我们看到，对于 Laplace 分布，MLE 是样本中位数。这说明 MLE 的形式完全由分布的尾部行为决定。
\end{example}
...
我们已经知道 MLE 是一致的。但这只告诉我们估计量在在大样本下趋于真值，却没有告诉我们\textbf{收敛的速度有多快}。
...
直观上理解：若 $\theta$ 的微小变化会导致密度函数较大的变化，则我们对 $\theta$ 的了解就更多，信息量就更大。
