...
设 $\theta_0$ 是参数真值，在正则条件 (R0)-(R2) 下，\footnote{正则条件 (R0) 确保密度可微；(R1) 确保信息矩阵存在且正定；(R2) 确保渐近正态性。具体定义见附录 \cref{sec:qa-regularity-conditions}。}
\[
...
\textbf{定理 6.1.2 的 Invariance 性质}：若 $\widehat{\theta}$ 是 $\theta$ 的 MLE，则对任意函数 $g(\theta)$，$g(\widehat{\theta})$ 是 $g(\theta)$ 的 MLE。\footnote{Laplace 分布 MLE 的完整推导见附录 \cref{sec:mle-asymptotic-proof}。}
...
因此 $\widehat{\theta}$ 不是有效的，但修正偏后的估计量 $[(n-1)/n]\widehat{\theta}$ 是渐近有效的。\footnote{Beta 分布 $\widehat{\theta} = \bar{X}$ 的方差为 $\theta^2 n^2/[(n-1)^2(n-2)]$，Rao-Cramér 下界为 $\theta^2/n$。Beta 分布 MLE 方差的推导见附录 \cref{sec:mle-asymptotic-proof}。}
\end{example}
...
\textbf{注意}：当 $H_0$ 为真时，$2n\bar{X}/\theta_0 \sim \chi^2(2n)$。因此临界值可从 $\chi^2$ 分布获得。\footnote{指数分布似然比检验的详细推导见附录 \cref{sec:mle-asymptotic-proof}。}
\end{example}
