...
这个方程称为\textbf{似然方程（score equation）}。\footnote{通用推导框架见附录 。}
...
令导数为零，解得 $\widehat{\theta} = \bar{x}$。由于 $\mathbb{E}(X) = \theta$，故 $\widehat{\theta} = \bar{X}$ 也是无偏估计。\footnote{详细推导见附录 。}
\end{example}
...
对数似然求导并令为零，解得 $\widehat{\theta} = \bar{x}$。由于 $\mathbb{E}(X) = \theta$，故 $\widehat{\theta} = \bar{X}$ 是无偏的。\footnote{详细推导见附录 。}
\end{example}
...
\]
而样本方差 $S^2 = \frac{1}{n-1}\sum_{i=1}^n (X_i - \bar{X})^2$ 是无偏的（$\mathbb{E}(S^2) = \sigma^2$）。这说明 MLE 有时并非无偏——无偏性和最大似然是两种不同的估计原则。\footnote{详细推导见附录 。}
\end{example}
...
\textbf{构造置信区间的关键}是找到枢轴量（pivot）——一个其分布不依赖于未知参数的函数。\footnote{枢轴量的构造方法及推导见附录 。}
...
\textbf{直觉理解}：区间宽度 $\propto \sigma/\sqrt{n}$——样本量越大，估计越精确。这与大数定律的直觉一致：更多信息带来更精确的估计。\footnote{详细推导见附录 。}
\end{example}
...
\textbf{用 $t$ 分布替代正态分布}：当方差未知时，我们用样本标准差 $S$ 替代总体标准差 $\sigma$。这引入了额外的随机性，导致分布从正态变为 $t$ 分布。$t$ 分布比正态分布更"重尾"，反映了未知方差带来的不确定性。\footnote{详细推导见附录 。}
\end{example}
...
其中 $\chi^2_{p, k}$ 是 $\chi^2(k)$ 分布的上侧 $p$ 分位数。\footnote{详细推导见附录 。}
\end{theorem}
...
在 $\alpha$ 水平下，当 $Z > z_\alpha$ 时拒绝 $H_0$。\footnote{详细推导见附录 。}
\end{example}
...
在 $\alpha$ 水平下，当 $|T| > t_{\alpha/2,n-1}$ 时拒绝 $H_0$。\footnote{详细推导见附录 。}
\end{example}
...
\textbf{直观理解}：$(O_i - E_i)^2/E_i$ 衡量每个类别的偏差，平方消去正负号，除以 $E_i$ 进行标准化，求和得到总体偏差。\footnote{详细推导见附录 。}
...
接受概率为 $1/M$。\footnote{接受-拒绝算法的推导及收敛性证明见附录 。}
...
\textbf{直觉}：如果样本是总体的良好代表，那么从样本有放回抽样就是在模拟"从总体中抽取样本"的过程。$T^*$ 的分布反映了 $T$ 的抽样分布的近似。\footnote{Bootstrap 的理论基础及更多细节见附录 。}
