...
风险函数（risk function）定义为损失的期望：$R(\theta, \delta) = \mathbb{E}[\mathcal{L}(\theta, \delta(Y))]$。一个自然的想法是寻找使风险函数最小化的决策函数。但在一般情况下，使 $R(\theta, \delta)$ 对所有 $\theta$ 同时最小的估计量不存在（见下例）。
...
\[
\mathbb{E}[\tilde{\delta}] = E\left[\frac{Y}{n}\left(1 - \frac{Y}{n}\right)\right] = \frac{n-1}{n}\theta(1-\theta).
\]
...
计算条件期望：
\[
\mathbb{E}[Z \mid \overline{X} = \overline{x}] = P(X_1 \leq c \mid \overline{X} = \overline{x}) = \Phi\left[\frac{\sqrt{n}(c - \overline{x})}{\sqrt{n-1}}\right].
\]
...
Hint: Can you find directly a function $\psi(\overline{X})$ of $\overline{X}$ such that $\mathbb{E}[\psi(\overline{X})] = \theta$ ? Is $E(X_1|\overline{x}) = \psi(\overline{x})$ ? Why?
\end{exercise}
...
In the preceding exercise, given that $E(Y) = \mathbb{E}[K(X)] = \theta$ , prove that $Y$ is $N(\theta, 1)$ .
