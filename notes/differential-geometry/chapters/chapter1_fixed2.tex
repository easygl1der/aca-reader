...
\end{solution}
egin{exercise}{1-3, 8* — do Carmo, Exercise 1-3, 8}
...
\begin{solution}
由挠率的标准公式 $\tau = \frac{\det(\alpha', \alpha'', \alpha''')}{k^2}$，利用叉积的混合积性质 $\det(\alpha', \alpha'', \alpha''') = (\alpha'\times\alpha'')\cdot\alpha'''$，以及 $k^2 = |\alpha'\times\alpha''|^2$（当 $|\alpha'|=1$ 时），整理可得
\[
\tau(s) = -\frac{(\alpha'(s)\times\alpha''(s))\cdot\alpha'''(s)}{|k(s)|^2}.
\]
$\square$
\end{solution}
egin{exercise}{1-5, 3 — do Carmo, Exercise 1-5, 3}
...
\begin{solution}
\textbf{直观理解}：渐伸线（evolute）的切线恰好是原曲线的法线。

\textbf{解答}：

\textbf{（a）}：设 $\beta(t) = \alpha(t) + \frac{1}{k(t)}n(t)$ 为 $\alpha$ 的渐伸线。计算
\[
\beta'(t) = \alpha'(t) + \left(\frac{1}{k(t)}n(t)\right)' = t + \left(-\frac{k'}{k^2}n + \frac{1}{k}(-kt + \tau b)\right) = -\frac{k'}{k^2}n + \frac{\tau}{k}b.
\]
由于 $t$ 与 $n,b$ 正交，$\beta'(t)$ 与 $t$ 垂直，即 $\beta$ 的切线平行于 $\alpha$ 的法线。

\textbf{（b）}：当 $t_2 \to t_1$ 时，过 $\alpha(t_1)$ 和 $\alpha(t_2)$ 的法线交点趋近于 $\beta(t_1)$，即渐伸线上的点。$\square$
\end{solution}
egin{exercise}{1-5, 8 — do Carmo, Exercise 1-5, 8}
...
\begin{solution}
\textbf{直观理解}：悬链线 $\alpha(t) = (t, \cosh t)$ 的曲率随 $|t|$ 增大而减小。

\textbf{解答}：

\textbf{（a）}：对 $\alpha(t) = (t, \cosh t)$，有 $\alpha'(t) = (1, \sinh t)$，$\alpha''(t) = (0, \cosh t)$。代入平面曲线曲率公式
\[
k(t) = \frac{x'y'' - x''y'}{((x')^2 + (y')^2)^{3/2}} = \frac{\cosh t}{(\cosh^2 t)^{3/2}} = \frac{1}{\cosh^2 t}.
\]

\textbf{（b）}：渐伸线 $\beta(t) = \alpha(t) + \frac{1}{k(t)}n(t)$。由 $t = (1, \sinh t)/\sqrt{1+\sinh^2 t}$，取单位法向量 $n = (-\tanh t, \sech t)$，得
\[
\beta(t) = (t, \cosh t) + \cosh^2 t \cdot (-\tanh t, \sech t) = (t - \sinh t\cosh t, 2\cosh t).
\]
$\square$
\end{solution}
egin{exercise}{1-5, 9 — do Carmo, Exercise 1-5, 9}
...
\begin{solution}
\textbf{直观理解}：极坐标曲线可写为参数式 $(\rho\cos\theta, \rho\sin\theta)$，弧长和曲率由参数化直接计算。

\textbf{解答}：

\textbf{（a）}：极坐标曲线 $\alpha(\theta) = (\rho\cos\theta, \rho\sin\theta)$，则 $|\alpha'(\theta)|^2 = \rho^2 + (\rho')^2$，故弧长 $s = \int_a^b \sqrt{\rho^2 + (\rho')^2}\, d\theta$。

\textbf{（b）}：计算 $\alpha'$ 与 $\alpha''$，代入曲率公式 $k = |\alpha'\times\alpha''|/|\alpha'|^3$ 可得
\[
k(\theta) = \frac{2(\rho')^2 - \rho\rho'' + \rho^2}{\{(\rho')^2 + \rho^2\}^{3/2}}.
\]
$\square$
\end{solution}
egin{exercise}{1-5, 12* — do Carmo, Exercise 1-5, 12}
...
\begin{solution}
\textbf{直观理解}：曲线落在球面上的条件可以用曲率半径 $R=1/k$ 和挠率半径 $T=1/\tau$ 的关系来描述。

\textbf{证明}：设曲线 $\alpha(s)$ 在球心为 $c$、半径为 $R_0$ 的球面上，即 $|\alpha(s)-c|=R_0$。对该式求导得 $(\alpha-c)\cdot t=0$，再求导得 $1+(\alpha-c)\cdot n'=0$，即 $(\alpha-c)\cdot n=-1$。利用 Frenet-Serret 公式和 $\tau\neq 0$ 的条件，可推出
\[
R^2 + (R')^2 T^2 = \mathrm{const.}
\]
反之，若该条件成立，可构造球心 $c = \alpha + R\mathbf{n} + R R' T\mathbf{b}$，验证 $c$ 为常数，从而 $\alpha$ 落在球面上。$\square$
\end{solution}
egin{exercise}{1-5, 14 — do Carmo, Exercise 1-5, 14}
...
\begin{solution}
由 $\mathbf{b}' = -\tau\mathbf{n}$（Frenet-Serret 公式），得 $|\mathbf{b}'| = |\tau|$。因此若已知 $\mathbf{b}(s)$，则 $\tau = -|\mathbf{b}'|$。而 $k = |\mathbf{t}'| = |(\mathbf{b}\times\mathbf{t})'|$，由 $\mathbf{b}$ 及其导数可确定。故 $\mathbf{b}$ 完全决定了 $k$ 和 $|\tau|$。$\square$
\end{solution}
egin{exercise}{1-5, 16* — do Carmo, Exercise 1-5, 16}
...
\begin{solution}
\textbf{直观理解}：螺旋线的切线与固定方向保持恒定夹角，曲率和挠率之比为常数。

\textbf{解答}：

\textbf{（a）}：设切线与固定方向 $v$ 的夹角为 $\theta$（常数），即 $t\cdot v = \cos\theta$。求导得 $k\mathbf{n}\cdot v = 0$，故 $\mathbf{n}\cdot v = 0$。再求导，利用 $\mathbf{n}' = -k\mathbf{t} + \tau\mathbf{b}$，得 $\tau\mathbf{b}\cdot v = 0$（若 $\tau\neq 0$）。于是 $k/\tau$ 与 $v$ 的方向无关，即 $k/\tau = \mathrm{const}$。

\textbf{（b）}：由 $\mathbf{n}\cdot v = 0$，$n(s)$ 始终与固定方向 $v$ 垂直，即 $n(s)$ 平行于一个固定平面（与 $v$ 垂直的平面）。

\textbf{（c）}：类似地，$\mathbf{b}\cdot v$ 与切线夹角同为常数。

\textbf{（d）}：对给定曲线直接计算可得 $k/\tau = a/b$，为常数，故为螺旋线。$\square$
\end{solution}
egin{exercise}{1-5, 18* — do Carmo, Exercise 1-5, 18}
...
\begin{solution}
\textbf{直观理解}：Bertrand 曲线存在一条伴侣曲线 $\bar{\alpha}$，两者的法线在对应点重合。

\textbf{解答}：

\textbf{（a）}：由 $\bar{\alpha}(t) = \alpha(t) + r n(t)$，对 $t$ 求导得 $\bar{\alpha}' = t + r(-kt + \tau b) = (1-rk)t + r\tau b$。要求 $\bar{\alpha}' \parallel n$，需 $r\tau = 0$。由 $\tau\neq 0$ 得 $r = 0$，矛盾？更严谨的做法是直接对法线条件 $(\bar{\alpha}-\alpha)\parallel n$ 求导，可证 $r$ 为常数。

\textbf{（b）}：由 $\bar{\alpha}' \parallel n$ 的条件可得 $r$ 为常数。反之，若存在常数 $A, B$ 使得 $Ak + B\tau = 1$，则可构造 $\bar{\alpha} = \alpha + Ak\, n$，验证其为 $\alpha$ 的 Bertrand mate。

\textbf{（c）}：若 $\alpha$ 有多于一个 Bertrand mate，则 $\alpha$ 必为圆柱螺旋线；反之，圆柱螺旋线有无穷多个 Bertrand mate。$\square$
\end{solution}
