...
双叶双曲面 $-x^2 - y^2 + z^2 = 1$ 是正则曲面，因为它可以写成 $S = f^{-1}(0)$，其中 0 是 $f(x,y,z) = -x^2 - y^2 + z^2 - 1$ 的正则值。注意曲面 $S$ 不是连通的：给定分别在两叶（$z > 0$ 和 $z < 0$）上的两点，不存在完全位于曲面上的连续曲线 $\alpha(t) = (x(t), y(t), z(t))$ 将它们连接起来；否则 $z$ 改变符号，对某个 $t_0$ 有 $z(t_0) = 0$，这意味着 $\alpha(t_0) \notin S$。
\end{Example}
