...
\begin{solution}
\textbf{直观理解}：悬链线 $\alpha(t) = (t, \cosh t)$ 的曲率恒正，其渐伸线具有简洁的代数形式。

\textbf{解答}：

\textbf{（a）}：将悬链线写为参数方程 $\alpha(t) = (t, \cosh t)$，则 $\alpha'(t) = (1, \sinh t)$，$\alpha''(t) = (0, \cosh t)$。平面曲线的有向曲率公式为
\[
k(t) = \frac{x'y'' - x''y'}{((x')^2 + (y')^2)^{3/2}} = \frac{1\cdot\cosh t - 0\cdot\sinh t}{(1 + \sinh^2 t)^{3/2}} = \frac{\cosh t}{\cosh^3 t} = \frac{1}{\cosh^2 t}.
\]

\textbf{（b）}：悬链线的单位法向量（取指向曲线凹侧）为 $n(t) = \dfrac{(-\sinh t, 1)}{\cosh t}$，曲率半径 $R(t) = \cosh^2 t$。渐伸线为
\[
\beta(t) = \alpha(t) + R(t)n(t) = (t, \cosh t) + \cosh^2 t\cdot\frac{(-\sinh t, 1)}{\cosh t} = (t - \sinh t\cosh t, 2\cosh t).
\]
\qed
\end{solution}
...
\begin{solution}
\textbf{直观理解}：将极坐标曲线用参数方程表示，然后应用平面曲线曲率公式。

\textbf{解答}：

\textbf{（a）}：极坐标 $\rho = \rho(\theta)$ 可写为参数方程 $\alpha(\theta) = (\rho(\theta)\cos\theta, \rho(\theta)\sin\theta)$。则
\[
\alpha'(\theta) = (\rho'\cos\theta - \rho\sin\theta,\; \rho'\sin\theta + \rho\cos\theta),\quad |\alpha'(\theta)| = \sqrt{\rho^2 + (\rho')^2}.
\]
弧长 $s(\theta) = \int_a^\theta \sqrt{\rho^2 + (\rho')^2}\, d\theta$，故弧长公式如题所示。

\textbf{（b）}：由平面曲线有向曲率公式 $k = \dfrac{x'y''-x''y'}{((x')^2+(y')^2)^{3/2}}$，计算得
\[
x' = \rho'\cos\theta - \rho\sin\theta,\quad y' = \rho'\sin\theta + \rho\cos\theta,
\]
\[
x'' = \rho''\cos\theta - 2\rho'\sin\theta - \rho\cos\theta,\quad y'' = \rho''\sin\theta + 2\rho'\cos\theta - \rho\sin\theta.
\]
代入并整理得
\[
k(\theta) = \frac{2(\rho')^2 - \rho\rho'' + \rho^2}{(\rho^2 + (\rho')^2)^{3/2}}.
\]
其中分子可正可负，故 $k$ 为有向曲率。\qed
\end{solution}
...
\begin{solution}
\textbf{直观理解}：由 Frenet-Serret 公式 $\mathbf{b}' = -\tau\mathbf{n}$，副法向量的导数揭示了挠率的信息。由于 $\|\mathbf{b}'\| = |\tau|$，已知 $\mathbf{b}(s)$ 即可直接求得 $|\tau|$。而主法向量 $\mathbf{n} = -\frac{1}{\tau}\mathbf{b}'$（模长已知），再由 $\mathbf{t} = \mathbf{n}\times\mathbf{b}$ 和 $\mathbf{t}' = k\mathbf{n}$ 可求得曲率。

\textbf{解答}：由 $\mathbf{b}' = -\tau\mathbf{n}$ 得 $|\mathbf{b}'| = |\tau|$，故 $|\tau(s)| = |\mathbf{b}'(s)|$ 可直接从 $\mathbf{b}(s)$ 及其导数确定。又由 $\mathbf{b}' = -\tau\mathbf{n}$，在 $\tau \neq 0$ 的点上，$\mathbf{n} = -\frac{1}{\tau}\mathbf{b}'$。取范数得 $|\mathbf{n}| = |\tau|^{-1}|\mathbf{b}'| = 1$，与已知的 $\|\mathbf{n}\| = 1$ 一致。再由正交性 $\mathbf{t} = \mathbf{n}\times\mathbf{b}$ 确定切向量，而 $k = |\mathbf{t}'| = |\mathbf{n}'|$（由 $\mathbf{t}' = k\mathbf{n}$），故 $k$ 亦可由 $\mathbf{b}$ 及其导数确定。$\square$
\end{solution}
...
\begin{solution}
\textbf{直观理解}：螺旋线的切向量与固定方向（$z$ 轴）夹角恒定，由此可推出 $k/\tau = \text{const}$。

\textbf{解答}：

\textbf{（a）}：设固定单位方向向量为 $\mathbf{a}$，切向量 $\mathbf{t}(s)$ 与 $\mathbf{a}$ 的夹角恒为 $\theta$（$0 < \theta < \pi$），即 $\mathbf{t}(s)\cdot\mathbf{a} = \cos\theta = \text{const}$。对 $s$ 求导得 $\mathbf{t}'(s)\cdot\mathbf{a} = k(s)\mathbf{n}(s)\cdot\mathbf{a} = 0$，故 $\mathbf{a}$ 与 $\mathbf{n}(s)$ 垂直，从而 $\mathbf{a}$ 平行于由 $\mathbf{t}$ 和 $\mathbf{b}$ 张成的平面。设 $\mathbf{a} = \mathbf{t}\cos\theta + \mathbf{b}\sin\theta$。再对 $s$ 求导并用 Frenet-Serret 公式：
\[
0 = \mathbf{t}'\cos\theta + \mathbf{b}'\sin\theta = k\mathbf{n}\cos\theta - \tau\mathbf{n}\sin\theta = (k\cos\theta - \tau\sin\theta)\mathbf{n}.
\]
故 $k\cos\theta = \tau\sin\theta$，即 $k/\tau = \tan\theta = \text{const}$。反之亦然。

\textbf{（b）}：若 $n(s)$ 平行于固定平面，则存在单位向量 $\mathbf{a}$ 使得 $\mathbf{n}(s)\cdot\mathbf{a} = 0$。求导得 $k\mathbf{t}\cdot\mathbf{a} = 0$，即 $\mathbf{t}\cdot\mathbf{a} = \text{const}$，故 $\alpha$ 为螺旋线。逆命题同理。

\textbf{（c）}：若 $b(s)$ 与固定方向 $\mathbf{a}$ 的夹角恒定，即 $b(s)\cdot\mathbf{a} = \text{const}$，对 $s$ 求导得 $b'\cdot\mathbf{a} = -\tau\mathbf{n}\cdot\mathbf{a} = 0$，故 $\mathbf{n}\cdot\mathbf{a} = 0$，由 (a) 的证明知 $\alpha$ 为螺旋线。

\textbf{（d）}：对给定的曲线
\[
\alpha(s) = \left(\frac{a}{c}\int\sin\theta(s)\,ds,\; \frac{a}{c}\int\cos\theta(s)\,ds,\; \frac{b}{c}s\right),
\]
其中 $\theta(s) = \frac{\sqrt{a^2+b^2}}{a}s$（由 $\theta' = k = \sqrt{a^2+b^2}/a$ 积分），可验证 $k/\tau = a/b = \text{const}$，故 $\alpha$ 为螺旋线。$\square$
\end{solution}
...
\begin{solution}
\textbf{直观理解}：Bertrand 曲线是指存在另一条曲线，其法线与原曲线法线重合。Bertrand  mates 的存在性由曲率和挠率之间的线性关系刻画。

\textbf{解答}：

\textbf{（a）}：设 $\bar{\alpha}(t) = \alpha(t) + rn(t)$。对 $t$ 求导得
\[
\bar{\alpha}' = (1 - rk)t + r'n - r\tau b.
\]
由于 $\bar{\alpha}'$ 与 $\bar{\alpha}$ 的法线方向（即 $n$）垂直，故 $\bar{\alpha}'\cdot n = 0$，即 $r'n = 0$，故 $r' = 0$，即 $r$ 为常数。

\textbf{（b）}：设 $\bar{t}$ 为 $\bar{\alpha}$ 的单位切向量。由 (a) 得
\[
\bar{t}\cdot t = \frac{1 - rk}{\sqrt{(1-rk)^2 + (r\tau)^2}} = \cos\theta = \text{const.}
\]
整理得 $\frac{1 - rk}{r\tau} = \text{const.} = \frac{B}{r}$，即 $Ak + B\tau = 1$（其中 $A = r$）。反之，若该线性关系成立，定义 $\bar{\alpha} = \alpha + rn$，则可验证 $\bar{\alpha}$ 与 $\alpha$ 法线重合，故 $\alpha$ 为 Bertrand 曲线。

\textbf{（c）}：若 $\alpha$ 有两个不同的 Bertrand mates $\bar{\alpha} = \alpha + \bar{r}n$ 和 $\tilde{\alpha} = \alpha + \tilde{r}n$，则由 (b) 知存在常数 $c_1, c_2$ 使
\[
1 - \bar{r}k = c_1\bar{r}\tau,\quad 1 - \tilde{r}k = c_2\tilde{r}\tau.
\]
对两式求导得 $k' = \tau'c_1$ 和 $k' = \tau'c_2$，由于 $c_1 \neq c_2$，故 $k' = \tau' = 0$，即 $k$ 和 $\tau$ 均为常数。由曲线基本定理，这正好是圆柱螺旋线的特征。反之，圆柱螺旋线的曲率 $k$ 和挠率 $\tau$ 均为常数，对任意 $r$ 只要满足 $Ak + B\tau = 1$ 就给出一个 Bertrand mate，故有无穷多个。$\square$
\end{solution}
