...
\begin{figure}[H]
\centering
\subfloat[Figure 3-1. The Möbius strip — 不可定向曲面的经典例子]{\label{fig:fig3-1}\includegraphics[width=0.6\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/53c3f147d73fbbaff460736e7109c90902470b480130ac9116222c530a8f7b29.jpg}}
\end{figure}
...
\tag{3-8}
\]
这一性质确保了 $S_p$ 有实特征值和两两正交的特征方向。Gauss 映射 $N: S \to S^2$ 的几何图像见 \cref{fig:fig3-2}，$dN_p$ 的几何意义见 \cref{fig:fig3-3}。

\begin{figure}[H]
\centering
\begin{minipage}{0.48\textwidth}
\centering
\subfloat[Figure 3-2. The Gauss map $N: S \to S^2$]{\label{fig:fig3-2}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/216853bb9c9d2a1d4811f2cd06eea0b1ea035c4b263de20ca9c903b47accf589.jpg}}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
\centering
\subfloat[Figure 3-3. $dN_p$ 的几何意义]{\label{fig:fig3-3}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/fa20784ffa0cf87b26556fefc3048146f0287308bc09e31a93cf962aa4714cf4.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.48\textwidth}
\centering
\subfloat[Figure 3-5. Unit sphere: $d\bar{N}_p(v) = v$]{\label{fig:fig3-5}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/c0ffa599c2b6e7a0e2c8b098970dfcc6ec6474b8b6c2b0a1a998e43e4a126acf.jpg}}
\end{minipage}
\end{figure}

\begin{figure}[H]
\centering
\subfloat[Figure 3-6. 圆柱面的 $dN$]{\label{fig:fig3-6}\includegraphics[width=0.6\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/7e110dac56ae33192bfe686568e285d52b2bcf6e289089bc6e98471f331ffa9e.jpg}}
\end{figure}

\textbf{特例对比.} 平面和球面的 Weingarten 映射分别是 $S_p = 0$ 和 $S_p = -I$（见 \cref{fig:fig3-4,fig:fig3-5}）。圆柱面（见 \cref{fig:fig3-6}）则介于两者之间——沿母线方向 $dN = 0$，沿圆周方向 $dN = -I$。
...
Meusnier 定理告诉我们：法曲率只依赖于方向 $v$，而不依赖于具体曲线的选择。这是曲面内蕴几何的一个体现。Meusnier 定理的几何图像见 \cref{fig:fig3-9}，球面和柱面上的法截面曲线分别见 \cref{fig:fig3-11,fig:fig3-12}。
...
Construct the Dupin indicatrix at $p=(0,0,0)$ for the monkey saddle and compare with the intersection curve with a plane parallel to $T_p(S)$.
\end{exercise}
...
Dupin 指标线（Dupin indicatrix）是研究主曲率几何意义的工具——它由法曲率的等距像构成（见 \cref{fig:fig3-13}）。

\begin{figure}[H]
\centering
\subfloat[Figure 3-13. The Dupin indicatrix — 椭圆点与双曲点的刻画]{\label{fig:fig3-13}\includegraphics[width=0.6\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/6cd2b935fbe366fbd1d068ac39b24d5e8497b3d219b67564caa24aa4568cbea4.jpg}}
\end{figure}

\subsection*{3-4 节练习}

\begin{exercise}{3-3, 5* — do Carmo, Exercise 3-3, 5 (Enneper)}
For Enneper's surface $\mathbf{x}(u,v) = (u - u^3/3 + uv^2, v - v^3/3 + vu^2, u^2 - v^2)$, show: (a) $E = G = (1+u^2+v^2)^2$, $F = 0$, (b) $e = 2$, $g = -2$, $f = 0$, (c) $k_1 = 2/(1+u^2+v^2)^2$, $k_2 = -2/(1+u^2+v^2)^2$, (d) coordinate curves are lines of curvature, (e) asymptotic curves are $u \pm v = \text{const}$.
\end{exercise}
...
\[
\begin{aligned}
L_v - M_u &= L\Gamma_{12}^1 + M\bigl(\Gamma_{12}^2 - \Gamma_{11}^1\bigr) - N\,\Gamma_{11}^2, \\
N_u - M_v &= L\Gamma_{22}^1 + M\bigl(\Gamma_{22}^2 - \Gamma_{21}^1\bigr) - N\,\Gamma_{21}^2.
\end{aligned}
\tag{3-18}
\]
...
\begin{exercise}{3-5, 1* — do Carmo, Exercise 3-3, 6}
Consider the pseudosphere (surface of revolution of the tractrix).
\begin{itemize}
\item[a.] Determine an equation for the tractrix.
\item[b.] Show that the Gaussian curvature of any regular point of the pseudosphere is $-1$.
\end{itemize}
\end{exercise}

\begin{exercise}{3-5, 2 — do Carmo, Exercise 3-3, 7}
Let $S$ be a surface of revolution $\mathbf{x}(u,v) = (\varphi(v)\cos u, \varphi(v)\sin u, \psi(v))$ with $(\varphi')^2 + (\psi')^2 = 1$. Prove that $\varphi$ satisfies $\varphi'' + K\varphi = 0$ where $K$ is the Gaussian curvature.
\end{exercise}
...
L &= \langle \mathbf{x}_{\theta\theta}, N \rangle
  ...
  &\qquad + (-\sin\theta\sin\varphi)(\sin\theta\sin\varphi)
   + (-\cos\theta)(\cos\theta)
...
N_{coeff} = \langle \mathbf{x}_{\varphi\varphi}, N \rangle = -\sin^2\theta.
\tag{A3-2}
\]

\textbf{结论}：球面的第二基本形式系数为 $L = -1,\; M = 0,\; N_{coeff} = -\sin^2\theta$。这与 $S_p = -I$（从几何上看，$dN_p(v) = -v$）完全一致。
