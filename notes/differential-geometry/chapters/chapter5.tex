...
\centering
\subfloat[Figure 5-2. 常正曲率旋转面]{\label{fig:fig5-2}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/d0706b198f2897c57beca8da2254a5745b2ff1562571473ec50989e16d23cda5.jpg}}
\end{minipage}
\end{figure}
...
\centering
\subfloat[Figure 5-1a. 非刚性的卵形面例子：将平面区域 $P$ 向内"突起"得到 $S'$]{\label{fig:fig5-1a}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/1010f8a78b7253968f731675b5aa9f43bf3ca471ea4bd8f01af07ba38d735e75.jpg}}
\end{minipage}
\hfill
\begin{minipage}{0.45\textwidth}
\centering
\subfloat[Figure 5-1b. 对称"突起"得到 $S''$，$S'$ 与 $S''$ 等距但无正交线性变换将 $S'$ 变为 $S''$]{\label{fig:fig5-1b}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/c2b845abcc71c94b3bb04390b4cc2df94089704fb9781188491ac35498c18212.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.45\textwidth}
\centering
\subfloat[Figure 5-5. 锥面上过 $p$ 的唯一不可延拓测地线是过 $p$ 的母线]{\label{fig:fig5-5}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/beb5e685031a4887db06d1c883d81f99a5afeb21221367a9f0f8e06adb27e172.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.55\textwidth}
\centering
\subfloat[Figure 5-7. 完备非闭曲面的例子：绕以圆为渐近线的平面曲线旋转得到的柱面]{\label{fig:fig5-7}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/acad36bb78b1fb869d9d94c12cba6f4b336f5a326660d438ba1de2cd315c47a8.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.55\textwidth}
\centering
\subfloat[Figure 5-9. Hopf-Rinow 定理证明中的构造]{\label{fig:fig5-9}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/b76a37d9c31c41a342040fb614134337cbc63dd6b8139d1993a13b3af5be1dad.jpg}}
\end{minipage}
\end{figure}
...
\centering
\subfloat[Figure 5-3. $S - \{p\}$ 不是完备的： geodesic $\gamma$ 过 $p$ 但 $p \notin S-\{p\}$}{\label{fig:fig5-3}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/d7bfebe261ddfb7f614425e1e3b7a806239728d58b983dc44364b23865d632e3.jpg}}
\end{minipage}
\end{figure}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 单叶锥面沿一条母线剪开后可等距展开为平面上的一个扇形区域（见第 4-2 节例 3）。在平面上，任意两点之间的直线段即为最短测地线，将其卷回锥面即得到锥面上的最短测地线。

\textbf{解答}: 设 $S$ 是单叶锥面（去掉顶点），由第 4-2 节例 3，存在从锥面去掉一条母线得到的开集 $S\setminus\gamma$ 到平面扇形 $W=\{(r,\theta):0<r<\infty,\ \alpha_0<\theta<\alpha_1\}$（其中 $\alpha_1-\alpha_0=2\pi\sin\beta<2\pi$）的等距映射 $\varphi:S\setminus\gamma\to W$。对任意 $p,q\in S$，分两种情况：

(1) 若 $p,q$ 都不在母线 $\gamma$ 上，则 $\varphi(p),\varphi(q)\in W$，而 $W$ 是平面上凸区域，故存在唯一直线段 $\overline{\varphi(p)\varphi(q)}\subset W$。像曲线 $\varphi^{-1}(\overline{\varphi(p)\varphi(q)})$ 即为连接 $p,q$ 的最短测地线。

(2) 若 $p$ 或 $q$ 在母线 $\gamma$ 上，不妨设 $p\in\gamma$。由于 $\gamma$ 是正则曲线（去掉顶点），可在 $\gamma$ 附近取小弧段构造新的开集 $S\setminus\gamma_1$，其中 $\gamma_1$ 是另一条母线，使 $p$ 不在 $\gamma_1$ 上。重复上述过程，仍可在锥面上找到连接 $p,q$ 的最短测地线。

关键在于：锥面本身是完备的（去掉顶点后仍是完备的度量空间），而等距映射 $\varphi$ 将弧长距离保持不变，故 $S$ 上任意两点间的最短测地线必在 $S\setminus\gamma$ 的某个等距展开图中表现为平面直线段。由扇形的凸性，这样的直线段唯一，故最短测地线存在且唯一（若两点均不在母线上）。\qed
\end{solution}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-11]{\label{fig:fig5-11}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/37468c0aac1e027f8629b5ba8f338edfaa0a974896990c88c40f7008a1f9dd1d.jpg}}
\end{minipage}
\end{figure}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: $S_1$ 的完备性保证 Cauchy 序列收敛，$S_2$ 的单连通性与唯一测地线条件结合 Hadamard 定理给出 $S_2\cong\mathbb{R}^2$ 的拓扑结构。局部等距 $\varphi$ 保持测地线，故 $\varphi$ 将 $S_1$ 的完备性传递到 $S_2$，而 $S_2$ 的 Hadamard 结构使任意两点间的测地线唯一且整体最短，从而 $\varphi$ 将弧长距离 $d_1$ 等距映射到 $d_2$，是整体等距。

\textbf{解答}: 由 Hadamard 定理（\,5-6 节定理 2），$S_2$ 单连通且 $K\leq0$ 完备，故 $\exp_{q}:T_{q}(S_2)\to S_2$ 对所有 $q\in S_2$ 为覆盖映射。由于 $S_2$ 上任意两点间的测地线唯一，$\exp_q$ 必为单射，故 $\exp_q$ 为全局微分同胚，即 $S_2$ 微分同胚于 $\mathbb{R}^2$。

设 $\\_l$ 是 $S_1$ 上的弧长距离。取 $p\in S_1$，令 $q=\varphi(p)$。对 $S_1$ 上任意分段光滑曲线 $\\_\alpha$，局部等距条件 $|\varphi\circ\alpha'(t)|_{S_2}=|\alpha'(t)|_{S_1}$ 蕴含 $L\_{S_2}(\varphi\circ\alpha)=L\_{S_1}(\alpha)$。故 $\varphi$ 保持弧长：$d\_2(\varphi(p),\varphi(q))=d\_1(p,q)$。

下证 $\varphi$ 是满射。取任意 $r\in S_2$，由 $S_2\cong\mathbb{R}^2$，存在从 $q$ 到 $r$ 的测地线 $\gamma$。设 $\tilde{\gamma}$ 是 $T\_{p}(S_1)$ 中满足 $d\exp\_{p}(\tilde{\gamma}(0))=\\_\gamma'(0)$ 的曲线。由于 $\varphi$ 是局部等距且保持指数映射：$\varphi(\exp\_p(\tilde{\gamma}(t)))=\exp\_{q}(\tilde{\gamma}(t))$，沿 $\tilde{\gamma}$ 积分得 $\varphi$ 的像覆盖整个 $S_2$。

最后，由 $d\_2(\varphi(p\_1),\varphi(p\_2))=d\_1(p\_1,p\_2)$ 对所有 $p\_1,p\_2\in S_1$ 成立，$\varphi$ 为等距双射，故为整体等距。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: (a) 部分的结论说明与测地线方向正交的 Jacobi 场始终保持正交——这是 Jacobi 方程的直接推论，因为混合项 $\langle DJ/ds, \gamma'\rangle$ 的导数在正交初始条件下恒为零。(b) 部分说明在此正交基下，Jacobi 场简化为一个标量二阶 ODE 的解。

\textbf{解答}: (a) 沿 $\gamma$ 有 Jacobi 方程 $\frac{D^2J}{ds^2}+KJ=0$，其中 $K$ 是高斯曲率。对 $\langle J(s),\gamma'(s)\rangle$ 求导：
\[
\frac{d}{ds}\langle J,\gamma'\rangle = \langle \frac{DJ}{ds},\gamma'\rangle + \langle J,\frac{D\gamma'}{ds}\rangle = \langle \frac{DJ}{ds},\gamma'\rangle,
\]
因为 $\gamma$ 是测地线，$\frac{D\gamma'}{ds}=0$。再求导得
\[
\frac{d^2}{ds^2}\langle J,\gamma'\rangle = \langle \frac{D^2J}{ds^2},\gamma'\rangle = -K\langle J,\gamma'\rangle.
\]
...
(b) 设 $\{e_1(s),e_2(s)\}$ 是沿 $\gamma$ 的平行正交基，$e_1(0)=\gamma'(0)$，$e_2(0)=J'(0)$。将 $J(s)$ 在此基下展开为 $J(s)=a(s)e_1(s)+b(s)e_2(s)$。由 (a) 的结果 $\langle J,\gamma'\rangle=0$ 及 $e_1(s)=\gamma'(s)$，得 $a(s)\equiv 0$，故 $J(s)=b(s)e_2(s)$。代回 Jacobi 方程，由 $e_2(s)$ 的平行性可推出标量方程 $b''+K(s)b=0$。由 $J(0)=0$ 得 $b(0)=0$，由 $|J'(0)|=1$ 和 $e_2(0)=J'(0)$ 得 $b'(0)=1$。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 抛物面 $z=x^2+y^2$ 有 $K>0$ 但趋于零（远处非常平坦），这意味着 Jacobi 场增长但不会在有限距离内归零——类似于平面情形 $(K=0)$ 但有轻微弯曲。

\textbf{解答}: 抛物面 $z=x^2+y^2$ 的第一基本形式为 $E=G=1+4(x^2+y^2)$，$F=0$，高斯曲率为
\[
K = \frac{4}{(1+4r^2)^2},\qquad r^2=x^2+y^2.
\]
从原点出发的测地线（弧长参数化）满足 Jacobi 方程 $u''+K(s)u=0$（见练习 1）。由于 $K(s)>0$ 且 $K(s)\to 0$ 当 $s\to\infty$ 时，方程的解 $u(s)$ 满足 $u''\leq 0$ 当 $K$ 很小时——$u(s)$ 不会振荡，而是在正初始条件下单调递增（利用比较定理）。具体地，考虑 $v(s)$ 满足 $v''=0$，$v(0)=0$，$v'(0)=1$（即 $v(s)=s$）。由于 $K>0$，比较定理（练习 3）给出 $u(s)\geq v(s)=s>0$，从而 $u$ 无零点。故从 $p$ 出发的任意测地线上无共轭点。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 比较定理是 Jacobi 场理论的核心工具：更大曲率使共轭点更早出现且 Jacobi 场模更小（正曲率"聚拢"测地线）。这是 Jacobi 场在分析学上的 Sturm 型比较定理。

\textbf{解答}: (a) 由练习 1，$\langle J(s),\gamma'(s)\rangle\equiv 0$，故 $J(s)=v(s)e_2(s)$，$v$ 满足 $v''+K(s)v=0$，$v(0)=0$，$v'(0)=|J'(0)|=1$。同理 $\tilde{J}(s)=u(s)\tilde{e}_2(s)$。

(b) 设 $a$ 是 $\tilde{J}$ 的首个零点（首个共轭点）。取 $s_0\in(0,a)$，考察 $\tilde{v}(s)=u(s)$ 和 $v(s)$ 在 $s\in[0,s_0]$ 上的比较。因 $u(s)>0$ 在 $(0,a)$ 上，$K\leq\tilde{K}$，由 Sturm 比较定理（练习 6），$v(s)$ 在 $[0,s_0]$ 上无零点。由于 $s_0<a$ 任意，故 $\gamma$ 的首个共轭点不早于 $a$。

(c) 由 Gauss 引理，$|J(s)|=|v(s)|$，$|\tilde{J}(s)|=|u(s)|$。由 (b) 的比较定理证明过程知：在 $K\leq\tilde{K}$ 时，$v(s)\geq u(s)$（两者均为正）。故 $|J(s)|\geq|\tilde{J}(s)|$。

(d) 若 $K(s)=\tilde{K}(s)$，则 $v$ 与 $u$ 满足相同初始条件的同一方程，故 $v\equiv u$，等号恒成立。反之，若 $|J(s)|=|\tilde{J}(s)|$ 对所有 $s$ 成立，则 $v\equiv u$，代入方程得 $K\equiv\tilde{K}$。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 常曲率 $K=K_0$ 球面中，测地线（大圆）在 $s=\pi/\sqrt{K_0}$ 处恰好出现首个共轭点（对径点）。若实际曲面 $S$ 的曲率 $K\leq K_0$，则共轭点只能更晚（不能更早）。

\textbf{解答}: 设 $\tilde{S}$ 是截面曲率为常数 $K_0$ 的模型曲面（球面或双曲平面）。由第一比较定理（练习 3(b)），若 $\tilde{S}$ 的测地线在 $s=\pi/\sqrt{K_0}$ 处有共轭点，则 $S$ 的任意测地线在该区间内无共轭点。直接验证：在常数曲率 $K_0$ 曲面上，$u''+K_0u=0$，$u(0)=0$，$u'(0)=1$ 的解为 $u(s)=\frac{1}{\sqrt{K_0}}\sin(\sqrt{K_0}s)$，其首个零点在 $s=\pi/\sqrt{K_0}$。故 $S$ 的任意测地线在 $(0,\pi/\sqrt{K_0})$ 内无共轭点。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 与练习 4 相反：若曲率有正下界 $K_1$，则测地线在更短距离内就会聚——正曲率"聚拢"测地线的效果更强。

\textbf{解答}: 设 $u$ 是 $u''+K_1u=0$，$u(0)=0$，$u'(0)=1$ 的解，即 $u(s)=\frac{1}{\sqrt{K_1}}\sin(\sqrt{K_1}s)$，其在 $s=\pi/\sqrt{K_1}$ 处有零点。因 $K(s)\geq K_1$，由第二比较定理（练习 3(c)，注意方向反转），$J$ 的模 $|J(s)|\leq u(s)$（在同一初始条件下）。故 $J$ 的零点不晚于 $s=\pi/\sqrt{K_1}$，即任意测地线在 $(0,\pi/\sqrt{K_1}]$ 内必有共轭点。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: Sturm 振荡定理是 ODE 的经典结果：若 $u$ 是 $u''+Lu=0$ 的解而 $v$ 是 $v''+Kv=0$ 的解（$K\leq L$），且 $v$ 在 $s_0$ 处有零点，则 $u$ 也在 $s_0$ 之前变号。这里 $J$ 扮演 $v$ 的角色。

\textbf{解答}: 设 $v(s)=|J(s)|$。由 Jacobi 方程和 $K\leq L$，可证 $v''+Lv\leq 0$（实际上 $v''+Kv=0$，故 $v''+Lv = -L_v + Lv \leq Kv + Lv \leq 0$ 当 $v>0$）。取 $u$ 为 $u''+Lu=0$ 的任一解，令 $w=u/v$（在 $s=0$ 和 $s=s_0$ 附近有定义），则 $w$ 满足 $(vw')'=w(v''+Lu)\leq 0$，从而 $(vw')$ 在 $[0,s_0]$ 上单调递减。由 $v(0)=0$，$v(s_0)=0$，对 $(vw')$ 在 $[0,s_0]$ 上积分得
\[
0 = (vw')|_{s_0} - (vw')|_{0} = -\int_0^{s_0}(vw')'\,ds \geq 0.
\]
由 $u(0)=0$，$u$ 在 $s_0$ 附近非零可知 $w$ 有界，但 $(vw')$ 递减且两端均为零意味着 $u$ 必须在 $(0,s_0)$ 内有零点。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: Kneser 判据通过构造辅助函数 $w$ 给出了曲率积分条件下的共轭点判定——本质上是把 Sturm 振荡定理的条件写成曲率的积分不等式。

\textbf{解答}: (a) 直接计算：
\[
w'(t) = -\!K(t) - \frac{1}{4(t+1)^2},\qquad
w(t)^2 = \left(\int_t^\infty K + \frac{1}{4(t+1)}\right)^2 \geq \frac{1}{4(t+1)^2},
\]
故 $w'(t)+w(t)^2 \leq -K(t)$。

(b) 计算 $v(t)$ 的导数：
\[
v'(t)=w(t)v(t),\qquad v''(t)=(w'(t)+w(t)^2)v(t)=-L(t)v(t).
\]
且 $v(0)=\exp(0)=1$，$v'(0)=w(0)\cdot 1=0$。

(c) 由 (b) 知 $v$ 满足 $v''+Lv=0$，$v(0)=1$，$v'(0)=0$，$L(t)\geq K(t)$。比较方程 $u''+Ku=0$（对应 Jacobi 场的标量方程）由 Sturm 比较定理，$v$ 恒为正（无零点），故 $K$ 条件下的 Jacobi 场无零点，即 $\gamma$ 上无点与 $p$ 共轭。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: Jacobi 方程是线性二阶 ODE，4 维解空间由初始条件 $(J(0),J'(0))$ 张成。两个线性边界条件（$J(0)=w_0$，$J(l)=w_1$）一般唯一确定一个解——但 $\gamma(l)$ 与 $\gamma(0)$ 不共轭保证了线性映射 $(J(0),J'(0))\mapsto (J(0),J(l))$ 是同构。

\textbf{解答}: Jacobi 方程 $\frac{D^2J}{ds^2}+KJ=0$ 是线性常微分方程组。取沿 $\gamma$ 的平行正交基 $\{e_1(s),e_2(s)\}$（$e_1(0)=\gamma'(0)$），则 $J(s)=a(s)e_1(s)+b(s)e_2(s)$，其中 $b$ 满足 $b''+K(s)b=0$，而 $a$ 满足 $a''+K(s)a=0$。将边界条件投影到基上得到 $a(0),a(l),b(0),b(l)$ 的线性方程组。该方程组有唯一解当且仅当对应齐次方程（$w_0=w_1=0$）仅有平凡解——而齐次方程的解正是与端点共轭条件相对应的 Jacobi 场。由题设 $\gamma(l)$ 与 $\gamma(0)$ 不共轭，故仅有平凡解，从而非齐次问题有唯一解。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 这与练习 1(a) 完全类似，但初始条件更强：不仅 $J(0)$ 与 $\gamma'(0)$ 正交，且 $J'(0)$ 也与 $\gamma'(0)$ 正交（因为 $J'(0)=0$）。

\textbf{解答}: 令 $f(s)=\langle J(s),\gamma'(s)\rangle$。与练习 1(a) 同样推导得 $f''+K(s)f=0$。初始条件给出 $f(0)=0$ 和 $f'(0)=\langle J'(0),\gamma'(0)\rangle=0$。由 ODE 解的唯一性，$f(s)\equiv 0$，即 $\langle J(s),\gamma'(s)\rangle=0$。\qed
\end{solution}
...
\begin{minipage}{0.55\textwidth}
\centering
\subfloat[Figure 5-13. 曲线变分的直观含义]{\label{fig:fig5-13}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/3b2c5faa763a8cefdddf8608839bb7df8d3e556c44e581337b9e40ecc7d4c6c8.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.55\textwidth}
\centering
\subfloat[Figure 5-14. (c) 小问中的几何构型]{\label{fig:fig5-14}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/b0507eebec0f2e639ed7f70f73f1aeae139ba72e68df67292ad2984512a50206.jpg}}
\end{minipage}
\end{figure}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: (a) 非真变分的第一变分多了边界项——因为端点不固定，弧长泛函的临界点条件中需要考虑端点的变化。(b) 距离函数 $d(s) = d(\gamma(s),p)$ 在整个实直线上有下界（因为 $\gamma$ 是测地线，$d(s)$ 必在某处取到最小值），这由完备曲面上函数的连续性和紧性论证。(c) 若额外假设 $S$ 与平面同胚且 $K \leq 0$，则测地线附近无共轭点，垂直条件唯一。

\textbf{解答}: (a) 设 $h: [0,l]\times(-\epsilon,\epsilon)\to S$ 是一般变分（端点可变）。由第一变分公式的推导过程（见第 5-4 节引理 2 的证明），去掉"真"条件后得到的公式为
\[
L'(0) = -\int_0^l \langle A(s), V(s)\rangle\,ds + \langle\frac{\partial h}{\partial s}(l,0),\, \frac{\partial h}{\partial t}(l,0)\rangle - \langle\frac{\partial h}{\partial s}(0,0),\, \frac{\partial h}{\partial t}(0,0)\rangle.
\]
当 $h$ 为真时（即 $V(0)=V(l)=0$），后两项为零；非真情况下，这两项就是端点的贡献。

(b) 设 $S$ 完备，$\gamma: \mathbb{R}\to S$ 为测地线，$p \in S$，$d(s) = d(\gamma(s),p)$。由第 5-3 节命题 4，$d$ 连续。由于 $S$ 完备且非紧（若紧则 $d$ 在紧集上取最小值；若非紧，则 $d(s) \to \infty$ 当 $|s|\to\infty$，在 $[-R,R]$ 上取最小值），故 $d$ 在某 $s_0$ 处取全局最小值。设 $\Gamma$ 是连接 $p$ 与 $\gamma(s_0)$ 的最小测地线（Hopf-Rinow 定理保证存在）。由第一变分公式，若 $\Gamma$ 与 $\gamma$ 不正交，则存在变分使 $\Gamma$ 缩短，与最小性矛盾。设 $V$ 为 $\Gamma$ 相对 $\gamma$ 的变分场，$V(s_0)\neq 0$。由第一变分公式的极值性质，$0 = L'(0) = -\int \langle A,V\rangle$，而 $A=0$（$\Gamma$ 也是测地线），这不足以得出正交。更直接的几何论证：若 $\Gamma$ 与 $\gamma(s_0)$ 在 $s_0$ 处不正交，则沿 $\gamma$ 存在使 $d(s)$ 下降的方向，与 $s_0$ 的最小性矛盾。

(c) 进一步设 $S$ 与平面同胚且 $K \leq 0$。此时，$\exp_p: T_p(S)\to S$ 是局部微分同胚（由第 5-5 节，无共轭点），且由于 $S$ 同胚于 $\mathbb{R}^2$，$\exp_p$ 是单射（若 $\exp_p(v_1) = \exp_p(v_2)$，则存在从 $p$ 出发的两条相同终点的测地线，由唯一性 $v_1=v_2$）。于是 $s_0$ 唯一（因为从 $p$ 到 $\gamma(s)$ 的最短测地线唯一）。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 变分法的核心是将"泛函的临界点"问题转化为"微分方程"问题。通过分部积分和基本引理（变分场在端点处为零），可得出 Euler-Lagrange 方程。当 $F$ 不显含 $x$ 时，存在守恒律（首次积分）。

\textbf{解答}: (a) 设 $y=y(x)$ 是 $I(t)$ 的临界点，即 $dI/dt = 0$。直接计算
\[
...
\[
\int_{x_1}^{x_2}F_{y'}\eta'\,dx = \left[F_{y'}\eta\right]_{x_1}^{x_2} - \int_{x_1}^{x_2}\frac{d}{dx}F_{y'}\cdot\eta\,dx
= -\int_{x_1}^{x_2}\frac{d}{dx}F_{y'}\cdot\eta\,dx,
\]
因为 $\eta(x_1)=\eta(x_2)=0$。于是
\[
0 = \frac{dI}{dt}\bigg|_{t=0} = \int_{x_1}^{x_2}\eta\left(F_y - \frac{d}{dx}F_{y'}\right)dx.
\]
由变分法基本引理（$\eta$ 任意），得 $F_y - \frac{d}{dx}F_{y'} = 0$（对 $x\in[x_1,x_2]$）。

(b) 即 Euler-Lagrange 方程。

(c) 若 $F = F(y,y')$，则
\[
\frac{d}{dx}\left(y'F_{y'} - F\right) = y''F_{y'} + y'((F_y)y' + (F_{y'})y'') - (F_y y' + F_{y'}y'')
= y''F_{y'} + y'F_y y' + F_{y'}y'' - F_y y' - F_{y'}y'' = 0,
\]
因为 $F_y = (d/dx)F_{y'}$（由 Euler-Lagrange 方程）。于是 $y'F_{y'} - F = \mathrm{const}$（首次积分）。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 最小面积旋转面问题等价于使泛函 $I = \int y\sqrt{1+(y')^2}\,dx$ 取极小值的 Euler-Lagrange 方程求解。首次积分给出 $y/\sqrt{1+(y')^2} = 1/c$，从而解出悬链线方程 $y = \frac{1}{c}\cosh(cx+c_1)$。旋转面的测地线满足 Clairaut 关系是旋转对称性的直接推论。

\textbf{解答}: (a) 旋转面 $S$ 由 $y=f(x)$，$x\in[x_1,x_2]$ 绕 $x$ 轴旋转生成。其面积泛函为
\[
A(f) = 2\pi\int_{x_1}^{x_2} f(x)\sqrt{1+(f'(x))^2}\,dx.
\]
取 $F(y,y') = y\sqrt{1+(y')^2}$，则 Euler-Lagrange 方程给出
\[
F_y - \frac{d}{dx}F_{y'} = 0\quad\Longrightarrow\quad
\sqrt{1+(y')^2} - \frac{d}{dx}\left(\frac{yy'}{\sqrt{1+(y')^2}}\right) = 0.
\]
简化得
\[
...
\[
u'F_{u'} - F = \frac{f^2(u')^2}{\sqrt{f^2(u')^2+1}} - \sqrt{f^2(u')^2+1}
= -\frac{1}{\sqrt{f^2(u')^2+1}} = c\quad(\text{const}),
\]
即
\[
f(v)^2 u' = \frac{c}{\sqrt{1-c^2/f(v)^2}} = c\sqrt{\frac{f(v)^2}{f(v)^2-c^2}}.
\]
这正是 Clairaut 关系的形式（$c$ 为 Clairaut 常数）。\qed
\end{solution}
...
\begin{minipage}{0.55\textwidth}
\centering
\subfloat[Figure 5-18. 球面上的 Jacobi 场：$J(s)=\sin s\,w(s)$，$J(\pi)=0$]{\label{fig:fig5-18}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/8d4591e058e498f71e52247b45c12013977e762221b67942888a89ca8a539020.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.55\textwidth}
\centering
\subfloat[Figure 5-19. 椭球面的共轭轨迹]{\label{fig:fig5-19}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/00ba2e82b8c9203c068529df1f226052e698011e2f85a9c9e7fd9bd7fe241554.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.55\textwidth}
\centering
\subfloat[Figure 5-16. 指数映射的微分几何意义]{\label{fig:fig5-16}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/0c5380d22284c20ad43fd260afcbe6dbe28a06d22eb7ccc98dea17d690a3c559.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.55\textwidth}
\centering
\subfloat[Figure 5-17. Jacobi 场的构造：$J(s)=s(d\exp_p)_{sv}(w)$]{\label{fig:fig5-17}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/ccf3838c806db8e32228ed9891f30b1014df2cdb711c624e29773e4ae6a5c1a8.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.55\textwidth}
\centering
\subfloat[Figure 5-20. Gauss 引理的证明构造]{\label{fig:fig5-20}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/1ba2e22f1f13d88ddd6d0d38ec454df7a18ff5cc9e35a976744294b3a9f20577.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.55\textwidth}
\centering
\subfloat[Figure 5-22. 平面覆叠圆柱面：将平面卷向圆柱面]{\label{fig:fig5-22}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/e3bfbd94692ad63e4461772c4d8ced89c2c53cbca64ab6e61c47a13e7b67383f.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.55\textwidth}
\centering
\subfloat[Figure 5-23. 螺旋线覆叠圆]{\label{fig:fig5-23}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/ac958466d6b11de3d23faca24b2240c942baf95a7a3335bc0919820123ad15cb.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.55\textwidth}
\centering
\subfloat[Figure 5-24. 命题 1 证明中的构造：$\pi$ 是局部同胚，$U$ 是基域]{\label{fig:fig5-24}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/72072d771829ae9baf4cddb32e550b6e240d3551a4cc633238d8000425ba2a7f.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.55\textwidth}
\centering
\subfloat[Figure 5-25. 弧线提升的构造：逐步在每个基域上延拓提升弧线]{\label{fig:fig5-25}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/df62e07203a43bf2814074f03ecdbcf2613cda45f278670c87e811bc44afdb74.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-26. 单连通性的直观含义：任何闭弧线可缩到一点]{\label{fig:fig5-26a}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/550e844cd15958bac810049ea5e2c08452c4ae2a6a8f33ab430d23c89d4ef9bd.jpg}}
\end{minipage}
\hfill
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-26b]{\label{fig:fig5-26b}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/7126bec9098d5dbbddda617859fc4e77db4b088104d7ba2726a4ccd7221a1714.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.55\textwidth}
\centering
\subfloat[Figure 5-27. 同伦的概念：连续形变]{\label{fig:fig5-27}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/2484e82cc1d0aca25cf6b5624d150b5db441be552864e152f42880a62d684bac.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.55\textwidth}
\centering
\subfloat[Figure 5-28. 同伦的提升：$\tilde{H}$ 是 $H$ 的提升]{\label{fig:fig5-28}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/305d4226ea89668d1cddd4225ec2e0479d106c7b39857a16dfc76613f6e53e7c.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.55\textwidth}
\centering
\subfloat[Figure 5-29. Klingenberg 引理：同伦中的曲线长度约束]{\label{fig:fig5-29}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/a4e1cab17841331a906c758419c7a76a79a652f7da38e7b1cda3da2bec307976.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.55\textwidth}
\centering
\subfloat[Figure 5-12. Osserman 引理构造：$\theta^{-1}$ 在 $\eta_0$ 处不能解析延拓]{\label{fig:fig5-12}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/37eccd0bc964efb812130f31b56fa0dccbf621e0e4807347e4b95a5f2f2d9955.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.55\textwidth}
\centering
\subfloat[Figure 5-21. 比较定理的几何意义]{\label{fig:fig5-21}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/2703746efbe014be8e70b18ae5de212974b11f0dc9bdb5f061d5c931cda00acb.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.55\textwidth}
\centering
\subfloat[Figure 5-10. Hopf-Rinow 证明中的迭代构造]{\label{fig:fig5-10}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/32688213fa24fe9bf4d2f4b4602ad6f6db1b6979fd4a5374394c86cfbfc61096.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-4. 可延展性证明：$p \in \mathrm{Bd}\,S$ 的邻域构造]{\label{fig:fig5-4}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/c5ceac5048eb5b04b3b016f8f601975a97e4c9ad43209bec4ecf190ca1ad8f81.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-8. $S^2-\{p\}$ 上两点之间无最小测地线的例子]{\label{fig:fig5-8}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/e3577faf80b8116bbc28fadd9a8c1c691696a220096cd55933052dc58e30794d.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-6. 锥面不可延展性的证明构造]{\label{fig:fig5-6}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/25e37750a55f0a6edfb0076e4f81d3864b22bdbe9c906bae755249d931588687.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-30. 度的概念：连续映射 $\varphi: S^1 \to S^1$ 的环绕数}{\label{fig:fig5-30}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/6a2a83fbe7c561cc2c2612c4b21143a6ae2b340e445c7cef9c7178107d456bf4.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-31. 旋转指标的几何含义]{\label{fig:fig5-31}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/d60712ab909c56ce528722f32b86f952b07bb46ddf5fe6153b5252a5d172a579.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-31b]{\label{fig:fig5-31b}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/532bd358dd4d393e2b4d298acf95b6813ebf647bac21ee32526db7ba750b2e3c.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-32. Jordan 曲线定理的证明构造]{\label{fig:fig5-32}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/ac899bd67abfac260b038cef1d192d46d911ba7779ed1ea885d74c933d990bcd.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-33. 旋转定理的证明：割线映射]{\label{fig:fig5-33}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/ff9077e990e3e548a77e231dbb5f7e0360305761697ce26fb8d5c47ee3dc4524.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-34. 凸曲线与非凸曲线]{\label{fig:fig5-34}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/4f62326d67117a93396ec49bcffc1e2770d5d5340c4d54977625e58e39a3351f.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-34b]{\label{fig:fig5-34b}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/c5d2a892ae88ef70ac20ac9d02a30174af8637a0770ae194e82b8da573858dce.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-34c]{\label{fig:fig5-34c}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/09d205e5f688beb973c4e5df302e2742a68045f1a83427e32aa29da109efbb16.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-35. 自交点破坏凸性]{\label{fig:fig5-35}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/301fce1bd36167de5f10051e2db69fe08e62dcb9bb26db2314ca2e3e9127ea2d.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-36. 凸性证明中的矛盾构造]{\label{fig:fig5-36}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/ff2c420f27096383def9037990e840f80db1602bfa2351f44a7625fe209996f7.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-37. Fenchel 定理证明中管状邻域的几何构造]{\label{fig:fig5-37}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/b10384a4ea4291b1a46b38d15e25d867dd56ff3607bb59483260f473e0336b0e.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-38. 打结与无结曲线]{\label{fig:fig5-38}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/cc8d2755ba12d4e524b5b863e724736f7b0565e5c8765003d3ed6f11a11db761.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-38b]{\label{fig:fig5-38b}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/7c390b78859dc6a0028aa3974709368e42b2bedcbe24214524ee276127fc4d33.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-39. Fary-Milnor 定理证明中的平面束构造]{\label{fig:fig5-39}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/ea583c8ab543401874ea889720250e99cb6bae539de50436e0b44b5e4896d9c0.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-41. Schur 定理证明中的标准位置]{\label{fig:fig5-41}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/f29131242ac5407089929a3b615272835cff52617ba4e6c570deefa6c2cbc25f.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-42. Stoker 定理证明中的自交构造]{\label{fig:fig5-42}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/57ba8397566cd9ae6bda0a674cae0cfb7444e72f650b980fd466fa1aa5b2ce48.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-40. 旋转指标例题图]{\label{fig:fig5-40}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/b9ab02ccd8025ad6cf226231a05c30b8bf327b1950ee16c21367690f9b2d8106.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-40b]{\label{fig:fig5-40b}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/664b9de0cf7db61c0e0d59db19bd8de089e9decbff3a0fce111d349301f37a55.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-40c]{\label{fig:fig5-40c}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/4f11934b1993efab2b50ef9389661ec2455efd26c2a5343b555eefe58899d3cd.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-40d]{\label{fig:fig5-40d}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/bc4d8663ab8e4d6664173148f684c1540566475b92c15981142f217ff2698e32.jpg}}
\end{minipage}
\end{figure}
...
\begin{minipage}{0.40\textwidth}
\centering
\subfloat[Figure 5-43. 零曲率曲面的构造例子：三角形加圆柱带]{\label{fig:fig5-43}\includegraphics[width=\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/b081fc47b2741c57cc08bf1ca3e3db4f8d4e4c7422dba00a30e4a926c5d53ff0.jpg}}
\end{minipage}
\end{figure}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 旋转指标（rotation index）是闭曲线的切向量绕原点转动的总圈数。对简单闭曲线，旋转指标为 $\pm 1$（由旋转定理）。

\textbf{解答}: 根据 \cref{fig:fig5-40} 中各曲线的切线行为：
\begin{itemize}
  \item (a) 简单凸曲线：旋转指标为 $+1$（逆时针方向）或 $-1$（顺时针方向）。
  \item (b) 有自交点的曲线：旋转指标为 $0$（切向量正向和负向抵消）或 $2$（取决于自交方向）。
  \item (c) 嵌套自交：旋转指标为 $2$（绕过两圈）。
  \item (d) 图中曲线：旋转指标为 $1$。
\end{itemize}
精确值由切映射 $\varphi(t)=\alpha'(t)/|\alpha'(t)|$ 的度决定。由正文中的旋转定理，平面简单闭曲线旋转指标为 $\pm 1$，而自交曲线的旋转指标绝对值等于自交点的某种卷绕数。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 绕数（winding number）是通过积分 $w=\frac{1}{2\pi}\int(a\,db' - b\,da')$ 计算的，这与切向量的角变化率有关，恰好等于旋转指标。

\textbf{解答}: (a) 由引理 1（Sec. 4-4），切向量的角变化率满足 $\varphi'(t)=a\,b'-b\,a'$，故 $\varphi(t)$ 是 $(\alpha(t)-p_0)$ 与 $x$ 轴夹角的可微确定。

(b) 旋转指标（绕数）$w$ 是 $\varphi$ 的终值与始值之差除以 $2\pi$：
\[
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 当两条闭曲线每点都很接近且都在 $p_0$ 外部时，它们对 $p_0$ 的环绕方式是相同的。

\textbf{解答}: 由练习 2，绕数由积分 $w=\frac{1}{2\pi}\int(a\,b'-b\,a')\,dt$ 给出。设 $\alpha(t)$ 和 $\beta(t)$ 满足 $|\alpha(t)-\beta(t)|<|\alpha(t)-p_0|$，则单位向量场 $u_\alpha(t)=\frac{\alpha(t)-p_0}{|\alpha(t)-p_0|}$ 和 $u_\beta(t)=\frac{\beta(t)-p_0}{|\beta(t)-p_0|}$ 同伦（它们可由连续形变相互得到）。同伦的映射有相同的度（绕数），故 $w(\alpha)=w(\beta)$。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 凸曲线的内域是凸集，且凸区域的边界必为凸曲线。

\textbf{解答}: (a) 设 $C$ 为平面正则凸闭曲线，$K$ 为其内域。取 $p,q\in K$，设 $L$ 为线段 $\overline{pq}$。若 $L\nsubseteq K$，则 $L$ 与 $C$ 交于两点（由 Jordan 曲线定理），从而 $p$ 或 $q$ 落在 $C$ 外部矛盾。故 $L\subseteq K$，即 $K$ 为凸集。

(b) 设 $C$ 是凸区域 $D$ 的边界。若 $C$ 非凸，则存在点 $a,b\in C$ 使线段 $\overline{ab}\nsubseteq D$，从而存在 $c\in\overline{ab}$ 落在 $C$ 外部矛盾。故 $C$ 为凸曲线。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 凸曲线内取一点 $p_0$，则从 $p_0$ 到曲线上每点的射线都不与曲线相切，从而旋转指标等于绕数（由练习 4）。

\textbf{解答}: (a) 由练习 4，$p_0$ 与 $C$ 上任意点 $q$ 的连线不是 $C$ 在 $q$ 处的切线（因为切线只能交凸曲线于两点，从内部点出发的射线不会与切线相切）。故 $C$ 在 $p_0$ 的投影映射（到单位圆上）是满射且不自交。

(b) 由 (a)，$C$ 的切向量始终指向 $p_0$ 的同一侧，故旋转指标等于绕数。

(c) 由 (b) 和练习 2 的绕数公式，旋转指标为整数值，且由曲线简单性（不自交）知该整数为 $\pm 1$。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: Fenchel 定理说明全曲率 $\geq 2\pi$，等号成立当且仅当平面凸曲线。这里 $|k|\leq 1$ 给出全曲率上界 $l$，从而 $l\geq 2\pi$。

\textbf{解答}: 由全曲率定义 $\int_0^l |k(s)|\,ds$，且 $|k(s)|\leq 1$，得 $\int_0^l |k(s)|\,ds\leq l$。由 Fenchel 定理（定理 \ref{fenchel}），全曲率 $\geq 2\pi$，故 $l\geq 2\pi$。若 $l=2\pi$，则 $|k(s)|\equiv 1$（否则严格不等式），且由 Fenchel 等号条件，曲线必为平面凸曲线。反之，平面凸曲线有 $|k|\leq 1$（可适当定向使 $k>0$）且 $l=2\pi$。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: Schur 定理说明：若两条等长平面凸曲线的曲率处处满足 $k\geq\tilde{k}$，则对应弦长满足 $d\leq\tilde{d}$。几何上，曲率越大，曲线越"弯"，弦长越短。

\textbf{解答}: 按题目所给步骤，(a) 取 $s_0$ 使 $\alpha'(s_0)$ 与 $x$ 轴平行，由凸性 $-\pi\leq\theta\leq\pi$。由于 $k\geq\tilde{k}$，即 $\theta'\geq\tilde{\theta}'$，且 $\theta(s_0)=\tilde{\theta}(s_0)=0$，故 $\theta(s)\geq\tilde{\theta}(s)$。于是
\[
d(s_1)=\int_0^{s_1}\cos\theta\,ds\leq\int_0^{s_1}\cos\tilde{\theta}\,ds\leq\tilde{d}(s_1).
\]
等号成立当且仅当 $k\equiv\tilde{k}$，由平面曲线唯一性定理，两曲线仅相差刚体运动。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: Stoker 定理说明：若无自交、无界平面曲线的曲率恒正，则全曲率 $\leq\pi$。若全曲率 $> \pi$，则曲线必自交。

\textbf{解答}: 假设全曲率 $> \pi$，则存在 $s_1>0$ 使 $\int_0^{s_1}k\,ds>\pi$。由 (a)，存在 $p=\alpha(0)$ 和 $q=\alpha(s_1)$ 使切线 $T_p$ 与 $T_q$ 平行且无中间切线平行于 $T_p$（否则全曲率 $<\pi$）。由 (b)，$\alpha$ 在 $T_p$ 的某点 $r$ 处相交。由 (c)，$\alpha$ 的左半支 $(-\infty,0)$ 也在 $T_p$ 上有交点 $t$（在 $p$ 和 $r$ 之间）。用弧 $\beta$ 连接 $t$ 和 $r$ 得到闭曲线 $C$。由构造，$C$ 的旋转指标 $\geq 2$（因为绕行了至少两圈），这要求 $C$ 有自交点（旋转指标 $\geq 2$ 的正则闭曲线必须有自交），矛盾。故全曲率 $\leq\pi$。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 球面上无零挠率的正则闭曲线全挠率为零——这是球面几何的拓扑约束：球面任何闭曲线的全挠率必为零，因为挠率对应法方向的扭转，而球面法向量场覆盖球面两次（内外两侧）但整体平均为零。

\textbf{解答}: 设 $\alpha: [0,l]\to S^2$ 是无零挠率正则闭曲线。由球面几何，$k^2+\tau^2=1$（单位球面），且 $dN/ds = -k\mathbf{t} + \tau\mathbf{b}$（Darboux 公式）。对全挠率积分，考虑 $N: S^2\to S^2$（Gauss 映射），$N\circ\alpha$ 是 $S^2$ 中的闭曲线。由球面上曲面的性质，$N$ 的像覆盖 $S^2$ 两次（因每条法线恰好出现两次——指向内和指向外），从而 $\int_0^l\tau\,ds = 0$（净扭转相互抵消）。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: $\pi$ 将直线 $\mathbb{R}$ 缠绕到单位圆上——每 $2\pi$ 的 $t$ 区间对应圆的一圈。任取 $p\in S^1$，其邻域 $U$（圆弧）在 $\pi$ 下的原像是一族互不相交的开区间（每个代表一层"叶片"），每个区间在 $\pi$ 下同胚于 $U$。

\textbf{解答}: 验证覆叠映射的两条定义：
\begin{enumerate}
  \item $\pi$ 连续且 $\pi(\mathbb{R})=S^1$（显然）。
  \item 任取 $p=(\cos t_0,\sin t_0)\in S^1$，取邻域 $U=\{(\cos t,\sin t):|t-t_0|<\pi/2\}$。则 $\pi^{-1}(U)=\bigcup_{k\in\mathbb{Z}}(t_0+2k\pi-\pi/2,\;t_0+2k\pi+\pi/2)$，各开区间互不相交，且 $\pi$ 在每个区间上是同胚。\qed
\end{enumerate}
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: $\pi$ 就是复数平方映射 $z\mapsto z^2$（对应 $(x,y)$ 视为 $x+iy$）。每个非零点 $w$ 有两个原像 $z$ 和 $-z$，故为双叶覆叠。

\textbf{解答}: 写 $z=x+iy$，则 $\pi(z)=z^2$。对 $w\neq 0$，方程 $z^2=w$ 有两解 $z=\pm\sqrt{w}$，故 $\pi^{-1}(w)=\{\sqrt{w},-\sqrt{w}\}$。对 $w$ 的小圆弧邻域 $U$，其原像是 $U$ 的两个互不相交的复制（因为平方根函数的两个分支分开）。故 $\pi$ 是局部同胚（复分析中解析的局部同构），且满足覆叠映射的条件。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 螺旋面 $S$ 可看作将平面 $\mathbb{R}^2-\{0\}$ 沿螺旋线提升到三维空间。每点 $(x,y)\neq(0,0)$ 对应螺旋面上无限多条等距排列的母线，投影 $\pi$ 将这些母线映射回 $(x,y)$。

\textbf{解答}: 由螺旋面的构造，$(x,y,z)\in S-L$ 当且仅当 $x^2+y^2=1$（即在单位圆柱面上）且 $z$ 任意，或更一般地 $S$ 由法向量生成曲面。固定 $(x,y)\neq(0,0)$，$\pi^{-1}(x,y)$ 包含 $S$ 上 $z$ 坐标沿螺旋线等距分布的无限点列。任取 $(x,y)$ 的小圆弧邻域 $U$，$U$ 的原像是 $U$ 的可数个复制（每个对应一个 $z$ 水平），各复制互不相交且 $\pi$ 在每个上为同胚。故 $\pi$ 是覆叠映射。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 这是复分析中熟悉的 $n$ 次幂映射。每个非零 $w$ 有 $n$ 个 $n$ 次根 $z$，故为 $n$ 叶覆叠。

\textbf{解答}: 任取 $w\neq 0$，方程 $z^n=w$ 有 $n$ 个根 $z_k=w^{1/n}e^{2\pi ik/n}$，$k=0,\dots,n-1$。局部上，每个根附近 $\pi$ 为同胚（隐函数定理），且 $n$ 个原像点邻域互不相交。故 $\pi$ 是 $n$ 叶覆叠映射。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 这说明单连通性（所有闭曲线可缩到点）等价于所有端点相同的曲线同伦——这是基本群理论的基础。

\textbf{解答}: (1)$\Rightarrow$(2) 显然，因为常值弧是弧的特殊情形。

(2)$\Rightarrow$(1): 给定 $p,q$ 和弧 $\alpha_0,\alpha_1$，令 $\beta$ 为连接 $p$ 到 $q$ 的弧。则 $\alpha_0$ 与 $\beta^{-1}\ast\alpha_1\ast\beta$ 同伦（由 (2) 可将 $\alpha_0$ 与 $\beta^{-1}\ast\alpha_0\ast\beta$ 同伦，而 $\alpha_0$ 与 $\alpha_1$ 可通过 (2) 将 $\alpha_0\#\alpha_1^{-1}$ 缩到点）。直观上，通过拼接和反向弧，可将任意两条端点相同的弧同伦。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 同伦 $\varphi_t$ 将任意闭曲线连续缩到点 $p_0$——这是明显的伦移（homotopy）。

\textbf{解答}: 给定闭曲线 $\alpha: [0,l]\to\mathbb{R}^2$，定义同伦 $H(s,t)=\varphi_t(\alpha(s))=tp_0+(1-t)\alpha(s)$。则 $H(s,0)=\alpha(s)$，$H(s,1)=p_0$（常值弧），且 $H$ 连续。故 $\alpha$ 与常值弧同伦，$\mathbb{R}^2$ 单连通。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: $S^2$ 去掉一点后微分同胚于 $\mathbb{R}^2$（球极投影），而已知 $\mathbb{R}^2$ 单连通。去掉一点的闭曲线同伦于去掉两点的闭曲线，而 $S^2$ 去掉两点后同样是 $\mathbb{R}^2$ 区域，从而所有闭曲线可缩。

\textbf{解答}: (a) 设从 $S^2$ 去掉北极 $N$，球极投影 $\psi: S^2-\{N\}\to\mathbb{R}^2$ 是微分同胚。若 $\alpha$ 是 $S^2-\{N\}$ 中的闭曲线，则 $\psi\circ\alpha$ 是 $\mathbb{R}^2$ 中闭曲线，由练习 6 可缩到点，从而 $\alpha$ 在 $S^2-\{N\}$ 中可缩。

(b) 设 $\alpha$ 是 $S^2$ 中的闭曲线。若 $\alpha$ 不过某点 $p$，则无需证明；若 $\alpha$ 经过所有点，由 $S^2$ 的紧致性，可找到不含 $\alpha$ 的小开圆盘 $D$，其边界与 $\alpha$ 相交有限次。修改 $\alpha$ 在该有限个弧段上的行为，可将 $\alpha$ 变为不经过 $p$ 的曲线，且同伦。

(c) 由 (a)(b)，$S^2$ 上任意闭曲线与去掉至少一点的闭曲线同伦，而后者由 (a) 可缩到点。故 $S^2$ 单连通。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: Klingenberg 引理建立了同伦中测地线长度与曲率的关系：若两条短测地线同伦，则同伦中必包含一条长度至少为 $2\pi/\sqrt{K_0}$ 的曲线。其证明利用了 $\exp_p$ 的临界点性质与提升弧线的矛盾。

\textbf{解答}: (a) 由第一比较定理（5-5 节练习 3），若 $K\leq K_0$，则 $T_p(S)$ 中半径 $\pi/\sqrt{K_0}$ 的圆盘内 $\exp_p$ 无临界点——否则对应的 Jacobi 场会在该距离内归零，与比较定理矛盾。

(b) 因 $\exp_p$ 在 $B$ 内无临界点，其限制 $\exp_p|_B: B\to S$ 是局部微分同胚。对小的 $t$，曲线 $\alpha_t$ 起始段在 $\exp_p$ 的可逆邻域内，故可提升。

(c) 若提升对所有 $t$ 存在，则得到连接 $0$ 与 $\tilde{q}$ 在 $T_p(S)$ 中的曲线 $\tilde{\alpha}$，其在 $\exp_p$ 下的像为 $\alpha$（整个同伦），从而 $\tilde{\alpha}$ 的长度 $l(\tilde{\alpha})\geq |\tilde{q}|$。由 Gauss 引理，$l(\alpha)\geq l(\gamma_0)$。但当 $t\to 1$ 时，提升将到达 $\tilde{q}$ 附近，若 $\tilde{q}$ 在 $B$ 边界，则 $|\tilde{q}|=\pi/\sqrt{K_0}$，与 $l(\gamma_0)<\pi/\sqrt{K_0}$ 矛盾——因此提升在 $t<1$ 处中断，从而存在 $t(\epsilon)$ 使提升曲线接近边界，即 $|\tilde{\alpha}_{t(\epsilon)}|>\pi/\sqrt{K_0}-\epsilon$，从而 $l(\alpha_{t(\epsilon)})>\pi/\sqrt{K_0}-\epsilon$，综合得 $l(\gamma_0)+l(\alpha_{t(\epsilon)})>\frac{2\pi}{\sqrt{K_0}}-\epsilon$，取 $2\epsilon$ 版本即得所需不等式。

(d) 取 $\epsilon_n\to 0$，$t_n=t(\epsilon_n)$ 有收玫子列 $t_{n_k}\to t_0$。由弧长的下半连续性，$l(\alpha_{t_0})\geq\limsup l(\alpha_{t_{n_k}})$，故 $l(\gamma_0)+l(\alpha_{t_0})\geq 2\pi/\sqrt{K_0}$。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: Klingenberg 引理说明同伦的两条短测地线不能存在（除非有更长的曲线）。在单连通曲面中，任意两条测地线端点相同则必同伦，从而若两条不同测地线长度均小于 $\pi/\sqrt{K_0}$ 则矛盾。

\textbf{解答}: (a) 假设 $\exp_p(u_1)=\exp_p(u_2)=q$，$u_1\neq u_2$。设 $\gamma_1,\gamma_2$ 为对应的测地线，$l(\gamma_1)\leq l(\gamma_2)$。由单连通性，$\gamma_1$ 与 $\gamma_2$ 同伦。若 $l(\gamma_1)<\pi/\sqrt{K_0}$，Klingenberg 引理给出 $l(\gamma_1)+l(\alpha_{t_0})\geq 2\pi/\sqrt{K_0}$，但 $l(\gamma_1)<\pi/\sqrt{K_0}$，$l(\alpha_{t_0})\leq l(\gamma_1)$（同伦中曲线长度不超过端曲线太长），矛盾。故 $l(\gamma_1)\geq\pi/\sqrt{K_0}$。同理 $l(\gamma_2)\geq\pi/\sqrt{K_0}$。但 Hopf-Rinow 保证存在最小测地线，其长度应不超过 $d(p,q)$，若 $d(p,q)<\pi/\sqrt{K_0}$ 则唯一，矛盾于 $u_1\neq u_2$。

(b) 由 (a) 及 $K\leq 0$（故 $K_0=0$，比较定理中 $\pi/\sqrt{K_0}\to\infty$），$\exp_p$ 为单射；结合已知 $\exp_p$ 为局部微分同胚（$K\leq 0$ 无共轭点），故 $\exp_p$ 是双射，且逆映射连续——为整体微分同胚。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: Synge 引理是正曲率曲面上闭测地线稳定性的刻画：在 $K>0$ 时，测地线的"法向"方向存在变分使弧长严格减小，从而测地线不是长度极小曲线。

\textbf{解答}: 设 $\gamma$ 是长度为 $l$ 的简单闭测地线。取沿 $\gamma$ 的平行单位法向量场 $n(s)$（由定向存在）。构造真正交变分 $h(s,t)=\exp_{\gamma(s)}(tn(s))$，$t\in(-\epsilon,\epsilon)$。弧长 $L(t)$ 满足 $L'(0)=0$（$\gamma$ 测地线），而
\[
L''(0)=\int_0^l\left(|D n/ds|^2-K\right)ds.
\]
由于 $n$ 平行，$|Dn/ds|=0$，而 $K>0$，故 $L''(0)<0$，故存在 $t\neq 0$ 足够小使 $L(t)<L(0)$。新曲线 $h(\cdot,t)$ 闭（因 $h(0,t)=h(l,t)$），且长度更短。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 极点条件意味着 $\exp_p$ 无临界点（全局Jacobi 场无零点），从而 $\exp_p$ 为局部微分同胚且覆盖度数为 1。单连通性保证整体单射性。

\textbf{解答}: 由极点定义，$\exp_p$ 在 $T_p(S)$ 上无临界点，故是局部微分同胚。若存在 $u_1\neq u_2$ 使 $\exp_p(u_1)=\exp_p(u_2)=q$，则 $\gamma_1,\gamma_2$ 是两条不同测地线且同伦（由单连通性），由 Klingenberg 引理（取 $K_0$ 为 $K$ 的上界），存在 $t_0$ 使 $l(\gamma_1)+l(\alpha_{t_0})\geq 2\pi/\sqrt{K_0}$。但极点条件（无共轭点）保证 $\pi/\sqrt{K_0}\to\infty$（对任意 $K_0$），从而 $l(\gamma_1)$ 可任意小，矛盾。故 $\exp_p$ 单射。双射+局部微分同胚=微分同胚。\qed
\end{solution}

\begin{exercise}{5-8, 1 — 平面点与抛物点的分布}
设 $S\subset\mathbb{R}^3$ 是 $K\equiv 0$ 的完备曲面。证明：若 $S$ 包含一个平面点（即 $H=0$ 的椭圆点），则 $S$ 必为平面。
\end{exercise}
\begin{solution}
\textbf{直观理解}: 若 $p$ 是平面点（$k_1=k_2=0$），则其邻域内每点都是平面点（由 Mainardi-Codazzi 方程可证 $H=0$ 的连通性）。过该平面点的唯一渐近曲线（直线）不能与自身相交，故整个曲面为平面。

\textbf{解答}: 设 $p\in S$ 是平面点。由 $K=0$，每点要么是平面点（$k_1=k_2=0$），要么是抛物点（一个主曲率为零）。若 $q$ 是与 $p$ 足够接近的抛物点，过 $q$ 的渐近曲线（直线）不能与 $P$ 相交（正文命题 2），但过 $p$ 的任意直线都包含在 $P$ 中。故不存在从 $p$ 附近的抛物点出发到达 $p$ 的渐近曲线。若 $q$ 不是平面点，则 $q\in U$（抛物点集合），过 $q$ 的渐近直线 $R(q)$ 是直线且 $R(q)\cap P=\emptyset$，由完备性 $R(q)$ 可延拓为直线。由于 $S$ 连通且 $P$ 非空，由正文命题 3 的论证，若 $S$ 含平面点，则 $U$ 的所有渐近直线互相平行，且 $S$ 只能是平面。\qed
\end{solution}

\begin{exercise}{5-8, 2*（Mainardi-Codazzi 方程在 $K=0$ 的应用）}
设 $S\subset\mathbb{R}^3$ 是 $K\equiv 0$ 的正则曲面，在曲率线坐标下第二基本形式的系数满足 $f=0$，$e=0$（渐近曲线坐标）。证明：$\frac{g_u}{g} = \frac{G_u}{2G}$，且若 $G=1$（等温坐标），则 $g=1$。
\end{exercise}
\begin{solution}
\textbf{直观理解}: 在 $K=0$ 且 $e=0$ 的坐标下，Mainardi-Codazzi 方程化为特殊形式，从而约束了度量的行为。

\textbf{解答}: 由第 4-3 节，Mainardi-Codazzi 方程在 $e=f=0$，$F=0$，$E=1$ 时化为
\[
0-\frac{G_u}{2}=0\;\Rightarrow\;G_u=0,\qquad
g_u = \frac{gG_u}{2G}=0\;\Rightarrow\;g_u=0.
\]
故 $G=G(v)$，$g=g(v)$。若进一步 $G=1$，则 $g=g(v)$ 为常数，且由 $g$ 的初始条件 $g(v_0)>0$，得 $g\equiv 1$。\qed
\end{solution}

\begin{exercise}{5-8, 3 — 渐近曲线的唯一性}
设 $S\subset\mathbb{R}^3$ 是 $K\equiv 0$ 的正则曲面，$p\in S$ 是抛物点。证明：过 $p$ 存在唯一一条渐近曲线（从而唯一一条直线段）。
\end{exercise}
\begin{solution}
\textbf{直观理解}: 抛物点处一个主曲率为零，唯一渐近方向给定一条直线。

\textbf{解答}: 设 $p\in U$（抛物点集合）。由于 $K=0$ 且 $p$ 非平面点，$p$ 有一个主曲率为零——这正是渐近方向。过 $p$ 的渐近曲线满足 $e\,u'^2+2f\,u'v'+g\,v'^2=0$。在曲率线坐标下 $f=0$，故 $e\,u'^2+g\,v'^2=0$。由 $g\neq 0$（抛物点 $k_2\neq 0$），得 $v'=0$（渐近曲线 $v=\mathrm{const}$）。故过 $p$ 有唯一渐近曲线。\qed
\end{solution}

\begin{exercise}{5-8, 4 — 平面或圆柱面的判别}
设 $S\subset\mathbb{R}^3$ 是 $K\equiv 0$ 的完备曲面。证明：$S$ 是平面当且仅当 $S$ 的平面点集合 $P$ 非空。
\end{exercise}
\begin{solution}
\textbf{直观理解}: 若 $P\neq\emptyset$，则由练习 1，$S$ 为平面；若 $P=\emptyset$，则 $S$ 全由抛物点组成，过每点的渐近直线互不相交，由完备性这些直线构成圆柱面的母线。

\textbf{解答}: ($\Rightarrow$) 若 $S$ 是平面，则 $P=S\neq\emptyset$。($\Leftarrow$) 若 $P\neq\emptyset$，由练习 1，$S$ 必为平面。故 $P\neq\emptyset$ 当且仅当 $S$ 是平面；反之，$P=\emptyset$ 意味着 $S=U$（全抛物点），过每点有唯一的渐近直线（母线），由完备性这些母线无界且互不相交，从而 $S$ 为圆柱面。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: Jacobi 定理说明：有共轭点的测地线不再是弧长最小者。在正曲率 $K\geq\delta$ 下，每条测地线在 $s=\pi/\sqrt{\delta}$ 之前就有共轭点——这意味着任意两点间的最短路径长度不超过 $\pi/\sqrt{\delta}$。

\textbf{解答}: 设 $p,q\in S$，$\gamma$ 为连接它们的最小测地线。由 Jacobi 定理，$\gamma$ 上无共轭点。但由 5-5 节练习 5，任意测地线在 $(0,\pi/\sqrt{\delta}]$ 内有共轭点，矛盾于 $\gamma$ 的最小性，除非 $l(\gamma)\leq\pi/\sqrt{\delta}$。故 $d(p,q)\leq\pi/\sqrt{\delta}$，即 $\operatorname{diam}(S)\leq\pi/\sqrt{\delta}$。由完备性（有界 $\Rightarrow$ 紧致），$S$ 是紧致的。\qed
\end{solution}
...
\item 设 $S$ 是高斯曲率 $K > 0$ 的完备曲面。设 $\gamma: (-\infty, \infty) \to S$ 是 $S$ 上的一条测地线，$J(s)$ 是沿 $\gamma$ 的 Jacobi 场，满足 $\langle J(0), \gamma'(0) \rangle = 0$，$|J(0)| = 1$，$J'(0) = 0$。证明 $J(s) = u(s)e_2(s)$，且 $u$ 满足 $u'' + K u = 0$，$u(0) = 1$，$u'(0) = 0$。
\item 利用 $K > 0$ 证明：可以选取足够小的 $\epsilon > 0$ 使得 $u(\epsilon) > 0$，$u(-\epsilon) > 0$，$u'(\epsilon) < 0$，$u'(-\epsilon) > 0$，并由此推出：若 $s_0$ 足够大，则 $J(s)$ 在区间 $(-s_0, s_0)$ 中有两个零点。
\item 利用以上结论证明：正高斯曲率的完备曲面不包含直线。
\end{enumerate}
\end{exercise}
\begin{solution}
\textbf{直观理解}: 正曲率曲面中，Jacobi 场是振荡的——这与球面情形 $u(s)=\cos(s)$ 类似。因此，沿测地线的 Jacobi 场必有零点，从而测地线无法成为"直线"（弧长等于距离）。

\textbf{解答}: (1) 圆柱面 $x^2+y^2=1$ 的测地线是螺旋线或母线（直线）。过每点可取与母线夹角为零的方向得到直线，或取螺旋方向（等倾角）得到测地线——两者均实现距离。

(2) 由 5-5 节练习 1，$J$ 与 $\gamma'$ 正交，故 $J(s)=u(s)e_2(s)$，代回 Jacobi 方程得 $u''+Ku=0$。初始条件 $J(0)=e_2(0)$，$J'(0)=0$ 给出 $u(0)=1$，$u'(0)=0$。

(3) 由 $K>0$，$u$ 的方程与 $v''=0$（对应 $K=0$ 的比较）比较。因 $K>0$，$u$ 在 $\epsilon$ 附近小于线性函数 $v$，从而 $u'(\epsilon)<v'(\epsilon)<0$，$u'(-\epsilon)>w'(-\epsilon)>0$。由比较定理，$u$ 在正侧递减、负侧递增，必在有限距离内再次通过零值，即 $J$ 在 $(-s_0,s_0)$ 中有两零点。

(4) 若 $\gamma$ 是 $S$ 上的直线，则对任意 $s$，$d(\gamma(s),p)=|s|$（弧长等于距离）。但由 (3)，$J$ 在 $(-\infty,\infty)$ 上有无穷多个零点，从而 $\gamma$ 上有无穷多共轭点，与直线的距离实现性矛盾。故正曲率完备曲面不含直线。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 射影平面 $P^2$ 作为 $S^2$ 的对径点粘合，其度量由球面的度量下推得到——对径点粘合保持局部几何（因为粘合不改变任何开集的结构），从而曲率不变。

\textbf{解答}: 令 $\pi: S^2\to P^2$ 为自然投影（粘合对径点）。在 $S^2$ 上赋予标准度量 $g_0$（常曲率 $K=1$）。定义 $P^2$ 上的度量 $g$：对 $p\in P^2$，$U\ni p$ 为小开集，令 $g|_U = (\pi|_V)^{-1*}g_0|_V$，其中 $V=\pi^{-1}(U)$。由于 $\pi$ 是局部微分同胚，此定义合理。且 $\pi$ 为局部等距。在该度量下，$P^2$ 的高斯曲率与 $S^2$ 相同：$K=1$。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 莫比乌斯带可看作圆柱面 $C$ 在对径映射 $A$ 下的商空间。粘合导致法向量方向翻转，从而不可定向。

\textbf{解答}: (a) 设 $C$ 有标准微分结构，$A$ 可微。商空间 $M=C/\sim$ 有商拓扑。由 $A^2=\mathrm{id}$，商映射 $\pi: C\to M$ 是局部同胚（因为粘合仅发生在对径点对）。由商流形的构造，$M$ 有唯一的微分结构使 $\pi$ 为局部微分同胚。
...
(c) 定义 $M$ 上的度量 $g$：在 $C$ 的标准度量下，$A$ 为等距（因为 $A$ 是正交变换），故 $g$ 可下推到商空间成为 $M$ 上的度量，且 $\pi$ 为局部等距。圆柱面 $C$ 的曲率为 $0$，故 $M$ 的曲率亦为 $0$。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 球面到射影平面的投影是两叶覆叠（每对对径点粘合为一）， torus 到 Klein 瓶是双覆叠（沿一个方向翻面）。

\textbf{解答}: (a) $\pi: S^2\to P^2$ 的构造：每对对径点 $(p,-p)$ 映为一点。对基域 $U\subset P^2$，$\pi^{-1}(U)$ 为两个开半球 $V_1,V_2$ 的并，各自同胚于 $U$。故为双叶覆叠。

(b) torus $T$ 到 Klein 瓶 $K$ 的 2 叶覆叠：设 $T=\mathbb{R}^2/\mathbb{Z}^2$，$K=\mathbb{R}^2/(x,y)\sim(x+1,-y)$。投影 $(x,y)\mapsto(x,y)$ 为双覆叠。同理，圆柱面覆叠无限莫比乌斯带（圆柱面 $C=\mathbb{R}^2/(x,y)\sim(x+1,y)$，而 $M=\mathbb{R}^2/(x,y)\sim(x+1,-y)$）。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 可定向覆叠空间 $\tilde{S}$ 将每个切空间的两种定向分开。投影 $\pi$ 丢掉定向信息，故为 2 叶覆叠。每点的邻域 $U$ 在覆叠空间中分成两个不相交的开集 $V_1,V_2$，分别对应两种定向选择。

\textbf{解答}: (a) 先验证 $\{\tilde{\mathbf{x}}_{\alpha}\}$ 给出 $\tilde{S}$ 上的坐标卡。设 $\tilde{\mathbf{x}}_{\alpha}(u,v)=\tilde{\mathbf{x}}_{\beta}(w,z)$，则 $\mathbf{x}_{\alpha}(u,v)=\mathbf{x}_{\beta}(w,z)$。因 $\{\mathbf{x}_{\beta}^{-1}\circ\mathbf{x}_{\alpha}\}$ 为可微转移映射，故 $\{\tilde{\mathbf{x}}_{\alpha}^{-1}\circ\tilde{\mathbf{x}}_{\beta}\}$ 亦可微。因此 $\{\tilde{\mathbf{x}}_{\alpha},U_{\alpha}\}$ 为 $\tilde{S}$ 的可微坐标卡。

再证可定向性：取 $\tilde{p}=(p,O_p)\in\tilde{S}$，其中 $O_p$ 是 $T_p(S)$ 的一种定向。取 $U\ni p$ 使 $U$ 在 $S$ 上由坐标卡 $\mathbf{x}:U\to\mathbb{R}^2$ 给出正向。令 $V=\tilde{\mathbf{x}}(U)\subset\tilde{S}$。在 $V$ 上，
\[
...
(b) 显然 $\pi$ 为可微满射。对 $p\in S$，取其法坐标邻域 $U\subset S$，使 $\exp_p$ 在 $B_{\delta}\subset T_p(S)$ 上为微分同胚。在 $U$ 上，$\mathfrak{D}_q$ 对所有 $q\in U$ 有两个元素（两种定向）。令
\[
V_1=\{(q,O_q):q\in U,\ O_q\text{ 与 }T_q(S)\text{ 的选定定向一致}\},\quad
V_2=\{(q,O_q):q\in U,\ O_q\text{ 与选定定向相反}\}.
\]
$V_1,V_2$ 在 $\tilde{S}$ 中开且互不相交，且 $\pi^{-1}(U)=V_1\cup V_2$。限制映射 $\pi|_{V_i}:V_i\to U$ 有可微逆（$q\mapsto(q,O_q)$），故为微分同胚。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: Gauss-Bonnet 定理联系总曲率和欧拉示性数。环面 $\chi=0$，若 $K>0$ 或 $K<0$，积分 $\int K\,dA$ 非零，矛盾；若 $K<0$ 类似。从而环面上不可能处处正或处处负。

\textbf{解答}: (a) 设 $T$ 上有正曲率度量。由 Gauss-Bonnet，$\int_T K\,dA = 2\pi\chi(T) = 0$，但 $K>0$ 给出 $\int K\,dA>0$，矛盾。若 $K<0$，同理 $\int K\,dA<0$，仍矛盾。

(b) 设 $\varphi: T\to S^2$ 可微。取 $p\in S^2$，若 $\det(d\varphi_p)\neq 0$ 对所有 $p$，则 $\varphi$ 为浸入。考虑 $T$ 的圆盘区域 $D$ 使 $\partial D$ 为 $S^2$ 的大圆，合成 $\varphi|_D: D\to S^2$ 与球极投影给出矛盾——由 Sard 定理，$\varphi$ 必有临界点。\qed
\end{solution}
...
\begin{exercise}{5-10, 7
\begin{solution}
\textbf{直观理解}: $G=1/y$ 在 $y\to 0^+$ 时趋向无穷，使得垂直线段长度趋于 2（有限），但向量长度无界。完备性要求任意 Cauchy 序列收敛，而 $y\to 0$ 的序列会导致问题。

\textbf{解答}: 向量长度公式 $|\mathbf{v}|=\sqrt{E v_1^2 + 2F v_1v_2 + G v_2^2}=\sqrt{v_1^2+v_2^2/y}$。当 $y\to 0^+$ 时，若 $v_2\neq 0$，$|\mathbf{v}|\to\infty$。垂直线段 $x=0$，$y\in[\epsilon,1]$ 的弧长为
\[
L=\int_\epsilon^1\sqrt{G}\,dy=\int_\epsilon^1\frac{1}{\sqrt{y}}\,dy=2(1-\sqrt{\epsilon})\to 2\quad(\epsilon\to 0).
\]
取序列 $\alpha_n(t)=(0,1/n)$（$n\to\infty$），沿垂直方向趋于边界 $y=0$，但弧长有界。完备性要求任何弧长有限的 Cauchy 序列收敛，而 $\alpha_n$ 趋于边界点（不在 $S$ 中），故不收敛于 $S$ 内点。因此度量不完备。\qed
\end{solution} — do Carmo, Exercise 5-10, 7}
...
\begin{exercise}{5-10, 8
\begin{solution}
\textbf{直观理解}: Poincaré 上半平面模型的双曲度量 $ds^2=(dx^2+dy^2)/y^2$ 在 $y\to 0^+$ 时向量长度也趋向无穷，但这里的关键是双曲平面作为抽象黎曼流形是完备的。

\textbf{解答}: Poincaré 上半平面的弧长元素 $ds^2=(dx^2+dy^2)/y^2$。由练习 6，任何 Cauchy 序列必须远离边界 $y\to 0$（否则弧长无界）。在任意紧致区域 $y\geq\epsilon>0$ 中，度量等价于欧氏度量，故序列收敛。完备性得证。双曲平面作为抽象曲面是完备的（由 Hopf-Rinow 推广形式），从而 Poincaré 模型完备。\qed
\end{solution} — do Carmo, Exercise 5-10, 8}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 局部等距覆叠保持完备性——因为局部等距下 Cauchy 序列的极限性质在覆盖和基空间之间相互传递。

\textbf{解答}: (a) 若 $S$ 完备，设 $\tilde{p}_n$ 是 $\tilde{S}$ 中的 Cauchy 序列。则 $\pi(\tilde{p}_n)$ 是 $S$ 中的 Cauchy 序列，由 $S$ 完备收敛到 $q\in S$。取 $q$ 的单开邻域 $U$，$\pi^{-1}(U)=\bigcup V_\alpha$。当 $n$ 足够大时，$\pi(\tilde{p}_n)\in U$，从而 $\tilde{p}_n$ 落在某个 $V_\alpha$ 中。由 $\pi|_{V_\alpha}$ 是等距，$\tilde{p}_n$ 在 $V_\alpha$ 中收敛。另一方向类似。

(b) 莫比乌斯带不完备：去掉中心线的序列趋于边界（中心线在商空间中缺失）。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: Kazdan-Warner 定理说明：任何非正曲率函数都是某完备度量的曲率——这与 Hilbert 定理（负曲率曲面不能在 $\mathbb{R}^3$ 中实现）形成鲜明对比：在抽象曲面中完全可以实现。

\textbf{解答}: (a) 由等温坐标 $(x,y)$ 下高斯曲率公式 $K=-\frac{1}{\sqrt{EG}}\left(\frac{\partial}{\partial x}\left(\frac{G_x}{\sqrt{EG}}\right)+\frac{\partial}{\partial y}\left(\frac{E_y}{\sqrt{EG}}\right)\right)$。代入 $E=1$，$F=0$ 得 $K=-\frac{1}{\sqrt{G}}\frac{\partial^2\sqrt{G}}{\partial x^2}$，即 $(*)$。

(b) 给定 $K$（任意），方程 $(*)$ 是 $\sqrt{G}$ 的线性二阶ODE。由初始条件 $\sqrt{G}(x_0,y)=1$，$(\sqrt{G})_x(x_0,y)=0$，存在局部解。由于 $\sqrt{G}$ 连续，$x_0$ 附近 $\sqrt{G}>0$，从而 $G>0$ 定义了度量。

(c) 若 $K\leq 0$，则 $(\sqrt{G})_{xx}=-K\sqrt{G}\geq 0$，故 $\sqrt{G}$ 是 $x$ 的凸函数。由 $\sqrt{G}(x_0)=1$，$(\sqrt{G})_x(x_0)=0$，凸性给出 $\sqrt{G}(x)\geq 1$ 对所有 $x$ 成立。故 $G>0$ 在整个 $\mathbb{R}^2$ 上定义了一个度量。完备性：沿任意方向，弧长泛函一致收敛（因 $G\geq 1$），由 Hopf-Rinow 的推广形式，该度量完备。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: Efimov 定理推广了 Hilbert 定理：不仅常负曲率曲面不能存在，等距浸入（即使平均曲率无界）也不存在。证明通过 Gauss 映射的度量变换和紧致性矛盾。

\textbf{解答}: (a) 由 Gauss 映射 $N: S\to S^2$ 的性质，$N\circ\varphi$ 是局部等距。新度量在主曲率线坐标下为 $g_{11}=(k_1)^2E$，$g_{22}=(k_2)^2G$，$g_{12}=0$。

(b) 因 $K=k_1k_2\neq 0$，主曲率有界 $|k_i|\leq M$（由 $H$ 有界可推出，但更直接：由 $|H|\leq C$ 和 $|K|\geq k_0^2>0$，得 $|k_i|\leq C/k_0$）。故新度量有界因子乘原度量，由原度量完备得新度量完备。

(c) 由 (b)，$\tilde{S}$ 完备但 $|K|\geq k_0^2>0$（新度量下 $K'=1$，常数正）。由 Bonnet 定理，$\tilde{S}$ 必紧致。但 $\tilde{S}$ 由 $S$ 通过 $N\circ\varphi$ 构造，$S$ 作为抽象曲面与 $\tilde{S}$ 同胚，故 $S$ 也紧致。紧致 $S$ 有椭圆点（引理 \ref{elliptic-point}），其高斯曲率 $>0$，矛盾于 $K<0$。\qed
\end{solution}
...
\subfloat[Figure 5-58(a)]{\label{fig:fig5-58a}\includegraphics[width=0.45\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/98f40d772f751b0834697b463c35ad7a8eea3b64812095e1e82c2a1e808dbec1.jpg}}
\subfloat[Figure 5-58(b)]{\label{fig:fig5-58b}\includegraphics[width=0.45\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/0b0675a867703b8fd8b714d446c3c80cc9c7c5e8feb73345a1bf9d0ea71370a3.jpg}}

\subfloat[Figure 5-58(c)]{\label{fig:fig5-58c}\includegraphics[width=0.6\textwidth]{../../../PDFs/differential-geometry/transcript/Do Carmo - Differential Geometry of Curves and Surfaces/hybrid_ocr/images/6e3e016f2d3b4b0520d5161b62ea9ac440ace5de8de078a69f8db919d2723357.jpg}}
\end{exercise}
\begin{solution}
\textbf{直观理解}: 旋转面情形Efimov定理的证明关键在于分析生成曲线的微分方程 $\varphi''+K\varphi=0$，$K<0$ 迫使 $\varphi$ 振荡或趋向无穷，从而面不完备。

\textbf{解答}: (a) 由第 3-3 节，旋转面方程 $K=-\varphi''/\varphi$（弧长参数）。若 $K<0$，则 $\varphi''>0$，$\varphi$ 下凸。生成曲线在两端必须满足 $\psi(0)=\psi(l)=0$，$\varphi>0$，故曲线必向无穷远处延伸（见 \cref{fig:fig5-58a} 和 (b)）。

(b) 设 $s\in[0,+\infty)$，由 $K\leq\delta<0$，方程 $\varphi''+\delta\varphi\leq 0$。若 $\varphi'(0)>0$，则 $\varphi'$ 递减，必在某 $s_0$ 处 $\varphi'(s_0)^2=1$（因为 $\varphi$ 有界且 $\varphi\to\infty$ 会导致 $K$ 太大）。

(c) 在 $p_0$ 之后，经线 $\varphi(s)$ 有三种延拓方式（I、II、III），但每种都会导致矛盾：I 导致 $\varphi$ 在有限距离内回零（与 $S$ 正则性矛盾），II 导致经线自交，III 导致 $\varphi$ 无限振荡而 $K$ 振荡太快（违背 $K\leq\delta<0$）。故 $S$ 不完备。\qed
\end{solution}
...
\end{exercise}
\begin{solution}
\textbf{直观理解}: Milnor 的证明通过 Gauss 映射构造新度量 $g_3=(g_1+g_2)/2$，将问题化为 $K=0$ 情形，结合 Hilbert 定理的证明技术导出矛盾。

\textbf{解答}: (a) 在渐近曲线坐标 $(u,v)$ 下，$g_1=du^2+2\cos\theta\,dudv+dv^2$，$g_2=du^2-2\cos\theta\,dudv+dv^2$，其中 $\theta$ 是两个度量主方向之间的夹角。

(b) 直接计算 $g_3=(g_1+g_2)/2=du^2+dv^2$（交叉项抵消），故 $K(g_3)=0$。由 $3g_3=g_1+2g_3\geq g_1$，故完备性从 $g_1$ 传递到 $g_3$。

(c) $K=0$ 的完备度量平面整体等距于 $\mathbb{R}^2$。由 (b)，存在等距 $\psi: S\to\mathbb{R}^2$。渐近曲线（在 $g_1$ 下为 $(1,\pm 1)$ 方向的等距线）映为 $\mathbb{R}^2$ 中斜率为 $\pm 1$ 的直线，构成矩形坐标系。

(d) 由 $\psi$ 将 $S$ 的渐近曲线（测地线在 $g_1$ 下）映为 $\mathbb{R}^2$ 的等距直线，和 Hilbert 定理的证明一样，这些直线形成"紧致区域"的边界，从而导出矛盾——与 $S$ 的非紧性矛盾。\qed
\end{solution}
