...
\end{exercise}
\begin{solution}
\textbf{直观理解}: 在上半平面 $y\to 0^+$ 时，度量系数 $G=1/y\to\infty$ 使得向量长度无界增加，但垂直线段的弧长却保持有限。这说明完备性在度量意义下的微妙之处。

\textbf{解答}: 设 $p=(x_0,y_0)\in\mathbb{R}_+^2$，$v=(v_1,v_2)$ 为切向量。向量长度
\[
|\mathbf{v}|=\sqrt{E v_1^2+2F v_1v_2+G v_2^2}=\sqrt{v_1^2+v_2^2/y}\geq\frac{|v_2|}{\sqrt{y}}.
\]
当 $y\to 0^+$ 时，若 $v_2\neq 0$，$|\mathbf{v}|\to\infty$。然而垂直线段 $x=0$，$y\in[\epsilon,1]$ 的弧长为
\[
L=\int_\epsilon^1\sqrt{G}\,dy=\int_\epsilon^1\frac{1}{\sqrt{y}}\,dy=2(1-\sqrt{\epsilon})\to 2\quad(\epsilon\to 0).
\]
取序列 $\alpha_n(t)=(0,1/n)$（$n\to\infty$），则 $\alpha_n$ 的弧长有界，且当 $n,m$ 足够大时，$d(\alpha_n,\alpha_m)\leq d(\alpha_n,\alpha_m)_{euclidean}\to 0$（因为 $1/n\to 0$），故 $\{\alpha_n\}$ 是 Cauchy 序列。但 $\alpha_n$ 趋于 $y=0$ 边界（不在 $S$ 中），因此 $S$ 中的 Cauchy 序列不收敛——度量不完备。\qed
\end{solution}
