...
\begin{Theorem}[球面刚性]\labelthm:sphere-rigidity}
...
\begin{Lemma}\label lem:sphere-lemma1}
设 $S$ 是正则曲面，$p \in S$ 满足：
\begin{enumerate}
  \item $K(p) > 0$（正高斯曲率）；
...
\begin{Lemma}\label lem:elliptic-point}
...
\begin{Proof}[定理 \refthm:sphere-rigidity} 的证明]
紧致性保证存在椭圆点（引理 \ref lem:elliptic-point}），而 $K$ 为常数，故 $K>0$。由紧致性，连续函数 $k_1$ 在某点 $p$ 处达到最大值；于是 $k_2 = K/k_1$ 在 $p$ 处达到最小值。由引理 \ref lem:sphere-lemma1}，$p$ 是脐点，即 $k_1(p) = k_2(p)$。
...
\begin{Theorem}[常平均曲率的紧致曲面是球面]\labelthm:sphere-constant-mean-curvature}
设 $S$ 是正高斯曲率 $K>0$ 且平均曲率 $H$ 常数的正则紧致连通曲面。则 $S$ 是球面。
\end{Theorem}

\begin{Remark}
常数 $K>0$ 的紧致连通曲面称为\textbf{卵形面}（ovaloid）。定理 \refthm:sphere-constant-mean-curvature} 可表述为：常平均曲率的卵形面是球面。H. Hopf 进一步证明：若正则曲面与球面同胚且常数平均曲率，则是球面。A. Alexandrov 更推广了这一结果。\cref{fig:fig5-1} 展示了一个与球面同胚但非刚性的曲面例子。
\end{Remark}
...
\textbf{整体微分几何的典型模式.} 定理 \refthm:sphere-rigidity} 是整体微分几何的典型结果：局部几何量（此处为常数曲率）加上弱整体假设（紧致性+连通性） $\Rightarrow$ 强整体约束（是球面）。

\subsection*{5-2 节练习}

\begin{exercise}{5-2, 1 — do Carmo, Exercise 5-2, 1}
设 $S \subset \mathbb{R}^3$ 是紧致正则曲面，固定点 $p_0 \notin S$，定义 $d(q) = \frac{1}{2}|q-p_0|^2$，$q\in S$。证明 $d$ 的最大值点 $q_0$ 是 $S$ 的椭圆点（给出引理 \ref lem:elliptic-point} 的另一证明）。
\end{exercise}
...
\begin{Theorem}[Hopf-Rinow]\labelthm:hopf-rinow}
...
证明：$S$ 中序列 $\{p_n\}$ 按内蕴距离 $d$ 收敛到 $p_0$ 当且仅当 $\{p_n\}$ 按 $\mathbb{R}^3$ 中的欧氏距离收敛到 $p_0$。
\end{exercise}
...
\begin{Lemma}\label lem:first-variation-formula}
$L(t)$ 在 $t=0$ 附近可微，且
\[
...
\begin{Lemma}\label lem:curvature-commutation}
...
\begin{Proof}[证明概要]
直接计算 $L''(0)$，利用 $\gamma$ 是测地线（$A(s)=0$）、正交性（$\langle\partial h/\partial s, \partial h/\partial t\rangle=0$）以及引理 \ref lem:curvature-commutation} 处理混合协变导数的交换。\footnote{详细推导见附录 \cref{sec:derivation-second-variation}}
\end{Proof}
...
（a）（最小面积旋转面。）设 $S$ 是由曲线 $y=f(x)$，$x\in[x_1,x_2]$ 绕 $x$ 轴旋转得到的旋转面。证明：若 $S$ 在所有生成旋转面中面积最小，则 $y = \frac{1}{c}\cosh(cx+c_1)$（悬链面）。（b）（旋转面的测地线。）设 $\mathbf{x}(u,v) = (f(v)\cos u, f(v)\sin u, g(v))$。证明测地线（除子午线和平行线外）满足 Clairaut 关系：$u' = c/(f^2)\sqrt{(f'^2+g'^2)(f^2-c^2)}^{-1}\cdot\sqrt{}$。
\end{exercise}
...
\begin{Proof}
由 $h_t(s)$ 是测地线得 $(D/\partial s)(\partial h/\partial s) = 0$。对 $t$ 求偏导并用引理 \ref lem:curvature-commutation}（第 5-4 节）得
\[
...
\begin{Theorem}\labelthm:no-conjugate-negative-curvature}
...
\begin{Lemma}[Gauss]\label lem:gauss-lemma}
...
设 $S$ 和 $\tilde{S}$ 是完备曲面，$p\in S$，$\tilde{p}\in\tilde{S}$，$i: T_p(S) \to T_{\tilde{p}}(\tilde{S})$ 是线性等距。设 $\gamma: [0,\infty) \to S$ 是测地线，$J(s)$ 是沿 $\gamma$ 的 Jacobi 场满足 $J(0)=0$，$\langle J'(0),\gamma'(0)\rangle=0$，$|J'(0)|=1$。（a）证明：$J(s)=v(s)e_2(s)$，$\tilde{J}(s)=u(s)\tilde{e}_2(s)$，其中 $v$ 和 $u$ 分别满足 $v''+K(s)v=0$，$v(0)=0$，$v'(0)=1$ 和 $\tilde{v}''+\tilde{K}(s)\tilde{v}=0$，$\tilde{v}(0)=0$，$\tilde{v}'(0)=1$。（b）（第一比较定理）假设 $K(s)\leq \tilde{K}(s)$，证明：若 $\tilde{\gamma}$ 的首个共轭点出现在 $s=a$，则 $\gamma$ 的首个共轭点不早于 $s=a$。（c）（第二比较定理）在同样假设下，证明 $|J(s)| \geq |\tilde{J}(s)|$ 对所有 $s$ 成立。（d）证明：在 (c) 中，等号恒成立当且仅当 $K(s)=\tilde{K}(s)$。
\end{exercise}
...
\begin{Lemma}\label lem:length-increasing}
...
\begin{Proof}[证明概要]
归结为 $K \leq 0$ 时 Jacobi 场的性质。利用引理 \ref lem:gauss-lemma}（Gauss）和 Jacobi 方程，可证 $|J(s)|^2 \geq s^2|w|^2$，取 $s=|u|$ 即得所需不等式。\footnote{详细推导见附录 \cref{sec:derivation-length-increasing}}
\end{Proof}
...
\begin{Theorem}[Hadamard 第一定理]\labelthm:hadamard-first}
...
\begin{Theorem}[Hadamard 第二定理]\labelthm:hadamard-second}
...
证明：弧道连通的 $B \subset \mathbb{R}^3$ 是单连通的，当且仅当任意闭弧线与常值弧线同伦。
\end{exercise}
...
设 $S \subset \mathbb{R}^3$ 是高斯曲率 $K \leq K_0$（$K_0\geq 0$ 常数）的完备曲面。设 $p,q \in S$，$\gamma_0$ 和 $\gamma_1$ 是连接 $p$ 到 $q$ 的两条不同测地线，$l(\gamma_0) \leq l(\gamma_1)$，且 $\gamma_0$ 与 $\gamma_1$ 同伦。证明：存在 $t_0 \in [0,1]$ 使得 $l(\gamma_0) + l(\alpha_{t_0}) \geq 2\pi/\sqrt{K_0}$（见 \cref{fig:fig5-29}）。
\end{exercise}
...
设 $S$ 定向且 $K>0$。证明：$S$ 上任意简单闭测地线与更短闭曲线同伦。
\end{exercise}
...
\begin{Theorem}[可微 Jordan 曲线定理]\labelthm:differentiable-jordan}
设 $\alpha: [0,l] \to \mathbb{R}^2$ 是正则、简单、闭平面曲线。则 $\mathbb{R}^2 - \alpha([0,l])$ 恰好有两个连通分支，$\alpha([0,l])$ 是它们的公共边界。
\end{Theorem}

\begin{Theorem}[旋转定理]\labelthm:rotation-index}
...
\begin{Theorem}[Fenchel 定理]\labelthm:fenchel}
...
\begin{Theorem}[Fary-Milnor]\labelthm:fary-milnor}
...
\begin{Remark}
定理 \refthm:rotation-index}（旋转定理）是 Gauss-Bonnet 定理应用于平面曲线时的核心应用之一，它说明平面凸曲线的旋转指标恒为 $\pm 1$，与曲线的具体形状无关。
\end{Remark}

\begin{Remark}
定理 \refthm:differentiable-jordan} 是 Jordan 曲线定理的可微版本（经典 Jordan 定理只要求曲线是连续的）。利用管状邻域和卷绕数的概念，可以给出简洁的可微情形证明。
\end{Remark}

\begin{Remark}
定理 \refthm:fary-milnor} 的证明用到了管状邻域上的 Gauss-Bonnet 定理（应用于管面的曲率积分）与几何对偶性（单位球面上法方向的覆盖次数）。
\end{Remark}
...
\begin{Remark}
定理 \refthm:fary-milnor} 中 " $>4\pi$" 的出现可以从管状邻域的几何理解：管面的高斯曲率积分给出 $2\int k\,ds$，而打结曲线的法方向在单位球面上覆盖至少两次（对应旋转指标 $\geq 2$），从而积分值加倍。
\end{Remark}
...
\begin{Remark}
特别值得强调的是 Hadamard 定理（定理 \refthm:hadamard-first} 和 \refthm:hadamard-second}）：它们建立了曲率符号（$K \leq 0$ 或 $K>0$）与曲面整体拓扑类型（平面型或球面型）之间的一一对应，是整体微分几何最漂亮的结果之一。
\end{Remark}
...
\begin{Remark}
Bonnet 定理（定理 \refthm:bonnet}）的证明是本课程中最复杂的变分计算之一：第二变分公式（命题 \ref{prop:second-variation}）结合精心构造的变分场（$V(s) = w(s)\sin\frac{\pi}{l}s$）给出关键的不等式，揭示了正曲率"压缩"测地线的几何机制。
\end{Remark}

\begin{Remark}
Hopf-Rinow 定理（定理 \refthm:hopf-rinow}）是完备曲面理论的基石：它保证完备曲面上任意两点间存在最小测地线，这一存在性是后续所有整体定理的第一步。
\end{Remark}

\begin{Remark}
球面刚性定理（定理 \refthm:sphere-rigidity}）是本章第一个整体定理，其证明方法（引理 \ref lem:sphere-lemma1} 利用 Mainardi-Codazzi 方程）成为后续曲面论研究的标准技术。
\end{Remark}
...
\begin{Remark}
Hilbert 定理（定理 \refthm:hilbert-theorem}）是整体微分几何最著名的"不可能定理"之一：它断言 $\mathbb{R}^3$ 中不存在常负曲率的完备正则曲面。这引导了 $\mathbb{R}^3$ 中负曲率曲面的研究——以及黎曼几何中双曲空间的发现。
\end{Remark}
...
\begin{Theorem}\labelthm:zero-curvature-surface}
...
\begin{Lemma}\label lem:zero-curvature-asymptotic}
...
\begin{Remark}
定理 \refthm:zero-curvature-surface} 的结论不能加强：存在非平面非圆柱面的完备 $K=0$ 曲面（如上例的构造）。要得到平面或圆柱面，$K \equiv 0$ 和完备性都是必要的。
\end{Remark}
...
\begin{Remark}
定理 \refthm:zero-curvature-surface} 也是 Hadamard 定理（第 5-6 节）的类比：$K<0$ 的完备单连通曲面微分同胚于平面；$K\equiv 0$ 的完备曲面则平面或圆柱面。这些都是曲率条件决定整体拓扑的经典范例。
\end{Remark}
...
\begin{Remark}
定理 \refthm:zero-curvature-surface} 在物理中也有应用：$K=0$ 的曲面在弹性理论（薄壳理论）和计算机图形学（曲面建模）中都有特殊地位。
\end{Remark}
...
\begin{Theorem}[Jacobi 测地线极小性定理]\labelthm:jacobi-minimality}
...
\begin{Remark}
该定理在 $K \leq 0$ 的曲面上恒成立（由定理 \refthm:no-conjugate-negative-curvature}）：$K \leq 0$ 的完备曲面上任意测地线弧都是弧长最小者。
\end{Remark}
...
\begin{Theorem}[Hilbert]\labelthm:hilbert-theorem}
...
\textbf{第二步：利用 Hadamard 定理.} 因 $K<0$，由 Hadamard 第一定理（定理 \refthm:hadamard-first} 的弱化版本：$K \leq 0$ 的完备曲面，$\exp_p$ 是局部微分同胚且无共轭点），$S$ 微分同胚于 $\mathbb{R}^2$（因 $K<0$ 无共轭点，故 $\exp_p$ 单射）。\footnote{详细推导见附录 \cref{sec:derivation-hilbert-step2}}
