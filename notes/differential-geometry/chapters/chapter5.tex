...
\begin{Lemma}\label{lema:sphere-lemma1}
设 $S$ 是正则曲面，$p \in S$ 满足：
\begin{enumerate}
  \item $K(p) > 0$（正高斯曲率）；
...
\begin{Lemma}\label{lema:elliptic-point}
...
设 $S \subset \mathbb{R}^3$ 是紧致正则曲面，固定点 $p_0 \notin S$，定义 $d(q) = \frac{1}{2}|q-p_0|^2$，$q\in S$。证明 $d$ 的最大值点 $q_0$ 是 $S$ 的椭圆点（给出引理 \ref{elliptic-point} 的另一证明）。
\end{exercise}
...
\begin{Lemma}\label{lema:first-variation-formula}
$L(t)$ 在 $t=0$ 附近可微，且
\[
...
\begin{Lemma}\label{lema:curvature-commutation}
...
\begin{Lemma}[Gauss]\label{lema:gauss-lemma}
...
\begin{Lemma}\label{lema:length-increasing}
...
\begin{Lemma}\label{lema:zero-curvature-asymptotic}
