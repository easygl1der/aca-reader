...
% Document environment: only when NOT standalone (i.e., when included via wrapper)
% When \STANDALONE is defined (wrapper mode), the wrapper provides
% \begin{document} so we skip it here; when undefined (standalone mode),
% we open the environment here.
\ifdefined\STANDALONE
  % wrapper mode: skip document env, chapter will be handled by wrapper
\else
  \begin{document}
  \maketitle
\fi
...
\begin{enumerate}
    \item 当数据位于未知低维流形上时，\textbf{分布估计的统计极限}是什么？
    \item \textbf{深度生成模型能否隐式自适应流形结构}，避免维度灾难？
    \item 如何在流形约束下进行有效的\textbf{贝叶斯推断}和 MCMC 采样？
\end{enumerate}
...
单位分解函数 $\{\gamma_\lambda\}$ 实现。这一技术在
{tang2022minimax} 和 {tang2023lowdim} 的估计器构造中起了关键作用，是处理无全局参数化
流形的核心工具。
...
这一统一的框架使得 {tang2025regression} 可以同时处理分布回归中的多种误差度量。
...
{tang2024diffusion} 构造了一类随时间分段变化的神经网络架构
...
% ----------------------------------------------------------
\subsection{{tang2022minimax} --- 未知子流形上的极小极大分布估计}
...
\textbf{与系列的关系}：{tang2022minimax} 是整个系列的理论基石，
为 {tang2023lowdim}, {tang2024diffusion} 中具体生成模型的分析提供了理论参照系。
\textbf{局限}：估计器构造涉及过多技术细节，实际可操作性有限；
仅考虑无噪声设定。

% ----------------------------------------------------------
\subsection{{tang2023lowdim} --- 低维结构混合生成模型}
...
\[
O\!\Big(n^{-\frac{\alpha\wedge(\beta-1)+1}{2(\alpha\wedge(\beta-1))+d}}\Big)
\;\vee\;
O\!\Big(\frac{\log n}{\sqrt{n}}\Big),
\]
在 $\alpha \le \beta-1$ 时达到极小极大最优（至多对数因子）。
\textbf{核心发现：多编码器/解码器结构在流形分布估计中是必要的。}

\textbf{与 {tang2022minimax} 的关系}：{tang2022minimax} 提供了极小极大理论，
本篇则将理论转化为具体可计算的深度学习方法，
验证了单位分解技术在实际深度学习中的可实现性。

% ----------------------------------------------------------
\subsection{{tang2023robust} --- 黎曼子流形上的稳健贝叶斯推断}
...
% ----------------------------------------------------------
\subsection{{tang2023twosample} --- 对抗损失下的两样本检验}
...
% ----------------------------------------------------------
\subsection{{tang2024diffusion} --- 扩散模型自适应流形结构}
...
\textbf{与 {tang2022minimax} 的关系}：{tang2022minimax} 提供了一般极小极大框架，
本篇则具体证明了扩散模型能够达到该速率，为扩散模型的有效性
提供了严格的理论解释。

% ----------------------------------------------------------
\subsection{{tang2024mala} --- Metropolis-Adjusted Langevin 算法的混合时间}
...
\textbf{与 {tang2023robust} 的内在联系}：{tang2023robust} 建立了流形上的
BvM 定理（后验渐近正态）；本篇则利用 BvM，将 MALA 的最优维度依赖
从极度受限的乘积分布推广到一大类实际感兴趣的后验分布。

% ----------------------------------------------------------
\subsection{{tang2025regression} --- 分布回归的极小极大最优速率}
...
% ----------------------------------------------------------
\subsection{{tang2025conditional} --- 条件扩散模型的极小极大最优性}
...
\textbf{问题背景}：{tang2024diffusion} 证明了无条件扩散模型能够自适应流形结构。
...
\textbf{与 {tang2024diffusion}, {tang2025regression} 的关系}：{tang2024diffusion} 无条件情形
→ {tang2025regression} 分布回归理论框架 → 本篇将条件扩散模型接入该框架。
...
\[
\text{\citealt{tang2022minimax} 极小极大理论}
\;\xrightarrow{\;\text{具体化}\;}\;
\text{\citealt{tang2023lowdim} MLDMAE 方法}
\;\xrightarrow{\;\text{推广}\;}\;
\text{\citealt{tang2025regression} 分布回归框架}
\]
...
\[
\text{VAE 自适应流形 \citep{tang2021vae}}
\;\xrightarrow{\;\text{扩展到流形}\;}\;
\text{\citealt{tang2024diffusion} 扩散模型自适应流形}
\;\xrightarrow{\;\text{推广到条件设定}\;}\;
\text{\citealt{tang2025conditional} 条件扩散模型最优性}
\]

这条主线的核心洞见是：\textbf{扩散模型虽在原始高维空间运行，
却能隐式自适应低维结构。} \citep{tang2024diffusion} 的深层解释是：
...
\[
\text{\citealt{tang2023robust} 流形 BvM + RRWM}
\;\xrightarrow{\;\text{利用 BvM 性质}\;}\;
\text{\citealt{tang2024mala} MALA 最优混合时间}
\]
...
\begin{enumerate}
    \item \textbf{2021} --- {tang2021vae}（VAE 流形自适应）
    \item \textbf{2022} --- {tang2022minimax}
          （流形分布估计极小极大框架）
    \item \textbf{2023a} --- {tang2023lowdim}（MLDMAE）
    \item \textbf{2023b} --- {tang2023robust}
          （流形稳健贝叶斯 + BvM）
    \item \textbf{2023c} --- {tang2023twosample}（两样本检验）
    \item \textbf{2024a} --- {tang2024diffusion}（扩散模型自适应流形）
    \item \textbf{2024b} --- {tang2024mala}（MALA 最优混合时间）
    \item \textbf{2025a} --- {tang2025regression}（分布回归极小极大速率）
    \item \textbf{2025b} --- {tang2025conditional}（条件扩散模型最优性）
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{单位分解（Partition of Unity）}：
          {tang2022minimax}, {tang2023lowdim} 利用此技术处理无全局参数化流形——
          是估计器构造的核心工具。

    \item \textbf{$\gamma$-Hölder IPM}：
          {tang2022minimax}, {tang2023twosample}, {tang2025regression} 将不同光滑度的判别器类
          统一在同一个框架下，实现多种距离度量的同时最优。

    \item \textbf{Gaussian 平滑 + 四阶矩误差}：
          {tang2024diffusion} 将 Langevin 扩散的分析从 $L_\infty$ 弱化到
          四阶矩，建立了更弱但更实用的误差条件。

    \item \textbf{时间分段神经网络架构}：
          {tang2024diffusion}, {tang2025conditional} 构造了随时间演化的神经网络架构，
          有效平衡了近时间端（$t$ 小）与远时间端（$t$ 大）的逼近精度。

    \item \textbf{$s$-conductance profile}：
          {tang2024mala} 提出的马尔可夫链分析新技术，
          提供了介于经典 conductance 与 conductance profile 之间的
          精细分析工具。

    \item \textbf{黎曼流形 Bernstein-von Mises 定理}：
          {tang2023robust} 将经典 BvM 推广到黎曼子流形 setting，
          为流形贝叶斯推断提供了严格的理论基础。

    \item \textbf{Girsanov 定理桥梁}：
          {tang2024diffusion}, {tang2025conditional} 通过 Girsanov 定理建立分数估计误差
...
\begin{enumerate}
    ...
    \item \textbf{噪声观测情形}：
          {tang2022minimax} 的无噪声设定过于理想化。
          噪声情形下收敛速率急剧下降（仅对数级），本质上是病态问题。
          是否有方法在适度噪声假设下恢复多项式速率？

    \item \textbf{光滑性自适应}：
          {tang2023twosample} 的两样本检验中，最优截断水平 $J$ 依赖于
...
% Close document environment only when NOT standalone (i.e., standalone mode)
\ifdefined\STANDALONE
  % wrapper mode: wrapper handles \end{document}
\else
  \end{document}
\fi
