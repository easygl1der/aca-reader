...
一个关键的洞察是：\textbf{散度（Divergence）取代了传统几何中的距离概念}，因为概率分布之间的"差异"往往不是对称的。这为贝叶斯分析提供了统一的几何框架。
...
Fisher 信息具有重要的\textbf{不变性}：在充分统计量下保持不变。这使其成为概率分布空间上的"自然"黎曼度量。
...
当散度由凸函数通过 Bregman 形式诱导时，流形获得一种\textbf{对偶平坦结构}：

\begin{enumerate}
    \item 由 $\psi$ 诱导一组\textbf{仿射坐标} $\theta$（自然参数）
    \item 由 Legendre 变换 $\psi^*$ 诱导\textbf{对偶仿射坐标} $\eta$：
        \begin{equation}
            \eta = \nabla \psi(\theta), \quad \theta = \nabla \psi^*(\eta)
        \end{equation}
    \item 这两组坐标通过 \textbf{Legendre 变换} 相互联系
\end{enumerate}
