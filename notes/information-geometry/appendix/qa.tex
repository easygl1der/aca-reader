...
\subsection{散度与距离的核心区别}\label{sec:qa-divergence-vs-distance}

\textbf{问}：散度（Divergence）与距离的核心区别是什么？

\textbf{答}：散度不是距离，因为它不满足对称性：$D[P:Q] \neq D[Q:P]$。这反映了概率分布之间的"方向性"差异——从分布 $P$ 到 $Q$ 的信息损失与从 $Q$ 到 $P$ 是不同的。

在贝叶斯分析中，这对应于 $D_{KL}[p(\theta|x):p(\theta)]$ 衡量的是"给定数据后"相对于"先验"的信息增益，是有方向的。

\subsection{为何使用散度而非距离}\label{sec:qa-why-divergence}

\textbf{问}：为什么要用散度而不是距离？

\textbf{答}：因为概率分布之间的比较往往需要考虑方向性。例如，用简单分布近似复杂分布时，近似的"损失"取决于我们用哪个分布作为参考。

这正是贝叶斯推断的核心：后验分布 $p(\theta|x)$ 是对先验 $p(\theta)$ 的"更新"，这个更新过程天然是非对称的。

\subsection{指数族在贝叶斯分析中的重要性}\label{sec:qa-exponential-family-bayesian}

\textbf{问}：指数族为什么在贝叶斯分析中如此重要？
...
\subsection{混合族与指数族的关系}\label{sec:qa-mixture-vs-exponential}

\textbf{问}：混合族和指数族有什么联系？
...
\subsection{指数族的例子}\label{sec:qa-exponential-family-examples}
...
\subsection{指数族的几何意义}\label{sec:qa-exponential-family-geometry}

\textbf{问}：指数族的几何意义是什么？

\textbf{答}：从几何角度看，指数族在参数空间中是平坦的（欧几里得几何）。这意味着：
\begin{itemize}
...
\subsection{Jeffreys 先验与 Fisher 信息的关系}\label{sec:qa-jeffreys-prior-fisher-info}

\textbf{问}：贝叶斯推断中，Jeffreys 先验与 Fisher 信息是什么关系？

\textbf{答}：Jeffreys 先验定义为：
\begin{equation}
...
\subsection{KL 散度在贝叶斯分析中的应用}\label{sec:qa-kl-divergence-bayesian}

\textbf{问}：KL 散度在贝叶斯分析中的具体应用有哪些？

\textbf{答}：KL 散度在贝叶斯分析中有广泛应用：
...
\subsection{EM 算法与信息几何的关系}\label{sec:qa-em-algorithm-info-geometry}

\textbf{问}：EM 算法与信息几何是什么关系？

\textbf{答}：EM 算法是对偶平坦空间中的\textbf{交替最小化}算法的典型例子：
...
\subsection{变分推断的信息几何理解}\label{sec:qa-variational-inference-geometry}

\textbf{问}：变分推断如何从信息几何角度理解？

\textbf{答}：变分推断（VI）的核心是散度最小化：
...
\subsection{信息几何对贝叶斯模型选择的启示}\label{sec:qa-info-geometry-bayesian-model}

\textbf{问}：信息几何对贝叶斯模型选择有什么启示？

\textbf{答}：从信息几何角度看，模型选择本质上是在不同的子流形之间进行选择：
...
\subsection{Fisher 信息的唯一性}\label{sec:qa-fisher-information-uniqueness}

\textbf{问}：为什么 Fisher 信息是唯一的？

\textbf{答}：Fisher 信息具有独特的不变性：
...
\subsection{信息几何视角下的贝叶斯核心思想}\label{sec:qa-bayesian-core-info-geometry}

\textbf{问}：信息几何视角下的贝叶斯核心思想是什么？

\textbf{答}：综合以上讨论，信息几何为贝叶斯分析提供了统一的几何框架：
