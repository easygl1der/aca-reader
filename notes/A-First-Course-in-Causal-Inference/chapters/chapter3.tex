...
\subsection{卡方检验：二元结局的渐近检验}

对于二元结局，我们还可以使用\textbf{卡方检验（Chi-Square Test）}——这是 Fisher 精确检验的大样本渐近替代。

\textbf{背景}：卡方检验由卡尔·皮尔逊（Karl Pearson）于 1900 年提出，用于检验观察频数与期望频数之间是否存在显著差异。

\textbf{基本思想}：在零假设 $Z \perp Y$ 下，治疗组和对照组的成功概率相等：$p_1 = p_0$。我们可以构建 $2 \times 2$ 列联表：
...
\textbf{期望频数}（在 $Z \perp Y$ 下）：
\[
\mathbb{E}(n_{11}) = \frac{n_1 \cdot n_{.1}}{n}, \quad
\mathbb{E}(n_{10}) = \frac{n_1 \cdot n_{.0}}{n}, \quad
\mathbb{E}(n_{01}) = \frac{n_0 \cdot n_{.1}}{n}, \quad
\mathbb{E}(n_{00}) = \frac{n_0 \cdot n_{.0}}{n}
\]

\textbf{皮尔逊卡方统计量}：
\[
\chi^2 = \sum_{\text{所有格子}} \frac{(O - E)^2}{E}
= \frac{(n_{11} - \mathbb{E}(n_{11}))^2}{\mathbb{E}(n_{11})}
+ \frac{(n_{10} - \mathbb{E}(n_{10}))^2}{\mathbb{E}(n_{10})}
+ \frac{(n_{01} - \mathbb{E}(n_{01}))^2}{\mathbb{E}(n_{01})}
+ \frac{(n_{00} - \mathbb{E}(n_{00}))^2}{\mathbb{E}(n_{00})}
\]

其中 $O$ = 观察频数（observed），$E$ = 期望频数（expected）。

\textbf{渐近分布}：在零假设下，当样本量 $n$ 足够大时，
\[
\chi^2 \xrightarrow{d} \chi_1^2
\]
自由度为 $(2-1)\times(2-1)=1$，因为行合计与列合计已固定。

★ Insight ─────────────────────────────────────
为什么是 $(O-E)^2/E$ 而不是 $|O-E|$？平方项保证正值且对大的偏离更敏感；除以 $E$ 则标准化了不同期望频数的影响。这正是皮尔逊的洞见——将"偏离程度"量化为一个具有标准渐近分布的统计量。
─────────────────────────────────────────────────

\textbf{R 实现}：
\begin{verbatim}
# 构建 2x2 列联表
tb <- matrix(c(n11, n10, n01, n00), nrow = 2, byrow = TRUE)
rownames(tb) <- c("Z=1", "Z=0")
colnames(tb) <- c("Y=1", "Y=0")
...
\textbf{连续性修正}：R 中的 \texttt{chisq.test} 默认使用 Yates 连续性修正，对小样本的渐近近似进行校正：
\[
