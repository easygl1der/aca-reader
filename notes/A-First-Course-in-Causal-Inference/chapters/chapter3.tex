...
\textbf{渐近分布}：由于有限总体中心极限定理（Finite Population Central Limit Theorem\footnote{有限总体 CLT 是随机化推断的核心工具。传统统计学的中心极限定理研究的是 i.i.d. 样本的分布，而有限总体 CLT 研究的是从固定有限总体中进行随机抽样时，样本均值（或更一般的统计量）的分布。其核心结论是：对于从有限总体中随机抽取的样本，在一定正则条件下，当样本量增大时，样本均值的随机化分布会收敛到正态分布。关键在于，这个正态性\textbf{不依赖于潜在结果的分布}（即 $Y_i$ 本身可以是任意分布），而只依赖于随机化机制。这正是 FRT 能为 t 检验提供理论基础的原因。})，随机化分布近似正态：
...
完整推导见附录 \cref{sec:frt-algebra-derivation}。\qed
\end{Example}
...
完整推导见附录 \cref{sec:wilcoxon-variance-derivation}。\qed
\end{Example}
...
完整推导与二项分布比较见附录 \cref{sec:lady-tea-hypergeometric}。\qed
\end{Example}
