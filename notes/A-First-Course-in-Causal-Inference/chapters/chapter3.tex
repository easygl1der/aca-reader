...
第一点直观易懂——随机治疗分配不会偏向 treatment 或 control。第二点更微妙：Fisher 的意思是，随机化为统计检验提供了正当理由，这就是现在所谓的\textbf{费舍尔随机化检验（Fisher Randomization Test, FRT）}。\footnote{问：Fisher 随机化检验与经典假设检验有何不同？见附录 \cref{sec:qa-fisher-vs-classical}。}
...
\]
\citet{Rubin:1980} 称其为\textbf{sharp null 假设}，因为它可以基于观测数据确定所有潜在结果：$\bm{Y}(1) = \bm{Y}(0) = \bm{Y} = (Y_1, \ldots, Y_n)$，即观测结果向量。它也被称为\textbf{强零假设}。\footnote{问：为什么在 $H_{0F}$ 下 $\bm{Y}$ 是固定的？见附录 \cref{sec:qa-Y-fixed-under-H0F}。}
...
在实践中，$M$ 通常太大（例如当 $n=100, n_1=50$ 时，$M > 10^{29}$），枚举所有可能的治疗向量在计算上是不可行的。我们经常用 Monte Carlo 来近似 $p_\frt$。\footnote{问：$p_{frt}$ 是什么？为什么要用 Monte Carlo 近似？见附录 \cref{sec:qa-pfrt-mc}。}
...
然后计算方差。记 $s^2 = (n-1)^{-1} \sumn (Y_i - \bar{Y})^2$，其中 $\bar{Y} = n^{-1} \sumn Y_i$。利用简单随机抽样中 $\text{var}(Z_i) = n_1 n_0 / n^2$ 和 $\text{cov}(Z_i, Z_j) = -n_1 n_0 / [n^2 (n-1)]$（对于 $i \neq j$），我们有：
\begin{align*}
\text{var}(\hat{\tau})
&= \text{var}\left\{ n_1^{-1} \sum_{i=1}^n Z_i Y_i - n_0^{-1} \sum_{i=1}^n (1 - Z_i) Y_i \right\} \\
&= \text{var}\left\{ \frac{n}{n_0} \cdot n_1^{-1} \sum_{i=1}^n Z_i Y_i - n_0^{-1} \sum_{i=1}^n Y_i \right\} \\
&= \frac{n^2}{n_0^2} \cdot \frac{1}{n_1^2} \cdot \text{var}\left( \sum_{i=1}^n Z_i Y_i \right) \quad (\text{第二项是常数}) \\
&= \frac{n^2}{n_0^2 n_1^2} \cdot \left( \sum_{i=1}^n Y_i^2 \cdot \frac{n_1 n_0}{n} - \frac{n_1 n_0}{n(n-1)} \sum_{i=1}^n Y_i^2 \right) \\
&= \frac{n^2}{n_0^2 n_1^2} \cdot \frac{n_1 n_0}{n(n-1)} \left( (n-1)\sum_{i=1}^n Y_i^2 - \sum_{i=1}^n Y_i^2 \right) \\
&= \frac{n}{n_1 n_0} \cdot \frac{1}{n-1} \left( \sum_{i=1}^n Y_i^2 - n\bar{Y}^2 \right) \\
&= \frac{n}{n_1 n_0} \cdot s^2 .
\end{align*}

其中 $\text{var}(Z_i) = n_1 n_0 / n^2$ 来自超几何分布的性质，而 $\sum_{i \neq j} \text{cov}(Z_i, Z_j) = -n_1 n_0 / (n-1)$ 用于化简。\footnote{差值均值统计量期望与方差的完整推导见附录 \cref{sec:tau-hat-variance-derivation}。}
...
\textbf{ANOVA 分解的几何直觉}：将 $n$ 维向量 $\bm{Y}$ 投影到 treatment 组和 control 组形成的两个正交子空间。组内平方和是残差平方和，组间平方和 $\frac{n_1 n_0}{n} \hat{\tau}^2$ 是由治疗分配解释的变异。关键发现：$s^2$ 中混入了治疗效应信息，这解释了为什么 FRT 的 $t$ 统计量在 $\HF$ 下能精确分布。
...
\textbf{核心思想}：在 $\HF$ 下，随机性只来自治疗分配 $\bm{Z}$，秩 $R_i$ 是固定值。
