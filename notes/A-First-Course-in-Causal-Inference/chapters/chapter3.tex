...
在 FRT 中，这个分解揭示了 $s^2$ 不仅捕捉"误差方差"，还包含了潜在的治疗效应信息。当我们用 $\hat{\tau} / \sqrt{\frac{n}{n_1 n_0} s^2}$ 作为标准化统计量时，分母中的 $s^2$ 混入的 $\hat{\tau}^2$ 项使得随机化分布的推导需要借助 \cref{eq:frt-algebra}——这正是 \ref{ex:3.8} 的动机。
