...
\chapter{重叠性、外推风险与回归断点}\label{ch:20}
...
--- adapted from King and Zeng (2006)

在因果推断中，我们始终面对一个根本性的哲学困难：对于每个单元，我们只能观测到\textbf{一个}世界的结果，而必须推断\textbf{另一个}世界的结果。Rubin 的潜在结果框架（Rubin, 1974, 1975）用数学语言精确化了这个直观的"平行世界"隐喻。
...
\vspace{2cm}
\end{minipage}}
\caption{最低饮酒年龄（MLDA）政策对死亡率影响的 RDD 分析：断点附近死亡率存在跳跃。}
\label{fig:mlda}
\end{figure}
...
\subsection{Lee（2008）现任优势研究}\label{sec:20-lee-example}

Lee（2008）提供了一个著名的 RD 应用实例，研究美国众议院选举中的现任优势（incumbency advantage）。运行变量是上届选举的政党得票率（以 0 为断点），处理变量是本届选举中是否为执政党候选人，结局是本届选举的得票率。

直观上，现任优势是一个本质上难以用因果推断回答的问题——那些在上届选举中获胜的候选人之所以成功，可能是因为他们本身就更具竞争力，而非因为"现任"身份本身。然而，RD 设计提供了一个巧妙的研究框架。
...
## rdrobust 分析：三套带宽选择
fit1 = rdrobust(y, x, c = 0, kernel = "triangular", p = 1)
fit2 = rdrobust(y, x, c = 0, kernel = "triangular", p = 1, bwselect = "mserd")
fit3 = rdrobust(y, x, c = 0, kernel = "triangular", p = 1, bwselect = "cerrd")

## 输出点估计和置信区间
rbind(fit1$Estimate,
      fit2$Estimate,
      fit3$Estimate)[, c("Estimate", "ci.lb", "ci.ub")]
##        Estimate   ci.lb    ci.ub
## Main  0.0757   0.0670   0.0844
## MSE   0.0760   0.0672   0.0847
## CER   0.0759   0.0671   0.0847
\end{lstlisting}

三套带宽选择均给出约 $7.6\%$ 的现任优势估计，且置信区间高度重叠，说明结论对带宽选择不敏感。这一结果在政治科学中有重要意义：现任优势确实存在，且相当稳定。
...
\section{习题}\label{sec:20-homework}

\begin{HomeworkProblem}[A theorem for the role of outcome modeling in the presence of limited overlap]\label{exr:20-1}
...
\begin{HomeworkProblem}[Linear potential outcome models]\label{exr:20-2}
...
\begin{HomeworkProblem}[Simulation for regression discontinuity]\label{exr:20-3}
...
\begin{HomeworkProblem}[Re-analysis of the data on the minimum legal drinking age]\label{exr:20-4}
...
\begin{HomeworkProblem}[Re-analysis of \citet{lee2008randomized}'s data]\label{exr:20-5}
...
\begin{HomeworkProblem}[Recommended reading]\label{exr:20-6}
