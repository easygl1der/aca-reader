...
\VanderWeeleetal{2014} 是讨论 MR 方法论挑战的优秀综述论文。

\section{习题}\label{sec:25-exercises}

\begin{Exercise}{\ref{exr:25-1} Fisher 加权估计的推导}\label{exr:25-1}
证明第 \ref{sec:25-fixed-effect} 节中 $\hat\beta_{\mathrm{fisher0}}$ 的方差为 $\left(\sum_{j=1}^p 1 / \mathrm{se}_j^2\right)^{-1}$。提示：利用 $\hat\beta_j$ 的独立性。
\end{Exercise}

\begin{Exercise}{\ref{exr:25-2} InSIDE 假设的含义}\label{exr:25-2}
解释 InSIDE（工具强度独立于直接效应）假设在什么意义上弱于 $\alpha_j = 0$（对所有 $j$）的排他性限制假设。给出一个满足 InSIDE 但不满足 $\alpha_j = 0$ 的数值例子。
\end{Exercise}

\begin{Exercise}{\ref{exr:25-3} Egger 截距的解释}\label{exr:25-3}
在第 \ref{sec:25-egger-regression} 节中，我们看到 Egger 回归的截距 $\hat\alpha_{\mathrm{egger0}}$ 收敛到直接效应的加权平均 $\alpha_w$。讨论为什么这个性质在实践中有用——即为什么我们可能关心估计 $\alpha_w$ 而不仅仅是 $\beta$。
\end{Exercise}

\begin{Exercise}{\ref{exr:25-4} 数据分析}\label{exr:25-4}
分析 \texttt{mr.raps} 包中的 \texttt{bmi.bmi} 数据。参见 \citet[第 7.2 节]{Zhao2020} 了解更多细节。
\end{Exercise}

\section*{推荐阅读}\label{sec:25-recommended-reading}

\citet{DaveySmith2003} 综述了 MR 的潜力和局限性，是该领域的经典文献。

\citet{Bowden2015} 提出了 Egger 回归方法来检测和调整无效工具变量。

\citet{Zhao2020} 开发了基于鲁棒调整轮廓得分（robust adjusted profile score, RAPS）的两样本汇总数据 MR 推断方法。
