...
经典的潜在结果框架 $Y_i(1)$ 和 $Y_i(0)$ 依赖于一个隐含假设：\textbf{处理的施与独立于潜在结果}（SUTVA 的一部分）。然而在许多实际场景中，这个假设根本不成立。例如：
\begin{itemize}
...
这些现象的共同特征是：\textbf{处理的施与（compliance）取决于潜在结果}——用更专业的术语说，处理并非独立于潜在结果。这种情况下，标准的因果效应定义（ACE、ATT 等）可能产生令人困惑的解释。
...
\section{主分层的定义与记号}\label{sec:26-basic-setup}
...
\begin{Remark}
经典参考文献包括 \cite{RosenbaumRubin:1983b}，以及 \cite{AngristPischke:2009} 第 4 章关于 IV 和主分层的讨论。
\end{Remark}

\section{习题}\label{sec:26-homework}

\begin{HomeworkProblem}[Complete the proof of Theorem \ref{thm::identification-PCEs-pscore}]\label{exr:26-1}
...
\begin{HomeworkProblem}[Principal score method under monotonicity]\label{exr:26-2}
...
\begin{HomeworkProblem}[Principal score method in observational studies]\label{exr:26-3}
...
\begin{HomeworkProblem}[General principal score method]\label{exr:26-4}
...
\begin{HomeworkProblem}[Recommended reading]\label{exr:26-5}
