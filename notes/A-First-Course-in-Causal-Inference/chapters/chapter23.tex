\chapter{两阶段最小二乘}\label{chap:TSLS}
...
\begin{equation}
Y = D^{\top}\beta + \varepsilon, \tag{23.1}
\end{equation}
...
\begin{equation}
n^{-1}\sum_{i=1}^{n}Z_{i}Y_{i}=n^{-1}\sum_{i=1}^{n}Z_{i}D_{i}^{\top}\beta \tag{23.2}
\end{equation}
...
为了看清 TSLS 估计量的本质，我们将其显式地写出来。\footnote{推导过程略，如有需要可参考 \cite{AngristPischke2009}。}

第一阶段 OLS 回归可以写成：
\begin{equation}
\hat{D}_i = \hat{\Gamma}^{\top}Z_i, \tag{23.3}
\end{equation}
其中 $\hat{\Gamma} = \left(\sum_{i=1}^{n}Z_iZ_i^{\top}\right)^{-1}\sum_{i=1}^{n}Z_iD_i^{\top}$。

第二阶段的 OLS 估计量为：
\begin{equation}
\hat{\beta}_{\mathrm{TSLS}} = \left(\sum_{i=1}^{n}\hat{D}_i\hat{D}_i^{\top}\right)^{-1}\sum_{i=1}^{n}\hat{D}_iY_i. \tag{23.4}
\end{equation}

将 $Y_i = D_i^{\top}\beta + \varepsilon_i$ 代入，经过整理可得：
\begin{equation}
\hat{\beta}_{\mathrm{TSLS}} = \beta + \left\{\hat{\Gamma}^{\top}\left(n^{-1}\sum_{i=1}^{n}Z_iZ_i^{\top}\right)\hat{\Gamma}\right\}^{-1}\hat{\Gamma}^{\top}\left(n^{-1}\sum_{i=1}^{n}Z_i\varepsilon_i\right). \tag{23.5}
\end{equation}
...
\begin{equation}
\hat{V}_{\mathrm{TSLS}} = \left(\sum_{i=1}^{n}\hat{D}_i\hat{D}_i^{\top}\right)^{-1}\left(\sum_{i=1}^{n}\hat{\varepsilon}_i^{2}\hat{D}_i\hat{D}_i^{\top}\right)\left(\sum_{i=1}^{n}\hat{D}_i\hat{D}_i^{\top}\right)^{-1}. \tag{23.6}
\end{equation}
...
\begin{equation}
\beta_1 = \frac{\Gamma_1}{\gamma_1}. \tag{23.9}
\end{equation}

因此，我们可以构造间接最小二乘（Indirect Least Squares, ILS）估计量：
\begin{equation}
\hat{\beta}_{1,\mathrm{ILS}} = \frac{\hat{\Gamma}_1}{\hat{\gamma}_1}. \tag{23.10}
\end{equation}
...
\begin{equation}
\hat{\beta}_{1,\mathrm{ILS}} = \hat{\beta}_{1,\mathrm{TSLS}}. \tag{23.11}
\end{equation}
\end{theorem}

这个定理的本质是一个代数恒等式，它说明尽管 TSLS 和 ILS 看起来是不同的估计方法，但它们实际上是同一估计量的不同计算路径。\footnote{定理 \cref{th:TSLS-ILS-equivalence} 的证明见习题 \cref{hw:23.2}。}
...
具体地，FAR 置信区间的构造如下：
\begin{equation}
\left\{b: |t_Z(b)| \leq z_{1-\alpha/2}\right\}, \tag{23.12}
\end{equation}
