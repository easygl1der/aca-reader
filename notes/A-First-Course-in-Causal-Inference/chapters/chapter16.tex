...
\tag{16.5}
\cref{eq:16-unconfoundedness-meaning}
\]
...
\tag{16.6}
\cref{eq:16-negative-outcome-effect}
\]
...
\tag{16.7}
\cref{eq:16-negative-exposure-effect}
\]
...
为了更直观地理解，我们考虑一个正态线性模型的具体例子：\footnote{推导见附录 \cref{sec:appendix-m-bias-derivation}}
\[
\begin{cases}
...
\tag{16.12}
\cref{eq:16-m-bias-cov}
\]
这证实了未调整估计量是无偏的。但在给定 $X$ 的条件下，$Z$ 和 $Y$ 的偏相关系数为：\footnote{推导见附录 \cref{sec:appendix-partial-corr}}
\[
\rho_{ZY \mid X} \propto - a b c d ,
\tag{16.13}
\cref{eq:16-partial-corr}
\]
...
数学上，未调整的 OLS 估计量是：\footnote{推导见附录 \cref{eq:16-z-bias-unadj-derivation}}
\[
\tau_{\mathrm{unadj}} = \tau + \frac{c \, \operatorname{cov}(Z, U)}{\operatorname{var}(Z)}
= \tau + \frac{c b}{a^2 + b^2 + 1} ,
\tag{16.15}
\cref{eq:16-z-bias-unadj}
\]
其偏倚为 $b c / (a^2 + b^2 + 1)$。

调整后的 OLS 估计量（从 $Y$ 对 $(Z, X)$ 的回归）满足：\footnote{推导见附录 \cref{eq:16-z-bias-adj-derivation}}
\[
\tau_{\mathrm{adj}} = \tau + \frac{b c}{b^2 + 1} .
\tag{16.16}
\cref{eq:16-z-bias-adj}
\]
