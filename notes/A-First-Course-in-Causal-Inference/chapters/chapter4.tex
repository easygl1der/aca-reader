...
这个引理的证明是直接的代数运算。这个恒等式将在后续推导中发挥关键作用。\footnote{详见附录 \cref{sec:appendix-lemma-4-1}。}
...
证明 \eqref{eq:4-3}（即 OLS 系数等于差值均值）。证明 EHW 稳健方差估计量的 HC2 变体恰好等于 $\hat{V}$。
\end{Exercise}
\noindent\textbf{推导见附录} \cref{sec:appendix-4-3-5}。
