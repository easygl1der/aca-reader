...
即在协变量 $X$ 条件 下，治疗分配 $Z$ 与潜在结果相互独立。这个假设保证了我们可以从观测数据中识别因果效应。然而，\textbf{可忽略性假设无法从数据本身验证}——它是一种根本性的假设，依赖于研究者的领域知识和对因果结构的判断。
...
放弃假设（\ref{eq:17-ignorability}）并不意味着放弃一切分析工具。我们仍然可以假设\textbf{潜在可忽略性（latent ignorability）}：
...
其中 $U$ 是未测混杂变量，条件于 $(X,U)$ 时治疗与潜在结果独立。注意（\ref{eq:17-latent-ignorability}）比（\ref{eq:17-ignorability}）更弱——它允许 $Z$ 与 $Y$ 之间存在通过 $U$ 传递的关联，只要这种关联可以被 $U$ 完全解释。
...
\[
\RR_{ZY|x}^{\mathrm{true}} = \frac{\pr\{Y(1)=1 \mid X=x\}}{\pr\{Y(0)=1 \mid X=x\}} .
\tag{17.4}
\tag{17.5}
\]
...
\[
\RR_{ZY|x}^{\mathrm{obs}} = \frac{\pr(Y=1 \mid Z=1, X=x)}{\pr(Y=1 \mid Z=0, X=x)} .
\tag{17.6}
\tag{17.7}
\]
...
\[
\RR_{ZU|x} = \frac{\pr(U=1 \mid Z=1, X=x)}{\pr(U=1 \mid Z=0, X=x)} ,
\tag{17.8}
\tag{17.9}
\]
...
\[
\RR_{UY|x} = \frac{\pr(Y=1 \mid U=1, X=x)}{\pr(Y=1 \mid U=0, X=x)} ,
\tag{17.10}
\tag{17.11}
\]
...
在假设 (\ref{eq:17-latent-ignorability}) 下，假设
\[
\RR_{ZY|x}^{\mathrm{obs}} > 1, \quad \RR_{ZU|x} > 1, \quad \RR_{UY|x} > 1 .
\tag{17.12}
\tag{17.13}
\]
则有
\[
\RR_{ZY|x}^{\mathrm{obs}} \leq \frac{\RR_{ZU|x} \cdot \RR_{UY|x}}{\RR_{ZU|x} + \RR_{UY|x} - 1} .
\tag{17.14}
\tag{17.15}
\]
\end{Theorem}

\textbf{定理 \ref{th:17-cornfield} 的含义}：在潜在可忽略性下，治疗与结果之间的观测关联完全来自两条路径——$Z\leftarrow U \rightarrow Y$——的组合效应。上界 $\RR_{ZU|x} \cdot \RR_{UY|x} / (\RR_{ZU|x} + \RR_{UY|x} - 1)$ 体现了这两条路径的"乘法阻塞"：要产生一个较大的 $\RR_{ZY|x}^{\mathrm{obs}}$，两条路径都必须足够强。

特别地，从 (\ref{eq:17-cornfield-bound}) 可推出 \textbf{Cornfield 不等式}（Gastwirth et al., 1998）：
\[
\min(\RR_{ZU|x}, \RR_{UY|x}) \geq \RR_{ZY|x}^{\mathrm{obs}} .
\tag{17.16}
\tag{17.17}
\]
即：要完全解释掉观测到的风险比，未测混杂与治疗的关联以及与结果的关联，都必须\textbf{至少与观测到的风险比一样强}。这个简单而强大的结论，首次由 Cornfield et al. (1959) 在讨论吸烟与肺癌时得到。

\textbf{证明思路}：定义 $f_{1,x} = \pr(U=1 \mid Z=1, X=x)$ 和 $f_{0,x} = \pr(U=1 \mid Z=0, X=x)$。通过全概率公式分解 $\RR_{ZY|x}^{\mathrm{obs}}$ 并利用 $Z \perp Y \mid (X,U)$，可得恒等式 (\ref{eq:17-rr-identity})。\footnote{完整证明推导见附录 \cref{sec:appendix-17-proof-theorem}}

\section{E-value}\label{sec:17-evalue}

\subsection{从 Cornfield 不等式到 E-value}\label{sec:17-from-cornfield-to-evalue}

从定理 \ref{th:17-cornfield} 和引理 \ref{lm:17-beta-properties}(4)，定义 $w = \max(\RR_{ZU|x}, \RR_{UY|x})$，则
\[
\RR_{ZY|x}^{\mathrm{obs}} \leq \frac{w^2}{2w - 1} .
\tag{17.18}
\tag{17.19}
\]
...
\[
w \geq \RR_{ZY|x}^{\mathrm{obs}} + \sqrt{\RR_{ZY|x}^{\mathrm{obs}}(\RR_{ZY|x}^{\mathrm{obs}} - 1)} .
\tag{17.20}
\tag{17.21}
\]
...
给定观测条件风险比 $\RR_{ZY|x}^{\mathrm{obs}}$，定义 \textbf{E-value} 为
\[
\RR_{ZY|x}^{\mathrm{obs}} + \sqrt{\RR_{ZY|x}^{\mathrm{obs}}(\RR_{ZY|x}^{\mathrm{obs}} - 1)} .
\tag{17.22}
\tag{17.23}
\]
\end{Definition}

\textbf{E-value 的解释}：E-value 表示要完全解释掉观测到的风险比，$\max(\RR_{ZU|x}, \RR_{UY|x})$ 必须达到的最小值。换言之，如果领域知识认为任何合理的未测混杂，其与治疗和结果的关联程度都不可能超过某个阈值 $R$，那么当 $R < \mathrm{E-value}$ 时，我们有理由相信因果关系确实存在。
...
\begin{itemize}
  \item 解释点估计的 E-value：
    \[
    \mathrm{E-value} = 10.73 + \sqrt{10.73 \times 9.73} \approx 20.95 .
    \]
  \item 解释下限的 E-value：
    \[
    \mathrm{E-value}_{\mathrm{CI}} = 8.02 + \sqrt{8.02 \times 7.02} \approx 15.52 .
    \end{itemize}
\end{itemize}
...
图 \ref{fig:17-confounding-contour}（Hammond and Horn 数据的混淆强度轮廓）展示了要解释掉点估计和置信下限，两类混淆强度 $(\RR_{ZU|x}, \RR_{UY|x})$ 需要满足的条件。
...
当结果罕见时（$\pr(Y=1 \mid Z, X)$ 接近 0），条件比数比近似于条件风险比（见命题 \ref{prop:1-rare-approx}）。\footnote{详见附录 \cref{sec:appendix-17-rare-approx}}

因此，在罕见结果假设下，我们可以直接基于逻辑斯蒂回归估计的 $\hat\beta_1$ 计算 E-value：

\[
\widehat{\mathrm{E-value}} = \exp(\hat\beta_1) + \sqrt{\exp(\hat\beta_1)(\exp(\hat\beta_1) - 1)} .
\tag{17.26}
\tag{17.27}
\]
...
修改 $\RR_{UY|x}$ 的定义为：
\[
\RR_{UY|x} = \max_{z=0,1} \frac{\pr(Y=1 \mid Z=z, U=1, X=x)}{\pr(Y=1 \mid Z=z, U=0, X=x)} .
\tag{17.28}
\tag{17.29}
\]
则在假设 (\ref{eq:17-latent-ignorability}) 下，
\[
\RR_{ZY|x}^{\mathrm{true}} \geq \frac{\RR_{ZY|x}^{\mathrm{obs}}}{\frac{\RR_{ZU|x} \cdot \RR_{UY|x}}{\RR_{ZU|x} + \RR_{UY|x} - 1}} .
\tag{17.30}
\tag{17.31}
\]
\end{Theorem}

\textbf{定理 \ref{th:17-true-rr-bound} 的解读}：即使存在未测混杂，定理也给出了真实因果效应 $\RR_{ZY|x}^{\mathrm{true}}$ 的一个下界。这个下界等于观测风险比除以 Cornfield 上界。当灵敏度参数取不同值时，我们可以评估因果效应下界对未测混杂强度的敏感性。
...
从图 \ref{fig:17-evalue-vs-rr} 可以看出，当风险比较大时，E-value 近似等于 $2 \cdot \RR_{ZY|x}^{\mathrm{obs}}$——几乎是线性的。因此批评者认为 E-value 不过是风险比的一个单调变换，没有提供额外信息。
...
E-value 等于 $\max(\RR_{ZU|x}, \RR_{UY|x})$——即要解释掉观察到的关联，未测混杂与治疗和结果关联所必须达到的最小值。问题在于：这个未测混杂本身是无法观测的，因此很难判断某个 E-value 究竟算"大"还是"小"。
...
\begin{enumerate}
  ...
  \item \textbf{Cornfield 不等式}：要完全解释掉风险比 $\RR^{\mathrm{obs}}$，未测混杂与治疗的关联和与结果的关联都必须至少达到 $\RR^{\mathrm{obs}}$；
  \item \textbf{E-value} 是这一思想的精确数学化：$\mathrm{E-value} = \RR^{\mathrm{obs}} + \sqrt{\RR^{\mathrm{obs}}(\RR^{\mathrm{obs}} - 1)}$；
...
证明在假设 $Z \perp Y \mid (X,U)$ 下，如果 $\RR_{ZU|x} > 1$ 但 $\RR_{UY|x} < 1$，则 $\RR_{ZY|x}^{\mathrm{obs}} < 1$。这说明 (\ref{eq:17-assumptions}) 中的第三个条件在某种意义上是冗余的。
\end{HomeworkProblem}
...
（Schlesselman 1978 公式）在二值治疗 $Z$、结果 $Y$ 和未测混杂 $U$ 下，假设层内（$U=0$ 和 $U=1$）治疗对结果的风险比相同，$\RR_{ZY|U=0} = \RR_{ZY|U=1}$，且混杂对结果的风险比在两组治疗下也相同（记为 $\gamma$）。证明：
\[
...
我们要在假设 (\ref{eq:17-latent-ignorability}) 下，证明观测风险比的上界。核心思想是：条件于 $(X,U)$ 后，治疗 $Z$ 与结果 $Y$ 独立，因此观测风险比中的分子和分母分别是 $U$ 在治疗组和对照组中分布的加权平均。
...
由灵敏度参数的定义 (\ref{eq:17-sensitivity-zu})：
\[
\RR_{ZU|x} = \frac{f_{1,x}}{f_{0,x}} \quad \Longrightarrow \quad f_{0,x} = \frac{f_{1,x}}{\RR_{ZU|x}} .
\tag{17.A.3}
\tag{17.A.4}
\]

定义 $\gamma_x = \RR_{UY|x}$ 以简化记号。
...
代入 $f_{0,x} = f_{1,x} / \RR_{ZU|x}$，得：
\[
\pr(Y=1 \mid Z=0, X=x) = (\gamma_x - 1) \frac{f_{1,x}}{\RR_{ZU|x}} + 1 .
\tag{17.A.9}
\tag{17.A.10}
\]
...
因此观测风险比可以写成：
\[
\RR_{ZY|x}^{\mathrm{obs}} = \frac{(\gamma_x - 1) f_{1,x} + 1}{(\gamma_x - 1) \frac{f_{1,x}}{\RR_{ZU|x}} + 1} .
\tag{17.A.11}
\tag{17.A.12}
\]
...
\]
在 $a > 1, b > 1$ 时关于 $t$ 单调递增（习题 \ref{hw:17-1}）。因此 $f_{1,x}$ 越大，$\RR_{ZY|x}^{\mathrm{obs}}$ 越大。取上界 $f_{1,x} = 1$：
\[
\RR_{ZY|x}^{\mathrm{obs}} \leq \frac{(\gamma_x - 1) + 1}{\frac{\gamma_x - 1}{\RR_{ZU|x}} + 1} = \frac{\gamma_x \cdot \RR_{ZU|x}}{\gamma_x + \RR_{ZU|x} - 1} .
\tag{17.A.13}
\tag{17.A.14}
\]

这正是定理 \ref{th:17-cornfield} 中的上界 (\ref{eq:17-cornfield-bound})。

\section{恒等式 (\ref{eq:17-rr-identity})}\label{sec:appendix-17-identity}

由以上推导，我们实际得到了一个精确的恒等式（而非仅仅是上界）：

\[
\RR_{ZY|x}^{\mathrm{obs}} = \frac{(\RR_{UY|x} - 1) f_{1,x} + 1}{\frac{\RR_{UY|x} - 1}{\RR_{ZU|x}} f_{1,x} + 1} .
\tag{17.A.15}
\tag{17.A.16}
\]

这个恒等式涉及三个灵敏度参数 $(f_{1,x}, \RR_{ZU|x}, \RR_{UY|x})$，而定理 \ref{th:17-cornfield} 的上界只涉及后两者。数学上恒等式强于不等式，但恒等式需要多一个灵敏度参数——这是灵敏度分析中\textbf{准确度与便利性之间的权衡}。
...
对于二值结果 $Y$，逻辑斯蒂模型 (\ref{eq:17-logistic-model}) 中治疗指示变量 $Z$ 的系数 $\beta_1$ 是条件\textbf{比数比（odds ratio）}：

\[
\beta_1 = \log \mathrm{OR}_{ZY|x} = \log \frac{\mathrm{odds}(Y=1 \mid Z=1, X=x)}{\mathrm{odds}(Y=1 \mid Z=0, X=x)} .
\tag{17.C.1}
\tag{17.C.2}
\]
...
\[
\beta_1 \approx \log \RR_{ZY|x}^{\mathrm{obs}} .
\tag{17.C.5}
\tag{17.C.6}
\]

由此可得 $\exp(\beta_1) \approx \RR_{ZY|x}^{\mathrm{obs}}$，进而 E-value 的计算公式 (\ref{eq:17-evalue-formula}) 可直接应用于逻辑斯蒂回归系数。
