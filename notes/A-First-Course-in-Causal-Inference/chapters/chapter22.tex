...
\chapter{依从性与意向治疗分析}\label{ch:22}
...
\tag{22.1}\label{eq:22-monotonicity-inequality}
\]
...
\tag{22.2}\label{eq:22-tau-d-monotonicity}
\]
...
\tag{22.3}\label{eq:22-tau-y-under-exclusion}
\]
...
\tag{22.4}\label{eq:22-cace-formula}
\]
\end{Theorem}
...
\[
\tau_c = \frac{\tau_Y}{\tau_D}.
\tag{22.5}\label{eq:22-cace-ratio}
\]
...
\tag{22.6}\label{eq:22-cace-observed}
\]
...
\tag{22.7}\label{eq:22-wald-estimator}
\]
其中
\[
...
\tag{22.8}\label{eq:22-adjusted-variance}
\]
...
\tag{22.9}\label{eq:22-cace-variance}
\]
作为 $\hat{\tau}_c$ 的方差估计量。
...
这就是\textbf{弱工具变量（weak instrumental variable）}问题。在这种情况下：
\begin{itemize}
...
\section{CACE 的解释}\label{sec:22-cace-interpretation}
