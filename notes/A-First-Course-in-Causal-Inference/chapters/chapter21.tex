...
\begin{figure}[ht]
  \centering
  \safeincludegraphics[width=0.9\textwidth]{R/figures/ch21-weak-iv-hist.pdf}
...
\begin{itemize}
  ...
  \item $Y$：求职自我效能（\texttt{job\_seek}，取值 1--5 的离散量表）
...
\section{习题}\label{sec:21-homework}

\begin{HomeworkProblem}[Variance of the Wald estimator]\label{exr:21-1}
...
\begin{HomeworkProblem}[Asymptotic variance of the Wald estimator and its estimation]\label{exr:21-2}
...
\begin{HomeworkProblem}[Proof of the main theorem of  \citet{imbens1994identification} and \citet{angrist1996identification}]\label{exr:21-3}
...
\begin{HomeworkProblem}[More on the FAR confidence set]\label{exr:21-4}
...
\begin{HomeworkProblem}[More simulation for the FAR confidence set]\label{exr:21-5}
...
\begin{HomeworkProblem}[Binary IV and ordinal treatment received]\label{exr:21-6}
...
\begin{HomeworkProblem}[Data analysis: a flu shot encouragement design \citep{mcdonald1992effects}]\label{exr:21-7}
...
\begin{HomeworkProblem}[IV estimation conditional on covariates]\label{exr:21-8}
...
\begin{HomeworkProblem}[Data analysis: the Karolinska data]\label{exr:21-9}
...
\begin{HomeworkProblem}[Data analysis: a job training program]\label{exr:21-10}
...
\begin{HomeworkProblem}[Recommended reading]\label{exr:21-11}
