...
基于 \cref{th:19-worst-case}，我们可以计算最坏情况 p 值。对于给定的观测检验统计量 $T^{\obs}$，双边 p 值为：
\[
\text{p}_{\Gamma} = \Pr_{\Gamma}\left(|T - T^{\obs}| \geq |T^{\obs}|\right),
...
设 $T^{\obs}$ 为观测到的检验统计量。证明：最坏情况 p 值 $\text{p}_{\Gamma}$ 是 $\Gamma$ 的单调递增函数。解释这在直觉上意味着什么。
\end{Exercise}
