...
\begin{Theorem}[弱大数定律]\label{thm:lln}
...
\begin{Theorem}[Delta 方法]\label{thm:delta-method}
...
\log\widehat{\text{RR}} &= \log\frac{\hat{p}_1}{\hat{p}_0}, \\
\log\widehat{\text{OR}} &= \log\frac{\hat{p}_1/(1-\hat{p}_1)}{\hat{p}_0/(1-\hat{p}_0)} = \log\frac{n_{11}n_{00}}{n_{10}n_{01}} .
\end{align*}
...
考虑三元正态随机向量 $(Y, X, Z)^\top \sim N(\mathbf{0}, \Sigma)$。证明偏相关系数公式：
\[
\rho_{YZ|X} = \frac{\rho_{YZ} - \rho_{YX}\rho_{ZX}}{\sqrt{1-\rho_{YX}^2}\sqrt{1-\rho_{ZX}^2}} .
\]
\end{Exercise}
...
\textbf{历史注记}：Eve's Law 最早由 Laplace 在 1774 年提出，但 Joe Blitzstein 教授在哈佛统计系任教期间广泛推广了这个名称。
\endgroup
\end{Exercise}
