...
\chapter{加权最小二乘法用于因果推断}\label{ch:14}
...
\begin{Theorem}[定理 14.2 — 双重稳健性]\label{tab:thm-14-2}
若 $\bar{X}=0$ 且
\[
...
$\hat{\tau}_T^{\text{hajek}}$ 严格等于如下 WLS 回归中 $Z_i$ 的系数：
\[
...
\begin{Theorem}[定理 14.3 — 处理组的双重稳健性]\label{tab:thm-14-3}
...
讨论在何种情形下 WLS 估计量比 OLS 估计量更有效。在 Freedman and Berk (2008) 的模拟设置中，为什么 OLS 表现更好？这与本 章讨论的异方差情形有何关联？
\end{Exercise}
