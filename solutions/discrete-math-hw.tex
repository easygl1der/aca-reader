...
\setCJKmainfont{Songti SC}            % 中文衬线（宋体）
\setCJKsansfont{Heiti SC}            % 中文无衬线（黑体）
\setCJKmonofont{LXGW WenKai Mono}    % 中文等宽
...
%  数学命令
%----------------------------------------------------------------------------------------
%\d is already defined by amsmath as differential operator
...
\newenvironment{question}[2][]{
  ...
  \ifthenelse{\equal{#1}{}}{
    ...
    \ifthenelse{\equal{#2}{}}{\section*{(#1)}}{%
      \section*{(#1) \quad #2}}}
  \addcontentsline{toc}{section}{\ifthenelse{\equal{#2}{}}{#1}{#2}}
  \ifthenelse{\equal{#1}{}}{}{
    \lfoot{\small\itshape 第\thesection题（#1）未完，下页继续\ldots}
  }
}{
  \lfoot{}
}
...
%  Warning / Info
%----------------------------------------------------------------------------------------
\mdfdefinestyle{warning}{topline=false,bottomline=false,leftline=false,rightline=false,nobreak,singleextra={\draw(P-|O)++(-0.5em,0)node(tmp1){};\draw(P-|O)++(0.5em,0)node(tmp2){};\fill[black,rotate around={45:(P-|O)}](tmp1)rectangle(tmp2);\node at(P-|O){\color{white}\scriptsize\bf!};\draw[very thick](P-|O)++(0,-1em)--(O);}}
\newenvironment{warn}[1][Warning:]{\begin{mdframed}[style=warning]\noindent{\textbf{#1}}}{\end{mdframed}}

\mdfdefinestyle{info}{topline=false,bottomline=false,leftline=false,rightline=false,nobreak,singleextra={\draw(P-|O)++(-0.5em,0)node(tmp1){};\draw(P-|O)++(0.5em,0)node(tmp2){};\fill[black](P-|O)circle[radius=0.4em];\node at(P-|O){\color{white}\scriptsize\bf i};\draw[very thick](P-|O)++(0,-0.8em)--(O);}}
\newenvironment{info}[1][Info:]{\begin{mdframed}[style=info]\noindent{\textbf{#1}}}{\end{mdframed}}
...
\begin{question}{3-6(6)}
  ...
  \begin{Answer}
    ...
    \par\noindent\textbf{注：} 传递性证明的关键步骤——由对称性已知 $\langle b,a\rangle \in R$，再对 $(*)$ 取 $a\mapsto b$，$b\mapsto a$，$c\mapsto c$，由 $\langle b,a\rangle, \langle b,c\rangle \in R$ 推得 $\langle a,c\rangle \in R$。
  \end{Answer}
\end{question}
...
\begin{question}{3-7(1)}
  ...
  \begin{subq}{(d) 若 $R_1$ 和 $R_2$ 是传递的，则 $R_1 \circ R_2$ 也是传递的。}
    \begin{Answer}
      ...
      由 $R_2$ 传递性：$uR_2y$ 和 $yR_1v$ 不能直接传递，但由 $R_2$ 的传递性只能处理 $R_2$ 内部关系。正确路径为：由 $R_1$ 传递性，$xR_1u$ 和 $uR_2y$ 不能直接传递，须另寻。
