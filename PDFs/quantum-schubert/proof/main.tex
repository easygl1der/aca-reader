...
The coefficient $t_2 - y_1$ explicitly forms the exact geometric root basis element.
\end{example}
...
Every extracted polynomial is a strictly positive sum or product of the variables $(t_j - y_i)$.
\end{example}
...
The coefficient smoothly sums to $t_3 - t_1$, a manifest positive root.
\end{example}

\begin{example}[Point Overlap in $\tau$]
$$ h_2(\tau t) \circ h_2(t) = q h_1(t) + (t_3 - t_1 + t_1 - t_2) q + (t_3 - t_2)(t_3 - t_1) h_2(t) $$
This collapses beautifully to $q h_1(t) + (t_3 - t_2) q + (t_3 - t_2)(t_3 - t_1) h_2(t)$.
\end{example}
...
Notice the term $(t_2 - t_3)$. In the geometric problem, the positivity bounds mandate $c_{u,v}^w \in \mathbb{N}[t_j - t_{\tau(i)}]$. Here, $t_2 - t_{\tau(1)} = t_2 - t_3$, confirming the basis constraint is strictly satisfied.
\end{example}
...
All 20 examples concretely verify that every evaluated polynomial strictly respects the structural positivity dictated by the permutation limits.
\end{example}
