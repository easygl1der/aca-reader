...
description: Analyze videos using Google's Gemini API - describe content, answer questions, transcribe audio with visual descriptions, reference timestamps, clip videos, and process YouTube URLs. Supports 9 video formats, multiple models (Gemini 3.1/2.5/2.0/1.5), and context windows up to 2M tokens (6 hours of video).
license: MIT
allowed-tools:
  - Bash
  - Read
...
**Context Windows:**
- 2M token models (Gemini 3.1): ~2 hours (default) or ~6 hours (low-res)
- 1M token models (Gemini 2.5/1.5): ~1 hour (default) or ~3 hours (low-res)
...
- **1.1.0** (2026-03-14): Added Gemini 3.1 series models, added --list-models flag
