...
```bash
python3 ~/.claude/skills/xhs-fetch/download_xhs.py
```
...
```bash
# 默认模式（智能类型判断）- 采集首页5条
python3 ~/.claude/skills/xhs-fetch/xhs_workflow.py -n 5

# 指定分析模式
python3 ~/.claude/skills/xhs-fetch/xhs_workflow.py -n 5 -m summary      # 内容总结
python3 ~/.claude/skills/xhs-fetch/xhs_workflow.py -n 5 -m visual      # 视觉策划
python3 ~/.claude/skills/xhs-fetch/xhs_workflow.py -n 5 -m monetize    # 变现分析

# 单链接模式 - 直接分析你发送的小红书链接
python3 ~/.claude/skills/xhs-fetch/xhs_workflow.py -u "https://www.xiaohongshu.com/explore/xxx?xsec_token=xxx"
python3 ~/.claude/skills/xhs-fetch/xhs_workflow.py -u "链接" -m summary   # 单链接 + 指定模式
```
...
```bash
# 默认模式
python3 ~/.claude/skills/xhs-fetch/analyze_folder.py ~/tmp/xhs/01_笔记标题 -m default

# 指定模式
python3 ~/.claude/skills/xhs-fetch/analyze_folder.py ~/tmp/xhs/01_笔记标题 -m summary
python3 ~/.claude/skills/xhs-fetch/analyze_folder.py ~/tmp/xhs/01_笔记标题 -m visual
python3 ~/.claude/skills/xhs-fetch/analyze_folder.py ~/tmp/xhs/01_笔记标题 -m monetize
```
...
```bash
# 为最新下载的文件夹生成书面稿
python3 ~/.claude/skills/xhs-fetch/generate_script.py

# 为指定文件夹生成书面稿
python3 ~/.claude/skills/xhs-fetch/generate_script.py ~/tmp/xhs/01_笔记标题
```
