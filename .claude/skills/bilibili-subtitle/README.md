...
| `DASHSCOPE_API_KEY` 或 `ASSEMBLYAI_API_KEY` | 无字幕视频时的 ASR | 条件必须 |
| `ANTHROPIC_API_KEY` | 校对与摘要 | 可选 |

说明：

- 如果不需要 LLM 校对/摘要，可加 `--skip-proofread --skip-summary`
- 如果视频本身有字幕，可不配置 ASR API Key
- **AssemblyAI** (`universal-2` 模型) 输出自带标点，格式完整，可直接使用
- **DASHSCOPE** (Qwen ASR) 输出自带标点，但速度较慢
