...
## New: Automated Script with AssemblyAI + MiniMax

**A new Python script is available** that automates the entire process with better results:

### Features

- **Auto-detect subtitles**: Try YouTube subtitles first
- **AssemblyAI fallback**: If no subtitles, download and transcribe with AssemblyAI
- **MiniMax formatting**: Convert raw transcript to clean written form with punctuation
- **MiniMax summarization**: Generate structured video summary
- **Dual output**: Both transcript and summary saved to Obsidian

### Usage

```bash
python3 ~/.claude/skills/youtube-transcript/youtube_summary.py "YOUTUBE_URL"
```
...
| File | Description |
|------|-------------|
| `YouTube/youtube_summaries.md` | Structured video summaries |
| `YouTube/youtube_transcripts.md` | Full transcript written form |

### Workflow

```
1. Get video info (title, channel, duration)
2. Check for YouTube subtitles
   ├─ Has subtitles → parse VTT → format with MiniMax
   └─ No subtitles → download video → extract audio → AssemblyAI → MiniMax
3. Generate summary with MiniMax
4. Save to Obsidian
```
...
| API | Purpose | Note |
|-----|---------|------|
| `ASSEMBLYAI_API_KEY` | Audio transcription | Free tier: 10 min limit |
| `ANTHROPIC_AUTH_TOKEN` (MiniMax) | Format + Summary | Needs MiniMax token |

### Example

```bash
# Process a video
python3 ~/.claude/skills/youtube-transcript/youtube_summary.py "https://www.youtube.com/watch?v=dQw4w9WgXcQ"

# Or just the video ID
python3 ~/.claude/skills/youtube-transcript/youtube_summary.py "dQw4w9WgXcQ"
```

### Comparison: Old vs New

| Feature | Old (yt-dlp only) | New (Script) |
|---------|-------------------|--------------|
| Subtitle download | ✅ | ✅ |
| Transcript formatting | Manual | ✅ MiniMax |
| Video summary | ❌ | ✅ MiniMax |
| Save to Obsidian | ❌ | ✅ |
| No-subtitle handling | Whisper (local) | ✅ AssemblyAI (API) |
